...
This document covers the command reference and internals of rezops's headless browser.
...
rezops's browser is a compiled CLI binary that talks to a persistent local Chromium daemon over HTTP. The CLI is a thin client — it reads a state file, sends a command, and prints the response to stdout. The server does the real work via [Playwright](https://playwright.dev/).
...
`viewport --scale <n>` sets Playwright's `deviceScaleFactor` (context-level option, 1-3 rezops policy cap). A 2x scale doubles the pixel density of screenshots:
...
Each server session generates a random UUID as a bearer token. The token is written to the state file (`.rezops/browse.json`) with chmod 600. Every HTTP request that mutates browser state must include `Authorization: Bearer <token>`. This prevents other processes on the machine from controlling the browser.
...
SSE endpoints (`/activity/stream`, `/inspector/events`) accept the Bearer token OR the HttpOnly `REZOPS_sse` session cookie (30-minute stream-scope cookie minted by `POST /sse-session`). The `?token=<ROOT>` query-param auth is no longer supported.
...
- Console: `.rezops/browse-console.log`
- Network: `.rezops/browse-network.log`
- Dialog: `.rezops/browse-dialog.log`
...
The window has a subtle green shimmer line at the top edge and a floating "rezops" pill in the bottom-right corner so you always know which Chrome window is being controlled.
...
# Click the rezops icon in toolbar → Open Side Panel
```
...
```bash
bin/rezops-extension    # opens chrome://extensions, copies path to clipboard
```
...
4. **Navigate to the extension folder:** Press **Cmd+Shift+G** in the file picker to open "Go to folder", then paste one of these paths:
   - Global install: `~/.claude/skills/rezops/extension`
   - Dev/source: `<rezops-repo>/extension`
...
5. **Pin it:** Click the puzzle piece icon (Extensions) in the toolbar → pin "rezops browse"
6. **Set the port:** Click the rezops icon → enter the port from `$B status` or `.rezops/browse.json`
7. **Open Side Panel:** Click the rezops icon → "Open Side Panel"
...
| **Connection pill** | Small "rezops" pill in the bottom-right corner of every page when connected |
...
Each workspace gets its own isolated browser instance with its own Chromium process, tabs, cookies, and logs. State is stored in `.rezops/` inside the project root (detected via `git rev-parse --show-toplevel`).

| Workspace | State file | Port |
|-----------|------------|------|
| `/code/project-a` | `/code/project-a/.rezops/browse.json` | random (10000-60000) |
| `/code/project-b` | `/code/project-b/.rezops/browse.json` | random (10000-60000) |
...
| `BROWSE_STATE_FILE` | `.rezops/browse.json` | Path to state file (CLI passes to server) |
...
| **rezops browse** | **~3s** | **~100-200ms** | **0 tokens** (plain text stdout) |

The context overhead difference compounds fast. In a 20-command browser session, MCP tools burn 30,000-40,000 tokens on protocol framing alone. rezops burns zero.
...
rezops skips all of this. Compiled binary. Plain text in, plain text out. No protocol. No schema. No connection management.
...
| File | Role |
|------|------|
| `browse/src/cli.ts` | Entry point. Reads `.rezops/browse.json`, sends HTTP to the server, prints response. |
...
1. Push your branch
2. Pull in the skill directory: `cd ~/.claude/skills/rezops && git pull`
3. Rebuild: `cd ~/.claude/skills/rezops && bun run build`

Or copy the binary directly: `cp browse/dist/browse ~/.claude/skills/rezops/browse/dist/browse`
