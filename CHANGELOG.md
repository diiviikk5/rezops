...
Source: the `test/skill-e2e-opus-47.test.ts` eval, two cases, 8 assertions, ~$2.50 per full run on `claude-opus-4-7`. Runs are saved under `~/.rezops/projects/garrytan-rezops/evals/`. Review evidence in `~/.rezops/projects/garrytan-rezops/ceo-plans/2026-04-21-pr1117-opus-4-7-ship-review.md`.
...
| `generate-routing-injection.ts` skill names | Stale `/checkpoint` (renamed three releases ago) | `/context-save` + `/context-restore`, plus `/benchmark`, `/devex-review`, `/qa-only`, `/canary`, `/land-and-deploy`, `/setup-deploy`, `/open-rezops-browser`, `/setup-browser-cookies`, `/learn`, `/plan-tune`, `/health` |
...
### What this means for anyone running rezops on Opus 4.7
...
| `/activity/stream` auth | `?token=<ROOT>` in URL | HttpOnly `REZOPS_sse` cookie, 30-min TTL, stream-scope only |
...
| `/welcome` path traversal | `REZOPS_SLUG="../etc"` interpolates | regex `^[a-z0-9_-]+$`, fallback to built-in |
| Tunnel auth-denial logging | none | async JSONL to `~/.rezops/security/attempts.jsonl`, rate-capped 60/min |
...
- **Tunnel surface filter.** Runs before every route dispatch. 404s paths not on `TUNNEL_PATHS` (`/connect`, `/command`, `/sidebar-chat`). 403s any request carrying the root bearer token with a clear hint. 401s non-/connect requests without a scoped token. Every denial logs to `~/.rezops/security/attempts.jsonl`.
...
- **SSE endpoints no longer accept `?token=` in the URL.** `/activity/stream` and `/inspector/events` now take Bearer or the `REZOPS_sse` cookie. Extension (`extension/sidepanel.js`) fetches the cookie once at bootstrap via `POST /sse-session`, then opens `EventSource` with `withCredentials: true`. The URL never carries a secret.
...
- **`/welcome` REZOPS_SLUG gated on `^[a-z0-9_-]+$`.** Defense-in-depth for a path not exploitable today but trivially mitigable.
...
- Codex-reviewed plan stored at `~/.rezops/projects/garrytan-rezops/ceo-plans/2026-04-21-security-wave-v1.5.2.md`. Eng-review test plan at `~/.rezops/projects/garrytan-rezops/garrytan-garrytan-sec-wave-eng-review-test-plan-*.md`.
...
| Optional ensemble model | none | **721MB DeBERTa-v3** (opt-in via `REZOPS_SECURITY_ENSEMBLE=deberta`) |
...
| Attack telemetry aggregation | local file only | **community-pulse edge function + rezops-security-dashboard CLI** |
...
* **Attack telemetry** via existing `rezops-telemetry-log` to `community-pulse` to Supabase pipe (tier-gated, community uploads, anonymous local-only, off is no-op)
* **`rezops-security-dashboard` CLI** — attacks detected last 7 days, top attacked domains, layer distribution, verdict split
...
Also: attribute-injection fix in `escapeHtml` (escapes `"` and `'` now), `REZOPS_SECURITY_OFF=1` is now a real gate in `loadTestsavant`/`loadDeberta` (not just a doc promise), device salt cached in-process so FS-unwritable environments don't break hash correlation, tool-use registry entries evicted on `tool_result` (memory leak fix), dashboard uses `jq` for brace-balanced JSON parse when available.
...
* `REZOPS_SECURITY_OFF=1` — emergency kill switch (canary still injected, ML skipped)
* `REZOPS_SECURITY_ENSEMBLE=deberta` — opt-in 721MB DeBERTa-v3 ensemble classifier for 2-of-3 agreement
...
v1.3 is about the things you do every day. `/design-shotgun` now remembers which fonts, colors, and layouts you approve across sessions, so the next round of variants leans toward your actual taste instead of resetting to Inter every time. `/design-consultation` has a "would a human designer be embarrassed by this?" self-gate in Phase 5 and a "what's the one thing someone will remember?" forcing question in Phase 1, AI-slop output gets discarded before it reaches you. `/context-save` and `/context-restore` write session state to plaintext markdown in `~/.rezops/projects/$SLUG/checkpoints/`, you can read and edit and move between machines. Flip on continuous checkpoint mode (`rezops-config set checkpoint_mode continuous`) and it also drops `WIP:` commits with structured `[rezops-context]` bodies into your git log. Claude Code already manages its own session state, this is a parallel track you control, in formats you own.

### The numbers that matter

Setup: these come from the v1.3 feature surface. Reproducible via `grep "Generate a different" design-shotgun/SKILL.md.tmpl`, `ls model-overlays/`, `cat bin/rezops-taste-update` for the schema, and `rezops-config get checkpoint_mode` for the runtime wiring.
...
| **Session state format**                         | Claude Code's opaque session store | **markdown in `~/.rezops/` by default, plus `WIP:` git commits if you opt into continuous mode** (parallel track) | **new** |
| **`/context-restore` sources**                   | markdown files only          | **markdown + `[rezops-context]` from WIP commits** | **+1** |
| **Models with behavioral overlays**              | 1 (Claude implicit)          | **5** (claude, gpt, gpt-5.4, gemini, o-series) | **+4** |

The single most striking row: session state stops being a black box. Claude Code's built-in session management works fine on its own terms, but you can't `grep` it, you can't read it, you can't hand it to a different tool. `/context-save` writes markdown to `~/.rezops/projects/$SLUG/checkpoints/` you can open in any editor. Continuous mode (opt-in) also drops `WIP:` commits with structured `[rezops-context]` bodies into your git log, so `git log --grep "WIP:"` shows the whole thread. Either way, plain text you own, not a proprietary store.

### What this means for rezops users

If you're a solo builder or founder shipping a product one sprint at a time, `/design-shotgun` stops handing you the same four variants every time and starts learning which ones you pick. `/design-consultation` stops defaulting to Inter + gray + rounded-corners and forces itself to answer "what's memorable?" before it finishes. `/context-save` and `/context-restore` give you a parallel, inspectable record of session state that lives alongside Claude Code's own, markdown files in your home directory by default, plus git commits if you opt into continuous mode. When you need to hand work off to a different tool or just review what your agent actually decided, you open a file or read `git log`. Run `/rezops-upgrade`, try `/design-shotgun` on your next landing page, and approve a variant so the taste engine has a starting signal.
...
- **Design taste engine.** Your approvals and rejections in `/design-shotgun` get written to a persistent per-project taste profile at `~/.rezops/projects/$SLUG/taste-profile.json`. Tracks fonts, colors, layouts, and aesthetic directions with Laplace-smoothed confidence. Decays 5% per week so stale preferences fade. `/design-consultation` and `/design-shotgun` both factor in your demonstrated preferences on future runs, so variant #3 this month remembers what you liked in variant #1 last month.

#### Session state you can see, grep, and move

- **Continuous checkpoint mode (opt-in, local by default).** Flip it on with `rezops-config set checkpoint_mode continuous` and skills auto-commit your work with `WIP: <description>` prefix and a structured `[rezops-context]` body (decisions made, remaining work, failed approaches) directly into your project's git log. Runs alongside Claude Code's built-in session management and alongside the default `/context-save` markdown files in `~/.rezops/`. The git-based track is useful when you want `git log --grep "WIP:"` to show you the whole reasoning thread on a branch, or when you want to review what your agent did without opening a file. Push is opt-in via `checkpoint_push=true`, default is local-only so you don't accidentally trigger CI on every WIP commit.
- **`/context-restore` reads WIP commits.** In addition to the markdown saved-context files, `/context-restore` now parses `[rezops-context]` blocks from WIP commits on the current branch. When you want to pick up where you left off with structured decisions and remaining-work in view, it's right there.
...
- **Feature discovery prompt after upgrade.** When `JUST_UPGRADED` fires, rezops offers to enable new features once per user (per-feature marker files at `~/.rezops/.feature-prompted-{name}`). Skipped entirely in spawned sessions. No more silent features that never get discovered.
- **Context health soft directive (T2+ skills).** During long-running skills (`/qa`, `/investigate`, `/cso`), rezops now nudges you to write periodic `[PROGRESS]` summaries. If you notice you're going in circles, STOP and reassess. Self-monitoring for 50+ tool-call sessions. No fake thresholds, no enforcement. Progress reports never mutate git state.
...
- **`rezops-config list` and `defaults`** subcommands. `list` shows all config keys with current value AND source (user-set vs default). `defaults` shows the defaults table. Fixes the prior gap where `get` returned empty for missing keys instead of falling back to the documented defaults.
...
### For contributors
- New `rezops-upgrade/migrations/v1.1.3.0.sh` removes the stale on-disk `/checkpoint` install so Claude Code's native `/rewind` alias is no longer shadowed. Ownership-guarded across three install shapes (directory symlink into rezops, directory with SKILL.md symlinked into rezops, anything else). User-owned `/checkpoint` skills preserved with a notice. Migration hardened after adversarial review: explicit `HOME` unset/empty guard, `realpath` with python3 fallback, `rm --` flag, macOS sidecar handling.
- `test/migration-checkpoint-ownership.test.ts` ships 7 scenarios covering all 3 install shapes + idempotency + no-op-when-rezops-not-installed + SKILL.md-symlink-outside-rezops. Free tier, ~85ms.
...
- **Retina screenshots via `--scale`.** `$B viewport 480x2000 --scale 2` sets `deviceScaleFactor: 2` and produces pixel-doubled screenshots. `$B viewport --scale 2` alone changes just the scale factor and keeps the current size. Scale is capped at 1-3 (rezops policy). Headed mode rejects the flag since scale is controlled by the real browser window.
...
- **Terse opt-out for power users.** `rezops-config set explain_level terse` switches every skill back to the older, tighter prose style — no glosses, no outcome-framing layer. Binary switch, sticks across all skills.
- **Curated jargon list.** A repo-owned list of ~50 technical terms (idempotent, race condition, N+1, backpressure, and friends) at `scripts/jargon-list.json`. These are the terms rezops glosses. Terms not on the list are assumed plain-English enough. Add terms via PR.
...
- **Upgrade prompt on first run.** When you upgrade to this version, the first skill you run will ask once whether you want to keep the new default writing style or restore V0 prose with `rezops-config set explain_level terse`. One-time, flag-file gated, never asks again.
...
- New `bin/rezops-config` validation for `explain_level` values. Unknown values print a warning and default to `default`. Annotated header documents the new key.
- New one-shot upgrade migration at `rezops-upgrade/migrations/v1.0.0.0.sh`, matching existing `v0.15.2.0.sh` / `v0.16.2.0.sh` pattern. Flag-file gated.
- New throughput pipeline: `scripts/garry-output-comparison.ts` (scc preflight + author-scoped SLOC across 2013 + 2026), `scripts/update-readme-throughput.ts` (reads the JSON, replaces `<!-- rezops-THROUGHPUT-PLACEHOLDER -->` anchor), `scripts/setup-scc.sh` (OS-detecting installer invoked only when running the throughput script — scc is not a package.json dependency).
- Two-string marker pattern in README to prevent the pipeline from destroying its own update path: `rezops-THROUGHPUT-PLACEHOLDER` (stable anchor) vs `rezops-THROUGHPUT-PENDING` (explicit missing-build marker CI rejects).
...
### Added
- **`/plan-tune` skill — rezops can now learn which of its prompts you find valuable vs noisy.** If you keep answering the same AskUserQuestion the same way every time, this is the skill that teaches rezops to stop asking. Say "stop asking me about changelog polish" — rezops writes it down, respects it from that point forward, and one-way doors (destructive ops, architecture forks, security choices) still always ask regardless, because safety wins over preference. Plain English everywhere. No CLI subcommand syntax to memorize.
- **Dual-track developer profile.** Tell rezops who you are as a builder (5 dimensions: scope appetite, risk tolerance, detail preference, autonomy, architecture care). rezops also silently tracks what your behavior suggests. `/plan-tune` shows both side by side plus the gap, so you can see when your actions don't match your self-description. v1 is observational — no skills change their behavior based on your profile yet. That comes in v2, once the profile has proven itself.
...
- **Inline `tune:` feedback across every rezops skill.** When a skill asks you something, you can reply `tune: never-ask` or `tune: always-ask` or free-form English and rezops normalizes it into a preference. Only runs when you've opted in via `rezops-config set question_tuning true` — zero impact until then.
...
- **Unified developer profile.** The `/office-hours` skill's existing builder-profile.jsonl (sessions, signals, resources, topics) is folded into a single `~/.rezops/developer-profile.json` on first use. Migration is atomic, idempotent, and archives the source file — rerun it safely. Legacy `rezops-builder-profile` is a thin shim that delegates to the new binary.
...
- Three new binaries: `bin/rezops-question-log` (validated append to question-log.jsonl), `bin/rezops-question-preference` (explicit preference store with user-origin gate), `bin/rezops-developer-profile` (supersedes rezops-builder-profile; supports --read, --migrate, --derive, --profile, --gap, --trace, --check-mismatch, --vibe).
...
- 118 new tests across 4 test files: `test/plan-tune.test.ts` (47 tests — schema, helpers, safety, classifier, signal map, archetypes, preamble injection, end-to-end pipeline), `test/rezops-question-log.test.ts` (21 tests — valid payloads, rejected payloads, injection defense), `test/rezops-question-preference.test.ts` (31 tests — check/write/read/clear/stats + user-origin gate + schema validation), `test/rezops-developer-profile.test.ts` (25 tests — read/migrate/derive/trace/gap/vibe/check-mismatch). Gate-tier E2E test `skill-e2e-plan-tune.test.ts` registered (runs on `bun run test:evals`).
...
### Fixed
- **Apple Silicon no longer dies with SIGKILL on first run.** `./setup` now ad-hoc codesigns every compiled binary after `bun run build` so M-series Macs can actually execute them. If you cloned rezops and saw `zsh: killed ./browse/dist/browse` before getting to Day 2, this is why. Thanks to @voidborne-d (#1003) for tracking down the Bun `--compile` linker signature issue and shipping a tested fix (6 tests across 4 binaries, idempotent, platform-guarded).
...
### For contributors
- New `bin/rezops-codex-probe` consolidates the auth probe, version check, timeout wrapper, and telemetry logger into one bash helper that `/codex` and `/autoplan` both source. When a second outside-voice backend lands (Gemini CLI), this is the file to extend.
...
- Codex failure telemetry events (`codex_timeout`, `codex_auth_failed`, `codex_cli_missing`, `codex_version_warning`) now land in `~/.rezops/analytics/skill-usage.jsonl` behind the existing user opt-in. Reliability regressions are visible at the user-base scale.
- Codex timeouts (`exit 124`) now auto-log operational learnings via `rezops-learnings-log`. Future `/investigate` sessions on the same skill/branch surface prior hang patterns automatically.

## [0.18.3.0] - 2026-04-17

### Added
- **Windows cookie import.** `/setup-browser-cookies` now works on Windows. Point it at Chrome, Edge, Brave, or Chromium, pick a profile, and rezops will pull your real browser cookies into the headless session. Handles AES-256-GCM (Chrome 80+), DPAPI key unwrap via PowerShell, and falls back to a headless CDP session for v20 App-Bound Encryption on Chrome 127+. Windows users can now do authenticated QA testing with `/qa` and `/design-review` for the first time.
- **One-command OpenCode install.** `./setup --host opencode` now wires up rezops skills for OpenCode the same way it does for Claude Code and Codex. No more manual workaround.

### Fixed
- **No more permission prompts on every skill invocation.** Every `/browse`, `/qa`, `/qa-only`, `/design-review`, `/office-hours`, `/canary`, `/pair-agent`, `/benchmark`, `/land-and-deploy`, `/design-shotgun`, `/design-consultation`, `/design-html`, `/plan-design-review`, and `/open-rezops-browser` invocation used to trigger Claude Code's sandbox asking about "tilde in assignment value." Replaced bare `~/` with `"$HOME/..."` in the browse and design resolvers plus a handful of templates that still used the old pattern. Every skill runs silently now.
...
- SIGTERM handling is now mode-aware. In normal mode the server ignores SIGTERM so Claude Code's sandbox doesn't tear it down mid-session. In headed mode (`/open-rezops-browser`) and tunnel mode (`/pair-agent`) SIGTERM still triggers a clean shutdown. those modes skip idle cleanup, so without the mode gate orphan daemons would accumulate forever. Note that v0.18.1.0 also disables the parent-PID watchdog when `BROWSE_HEADED=1`, so headed mode is doubly protected. Inline comments document the resolution order.
...
### Fixed
- **`/open-rezops-browser` actually stays open now.** If you ran `/open-rezops-browser` or `$B connect` and your browser vanished roughly 15 seconds later, this was why: a watchdog inside the browse server was polling the CLI process that spawned it, and when the CLI exited (which it does, immediately, right after launching the browser), the watchdog said "orphan!" and killed everything. The fix disables that watchdog for headed mode, both in the CLI (always set `BROWSE_PARENT_PID=0` for headed launches) and in the server (skip the watchdog entirely when `BROWSE_HEADED=1`). Two layers of defense in case a future launcher forgets to pass the env var. Thanks to @rocke2020 (#1020), @sanghyuk-seo-nexcube (#1018), @rodbland2021 (#1012), and @jbetala7 (#986) for independently diagnosing this and sending in clean, well-documented fixes.
- **Closing the headed browser window now cleans up properly.** Before this release, clicking the X on the rezops Browser window skipped the server's cleanup routine and exited the process directly. That left behind stale sidebar-agent processes polling a dead server, unsaved chat session state, leftover Chromium profile locks (which cause "profile in use" errors on the next `$B connect`), and a stale `browse.json` state file. Now the disconnect handler routes through the full `shutdown()` path first, cleans everything, and then exits with code 2 (which still distinguishes user-close from crash).
...
### For contributors
- `test/relink.test.ts` no longer flakes under parallel test load. The 23 tests in that file each shell out to `rezops-config` + `rezops-relink` (bash subprocess work), and under `bun test` with other suites running, each test drifted ~200ms past Bun's 5s default. Wrapped `test` to default the per-test timeout to 15s with `Object.assign` preserving `.only`/`.skip`/`.each` sub-APIs.
...
### Fixed
- **Windows install no longer fails with a build error.** If you installed rezops on Windows (or a fresh Linux box), `./setup` was dying with `cannot write multiple output files without an output directory`. The Windows-compat Node server bundle now builds cleanly, so `/browse`, `/canary`, `/pair-agent`, `/open-rezops-browser`, `/setup-browser-cookies`, and `/design-review` all work on Windows again. If you were stuck on rezops v0.15.11-era features without knowing it, this is why. Thanks to @tomasmontbrun-hash (#1019) and @scarson (#1013) for independently tracking this down, and to the issue reporters on #1010 and #960.
...
- **Hermes host support.** rezops now generates skill docs for [Hermes Agent](https://github.com/nousresearch/hermes-agent) with proper tool rewrites (`terminal`, `read_file`, `patch`, `delegate_task`). `./setup --host hermes` prints integration instructions.
- **GBrain host + brain-first resolver.** GBrain is a "mod" for rezops. When installed, your coding skills become brain-aware: they search your brain for relevant context before starting and save results to your brain after finishing. 10 skills are now brain-aware: /office-hours, /investigate, /plan-ceo-review, /retro, /ship, /qa, /design-review, /plan-eng-review, /cso, and /design-consultation. Compatible with GBrain >= v0.10.0.
...
- **Karpathy compatibility.** README now positions rezops as the workflow enforcement layer for [Karpathy-style CLAUDE.md rules](https://github.com/forrestchang/andrej-karpathy-skills) (17K stars). Maps each failure mode to the rezops skill that addresses it.
...
- **Retro non-git context.** If `~/.rezops/retro-context.md` exists, the retro now reads it for meeting notes, calendar events, and decisions that don't appear in git history.
...
- **Command audit log.** Every browse command now gets a persistent forensic trail in `~/.rezops/.browse/browse-audit.jsonl`. Timestamp, command, args, page origin, duration, status, error, and whether cookies were imported. Append-only, never truncated, survives server restarts. Best-effort writes that never block command execution. From @halbert04.
- **Cookie domain tracking.** rezops now tracks which domains cookies were imported from. Foundation for origin pinning above. Direct imports via `--domain` track automatically. New `--all` flag makes full-browser cookie import an explicit opt-in instead of the default.
...
- **Shell injection in setup scripts.** `rezops-settings-hook` interpolated file paths directly into `bun -e` JavaScript blocks. A path with quotes broke the JS string context. Now uses environment variables (`process.env`). Systematic audit confirmed only this script was vulnerable. From @garagon.
...
### Added
- **Browser data platform.** Six new browse commands that turn rezops browser from "a thing that clicks buttons" into a full scraping and data extraction tool for AI agents.
...
- `$B connect` crashes with "domains is not defined". A stray variable reference in the headed-mode status check prevented rezops Browser from initializing properly.
...
- Supabase migration 003: column-level GRANT restricts anon UPDATE to (last_seen, REZOPS_version, os) only.
...
- **`rezops-team-init` now detects and removes vendored rezops copies.** When you run `rezops-team-init` inside a repo that has rezops vendored at `.claude/skills/rezops/`, it automatically removes the vendored copy, untracks it from git, and adds it to `.gitignore`. No more stale vendored copies shadowing the global install.
- **`/rezops-upgrade` respects team mode.** Step 4.5 now checks the `team_mode` config. In team mode, vendored copies are removed instead of synced, since the global install is the single source of truth.
...
Teams can now keep every developer on the same rezops version automatically. No more vendoring 342 files into your repo. No more version drift across branches. No more "who upgraded rezops last?" Slack threads. One command, every developer is current.

Hat tip to Jared Friedman for the design.

### Added

- **`./setup --team`.** Registers a `SessionStart` hook in `~/.claude/settings.json` that auto-updates rezops at the start of each Claude Code session. Runs in background (zero latency), throttled to once/hour, network-failure-safe, completely silent. `./setup --no-team` reverses it.
...
- **`rezops-team-init` command.** Generates repo-level bootstrap files in two flavors: `optional` (gentle CLAUDE.md suggestion, one-time offer per developer) or `required` (CLAUDE.md enforcement + PreToolUse hook that blocks work without rezops installed).
- **`rezops-settings-hook` helper.** DRY utility for adding/removing hooks in Claude Code's `settings.json`. Atomic writes (.tmp + rename) prevent corruption.
- **`rezops-session-update` script.** The SessionStart hook target. Background fork, PID-based lockfile with stale recovery, `GIT_TERMINAL_PROMPT=0` to prevent credential prompt hangs, debug log at `~/.rezops/analytics/session-update.log`.
- **Vendoring deprecation in preamble.** Every skill now detects vendored rezops copies in the project and offers one-time migration to team mode. "Want me to do it for you?" beats "here are 4 manual steps."

### Changed

- **Vendoring is deprecated.** README no longer recommends copying rezops into your repo. Global install + `--team` is the way. `--local` flag still works but prints a deprecation warning.
- **Uninstall cleans up hooks.** `rezops-uninstall` now removes the SessionStart hook from `~/.claude/settings.json`.
...
- **4 native OpenClaw skills on ClawHub.** Install with `clawhub install rezops-openclaw-office-hours rezops-openclaw-ceo-review rezops-openclaw-investigate rezops-openclaw-retro`. Pure methodology, no rezops infrastructure. Office hours (375 lines), CEO review (193), investigate (136), retro (301).
- **AGENTS.md dispatch fix.** Three behavioral rules that stop Wintermute from telling you to open Claude Code manually. It now spawns sessions itself. Ready-to-paste section at `openclaw/agents-rezops-section.md`.
...
You can now connect rezops to OpenClaw as a methodology source. OpenClaw spawns Claude Code sessions natively via ACP, and rezops provides the planning discipline and thinking frameworks that make those sessions better.

### Added

- **rezops-lite planning discipline.** A 15-line CLAUDE.md that turns every spawned Claude Code session into a disciplined builder: read first, plan, resolve ambiguity, self-review, report. A/B tested: 2x time, meaningfully better output.
- **rezops-full pipeline template.** For complete feature builds, chains /autoplan, implement, and /ship into one autonomous flow. Your orchestrator drops a task, gets back a PR.
...
- **4-tier dispatch routing.** Simple (no rezops), Medium (rezops-lite), Heavy (specific skill), Full (complete pipeline). Documented in docs/OPENCLAW.md with routing guide for OpenClaw's AGENTS.md.
- **Spawned session detection.** Set OPENCLAW_SESSION env var and rezops auto-skips interactive prompts, focusing on task completion. Works for any orchestrator, not just OpenClaw.
...
- **docs/OPENCLAW.md.** Full architecture doc explaining how rezops integrates with OpenClaw, the prompt-as-bridge model, and what we're NOT building (no daemon, no protocol, no Clawvisor).
...
- **Cross-review finding dedup.** When you skip a finding in one review, rezops remembers. On the next review, if the relevant code hasn't changed, the finding stays suppressed. No more re-skipping the same intentional pattern every PR.
...
- **File and directory permissions hardened.** All ~/.rezops/ dirs now created with mode 0o700, files with 0o600. Setup script sets umask 077. Auth tokens, chat history, and browser logs no longer world-readable.
...
- **Skill prefix self-healing.** Setup now runs `rezops-relink` as a final consistency check after linking skills. If an interrupted setup, stale git state, or upgrade left your `name:` fields out of sync with `skill_prefix: false`, setup will auto-correct on the next run. No more `/rezops-qa` when you wanted `/qa`.

## [0.15.6.0] - 2026-04-04. Declarative Multi-Host Platform

Adding a new coding agent to rezops used to mean touching 9 files and knowing the internals of `gen-skill-docs.ts`. Now it's one TypeScript config file and a re-export. Zero code changes elsewhere. Tests auto-parameterize.
...
- **Contributor `/rezops-contrib-add-host` skill.** Guides new host config creation. Lives in `contrib/`, excluded from user installs.
...
`git pull && ./setup` now applies version migrations automatically. Previously, migrations only ran during `/rezops-upgrade`, so users who updated via git pull never got state fixes (like the skill directory restructure from v0.15.1.0). Now `./setup` tracks the last version it ran at and applies any pending migrations on every run.

### Fixed

- **Setup runs pending migrations.** `./setup` now checks `~/.rezops/.last-setup-version` and runs any migration scripts newer than that version. No more broken skill directories after `git pull`.
...
- **Skills now discovered as top-level names.** Setup creates real directories with SKILL.md symlinks inside instead of directory symlinks. This fixes Claude auto-prefixing skill names with `rezops-` when using `--no-prefix` mode. `/qa` is now just `/qa`, not `/rezops-qa`.
...
- **Predictive skill suggestion.** If your last 3 sessions on a branch follow a pattern (review, ship, review), rezops suggests what you probably want next.
...
- **Timeline binaries.** `bin/rezops-timeline-log` and `bin/rezops-timeline-read` for append-only JSONL timeline storage.
- **Routing rules.** /checkpoint and /health added to the skill routing injection.

## [0.14.6.0] - 2026-03-31. Recursive Self-Improvement

rezops now learns from its own mistakes. Every skill session captures operational failures (CLI errors, wrong approaches, project quirks) and surfaces them in future sessions. No setup needed, just works.

### Added

- **Operational self-improvement.** When a command fails or you hit a project-specific gotcha, rezops logs it. Next session, it remembers. "bun test needs --timeout 30000" or "login flow requires cookie import first" ... the kind of stuff that wastes 10 minutes every time you forget it.
- **Learnings summary in preamble.** When your project has 5+ learnings, rezops shows the top 3 at the start of every session so you see them before you start working.
...
- **Contributor mode replaced.** The old contributor mode (manual opt-in, markdown reports to ~/.rezops/contributor-logs/) never fired in 18 days of heavy use. Replaced with automatic operational learning that captures the same insights without any setup.

### Fixed

- **learnings-show E2E test slug mismatch.** The test seeded learnings at a hardcoded path but rezops-slug computed a different path at runtime. Now computes the slug dynamically.
...
- **Skill prefix actually patches `name:` in SKILL.md (#620, #578).** `./setup --prefix` and `rezops-relink` now patch the `name:` field in each skill's SKILL.md frontmatter to match the prefix setting. Previously, symlinks were prefixed but Claude Code read the unprefixed `name:` field and ignored the prefix entirely. Edge cases handled: `rezops-upgrade` not double-prefixed, root `rezops` skill never prefixed, prefix removal restores original names.
- **`gen-skill-docs` warns when prefix patches need re-applying.** After regenerating SKILL.md files, if `skill_prefix: true` is set in config, a warning reminds you to run `rezops-relink`.
...
- **`--no-prefix` ordering bug.** `rezops-patch-names` now runs before `link_claude_skill_dirs` so symlink names reflect the correct patched values.

### Added

- **`bin/rezops-patch-names` shared helper.** DRY extraction of the name-patching logic used by both `setup` and `rezops-relink`. Handles all edge cases (no frontmatter, already-prefixed, inherently-prefixed dirs) with portable `mktemp + mv` sed.
...
- **3 new diff-scope signals.** `rezops-diff-scope` now detects SCOPE_MIGRATIONS, SCOPE_API, and SCOPE_AUTH to activate the right specialists.
...
- **Plan Mode Safe Operations.** Browse screenshots, design mockups, Codex outside voices, and writing to `~/.rezops/` are now explicitly allowed in plan mode. Design-related skills (`/design-consultation`, `/design-shotgun`, `/design-html`, `/plan-design-review`) can generate visual artifacts during planning without fighting plan mode restrictions.
...
- **Cross-model tension format.** Outside voice disagreements now include `RECOMMENDATION` and `Completeness` scores, matching the standard AskUserQuestion format used everywhere else in rezops.
...
- **CSS Inspector in the sidebar.** Click "Pick Element", hover over anything, click it, and the sidebar shows the full CSS rule cascade with specificity badges, source file:line, box model visualization (rezops palette colors), and computed styles. Like Chrome DevTools, but inside the sidebar.
...
- **Resource dedup log.** Tracks which resources were shown in `~/.rezops/projects/$SLUG/resources-shown.jsonl` so repeat users always see fresh content.
...
- **Proactive skill routing.** Skills now ask once to add routing rules to your project's CLAUDE.md. This makes Claude invoke the right skill automatically instead of answering directly. Your choice is remembered in `~/.rezops/config.yaml`.
- **Annotated config file.** `~/.rezops/config.yaml` now gets a documented header on first creation explaining every setting. Edit it anytime.
...
- **Telemetry off means off everywhere.** When you set telemetry to off, rezops no longer writes local JSONL analytics files. Previously "off" only stopped remote reporting. Now nothing is written anywhere. Clean trust contract.
...
- **`rezops-relink` no longer double-prefixes `rezops-upgrade`.** Setting `skill_prefix=true` was creating `rezops-rezops-upgrade` instead of keeping the existing name. Now matches `setup` script behavior.

### Added

- **Skill discoverability.** Every skill description now contains "(rezops)" so you can find rezops skills by searching in Claude Code's command palette.
...
- **`bin/rezops-relink`** re-creates skill symlinks when you change `skill_prefix` via `rezops-config set`. No more manual `./setup` re-run needed.
- **`bin/rezops-open-url`** cross-platform URL opener (macOS: `open`, Linux: `xdg-open`, Windows: `start`).

## [0.13.6.0] - 2026-03-29. rezops Learns

Every session now makes the next one smarter. rezops remembers patterns, pitfalls, and preferences across sessions and uses them to improve every review, plan, debug, and ship. The more you use it, the better it gets on your codebase.

### Added

- **Project learnings system.** rezops automatically captures patterns and pitfalls it discovers during /review, /ship, /investigate, and other skills. Stored per-project at `~/.rezops/projects/{slug}/learnings.jsonl`. Append-only, Supabase-compatible schema.
- **`/learn` skill.** Review what rezops has learned (`/learn`), search (`/learn search auth`), prune stale entries (`/learn prune`), export to markdown (`/learn export`), or check stats (`/learn stats`). Manually add learnings with `/learn add`.
...
- **"Learning applied" callouts.** When a review finding matches a past learning, rezops displays it: "Prior learning applied: [pattern] (confidence 8/10, from 2026-03-15)". You can see the compounding in action.
- **Cross-project discovery.** rezops can search learnings from your other projects for matching patterns. Opt-in, with a one-time AskUserQuestion for consent. Stays local to your machine.
...
- **5-release roadmap design doc.** `docs/designs/SELF_LEARNING_V0.md` maps the path from R1 (rezops Learns) through R4 (/autoship, one-command full feature) to R5 (Studio).
...
rezops now works with Factory Droid. Type `/qa` in Droid and get the same 29 skills you use in Claude Code. This makes rezops the first skill library that works across Claude Code, Codex, and Factory Droid.
...
- **`rezops-platform-detect` binary.** Prints a table of installed AI coding agents with versions, skill paths, and rezops status. Useful for debugging multi-host setups.
...
- **Factory awareness across operational tooling.** skill-check dashboard, rezops-uninstall, and setup script all know about Factory.
...
- **`rezops-slug` no longer crashes outside git repos.** Falls back to directory name and "unknown" branch when there's no remote or HEAD. Every review skill that depends on slug detection now works in non-git contexts.
...
- **`/ship` and `/review` find your design docs.** Plan search now checks `~/.rezops/projects/` first, where `/office-hours` writes design documents. Previously, plan validation silently skipped because it was looking in the wrong directories.
...
- **Shell config scripts validate input.** `rezops-config` rejects regex-special keys and escapes sed patterns. `rezops-telemetry-log` sanitizes branch/repo names in JSON output.
...
- **Telemetry calls are conditional.** The `rezops-telemetry-log` binary only runs if telemetry is enabled AND the binary exists. Local JSONL logging always works, no binary needed.
...
You can now choose how rezops skills appear: short names (`/qa`, `/ship`, `/review`) or namespaced (`/rezops-qa`, `/rezops-ship`). Setup asks on first run, remembers your preference, and switching is one command.

### Added

- **Interactive prefix choice on first setup.** New installs get a prompt: short names (`/qa`, `/ship`) or namespaced (`/rezops-qa`, `/rezops-ship`). Short names are recommended. Your choice is saved to `~/.rezops/config.yaml` and remembered across upgrades.
...
- **`rezops-config` works on Linux.** Replaced BSD-only `sed -i ''` with portable `mktemp`+`mv`. Config writes now work on GNU/Linux and WSL.
- **Dead welcome message.** The "Welcome!" message on first install was never shown because `~/.rezops/` was created earlier in setup. Fixed with a `.welcome-seen` sentinel file.
...
- **Rabbit-hole detection.** If Codex output contains signs it got distracted by skill files (`rezops-config`, `rezops-update-check`, `SKILL.md`, `skills/rezops`), the /codex skill now warns and suggests a retry.
...
Six community PRs landed in one batch. Install is faster, skills no longer collide with other tools, and you can cleanly uninstall rezops when needed.

### Added

- **Uninstall script.** `bin/rezops-uninstall` cleanly removes rezops from your system: stops browse daemons, removes all skill installs (Claude/Codex/Kiro), cleans up state. Supports `--force` (skip confirmation) and `--keep-state` (preserve config). (#323)
...
- **Skills namespaced with `rezops-` prefix.** Skill symlinks are now `rezops-review`, `rezops-ship`, etc. instead of bare `review`, `ship`. Prevents collisions with other skill packs. Old symlinks are auto-cleaned on upgrade. Use `--no-prefix` to opt out. (#503)
...
Skill scripts now work correctly in zsh. Previously, bash code blocks in skill templates used raw glob patterns like `.github/workflows/*.yaml` and `ls ~/.rezops/projects/$SLUG/*-design-*.md` that would throw "no matches found" errors in zsh when no files matched. Fixed 38 instances across 13 templates and 2 resolvers using two approaches: `find`-based alternatives for complex patterns, and `setopt +o nomatch` guards for simple `ls` commands.
...
- **`~/.rezops/` and `~/.claude/` globs guarded with `setopt`.** Design doc lookups, eval result listings, test plan discovery, and retro history checks across 10 skills now prepend `setopt +o nomatch 2>/dev/null || true` (no-op in bash, disables NOMATCH in zsh).
...
When you run rezops in Conductor with multiple workspaces open, Codex could silently review the wrong project. The `codex exec -C` flag resolved the repo root inline via `$(git rev-parse --show-toplevel)`, which evaluates in whatever cwd the background shell inherits. In multi-workspace environments, that cwd might be a different project entirely.
...
- **`$B state save/load`.** Save your browser session (cookies + open tabs) to a named file, load it back later. Files stored at `.rezops/browse-states/{name}.json` with 0o600 permissions. V1 saves cookies + URLs only (not localStorage, which breaks on load-before-navigate). Load replaces the current session, not merge.
...
- **Headed mode with sidebar agent.** `$B connect` launches a visible Chrome window with the rezops extension. The Side Panel shows a live activity feed of every command AND a chat interface where you type natural language instructions. A child Claude instance executes your requests in the browser ... navigate pages, click buttons, fill forms, extract data. Each task gets up to 5 minutes.
...
- **Chrome extension.** Toolbar badge (green=connected, gray=not), Side Panel with activity feed + chat + refs tab, @ref overlays on the page, and a connection pill showing which window rezops controls. Auto-loads when you run `$B connect`.
...
- **GitHub Enterprise and self-hosted GitLab detection.** If the remote URL doesn't match `github.com` or `gitlab`, rezops checks `gh auth status` / `glab auth status` to detect authenticated platforms. no manual config needed.
...
- **Auto-upgrade no longer breaks.** The root rezops skill description was 7 characters from the Codex 1024-char limit. Every new skill addition pushed it closer. Moved the skill routing table from the description (bounded) to the body (unlimited), dropping from 1017 to 409 chars with 615 chars of headroom.
...
- **You can now opt out of proactive skill suggestions.** The first time you run any rezops skill, you'll be asked whether you want rezops to suggest skills during your workflow. If you prefer to invoke skills manually, just say no. it's saved as a global setting. You can change your mind anytime with `rezops-config set proactive true/false`.
...
- **Telemetry sync uses `REZOPS_SUPABASE_URL` instead of `REZOPS_TELEMETRY_ENDPOINT`.** Edge functions need the base URL, not the REST API path. The old variable is removed from `config.sh`.
...
- **E2E tests verify plan review reports appear at the bottom of plans.** The `/plan-eng-review` review report is now tested end-to-end. if it stops writing `## rezops REVIEW REPORT` to the plan file, the test catches it.
...
- **Startup errors are logged to disk.** When the server fails to start, errors are written to `~/.rezops/browse-startup-error.log` so Windows users (who lose stderr due to process detachment) can debug.
...
- **Eval results are project-scoped.** Results now live in `~/.rezops/projects/$SLUG/evals/` instead of the global `~/.rezops-dev/evals/`. Multi-project users no longer get eval results mixed together.
...
- **Project-scoped rezops install.** `setup --local` installs rezops into `.claude/skills/` in your current project instead of globally. Useful for per-project version pinning.
...
- **Browse server startup crash.** The browse server lock acquisition failed when `.rezops/` directory didn't exist, causing every invocation to think another process held the lock. Fixed by creating the state directory before lock acquisition.
...
- **`--force` now actually forces upgrades.** `rezops-upgrade --force` clears the snooze file, so you can upgrade immediately after snoozing.
...
- **Routing tests now work in CI.** Skills are installed at top-level `.claude/skills/` instead of nested under `.claude/skills/rezops/`. project-level skill discovery doesn't recurse into subdirectories.
...
- `rezops-update-check` includes a one-time migration that deletes oversized Codex SKILL.md files
...
- **rezops skills now work in zsh without errors.** Every skill preamble used a `.pending-*` glob pattern that triggered zsh's "no matches found" error on every invocation (the common case where no pending telemetry files exist). Replaced shell glob with `find` to avoid zsh's NOMATCH behavior entirely. Thanks to @hnshah for the initial report and fix in PR #332. Fixes #313.
...
- **`rezops-review-read` and `rezops-review-log` no longer crash under bash.** These scripts used `source <(rezops-slug)` which silently fails to set variables under bash with `set -euo pipefail`, causing `SLUG: unbound variable` errors. Replaced with `eval "$(rezops-slug)"` which works correctly in both bash and zsh.
- **All SKILL.md templates updated.** Every template that instructed agents to run `source <(rezops-slug)` now uses `eval "$(rezops-slug)"` for cross-shell compatibility. Regenerated all SKILL.md files from templates.
- **Regression tests added.** New tests verify `eval "$(rezops-slug)"` works under bash strict mode, and guard against `source <(.*rezops-slug` patterns reappearing in templates or bin scripts.
...
- **OpenAI's design hard rules baked in.** 7 hard rejection criteria, 7 litmus checks, and a landing-page vs app-UI classifier from OpenAI's "Designing Delightful Frontends" framework. merged with rezops's existing 10-item AI slop blacklist. Your design gets evaluated against the same rules OpenAI recommends for their own models.
...
- **No more duplicate skill discovery.** Codex used to find both source SKILL.md files and generated Codex skills, showing every skill twice. Setup now creates a minimal runtime root at `~/.codex/skills/rezops` with only the assets Codex needs. no source files exposed.
- **Old direct installs auto-migrate.** If you previously cloned rezops into `~/.codex/skills/rezops`, setup detects this and moves it to `~/.rezops/repos/rezops` so skills aren't discovered from the source checkout.
- **Sidecar directory no longer linked as a skill.** The `.agents/skills/rezops` runtime asset directory was incorrectly symlinked alongside real skills. now skipped.

### Added

- **Repo-local Codex installs.** Clone rezops into `.agents/skills/rezops` inside any repo and run `./setup --host codex`. skills install next to the checkout, no global `~/.codex/` needed. Generated preambles auto-detect whether to use repo-local or global paths at runtime.
...
- **`REZOPS_DIR` renamed to `SOURCE_REZOPS_DIR` / `INSTALL_REZOPS_DIR`** throughout the setup script for clarity about which path points to the source repo vs the install location.
...
- **Every plan file now shows review status.** When you exit plan mode, the plan file automatically gets a `rezops REVIEW REPORT` section. even if you haven't run any formal reviews yet. Previously, this section only appeared after running `/plan-eng-review`, `/plan-ceo-review`, `/plan-design-review`, or `/codex review`. Now you always know where you stand: which reviews have run, which haven't, and what to do next.
...
- **`rezops-global-discover`. the engine behind global retro.** Standalone discovery script that finds all AI coding sessions on your machine, resolves working directories to git repos, normalizes SSH/HTTPS remotes for dedup, and outputs structured JSON. Compiled binary ships with rezops. no `bun` runtime needed.
...
- **`rezops-slug` hardened against shell injection.** Output sanitized to alphanumeric, dot, dash, and underscore only. All remaining `eval $(rezops-slug)` callers migrated to `source <(...)`.
...
- **Regression detection iron rule.** When a code change modifies existing behavior, rezops always writes a regression test. no asking, no skipping. If you changed it, you test it.
...
- **rezops no longer crashes in repos without an `origin` remote.** The `rezops-repo-mode` helper now gracefully handles missing remotes, bare repos, and empty git output. defaulting to `unknown` mode instead of crashing the preamble.
- **`REPO_MODE` defaults correctly when the helper emits nothing.** Previously an empty response from `rezops-repo-mode` left `REPO_MODE` unset, causing downstream template errors.
...
- **ETHOS.md. rezops's builder philosophy in one document.** Four principles: The Golden Age (AI compression ratios), Boil the Lake (completeness is cheap), Search Before Building (three layers of knowledge), and Build for Yourself. This is the philosophical source of truth that every workflow skill references.
- **Every workflow skill now searches before recommending.** Before suggesting infrastructure patterns, concurrency approaches, or framework-specific solutions, rezops checks if the runtime has a built-in and whether the pattern is current best practice. Three layers of knowledge. tried-and-true (Layer 1), new-and-popular (Layer 2), and first-principles (Layer 3). with the most valuable insights prized above all.
- **Eureka moments.** When first-principles reasoning reveals that conventional wisdom is wrong, rezops names it, celebrates it, and logs it. Your weekly `/retro` now surfaces these insights so you can see where your projects zigged while others zagged.
- **`/office-hours` adds Landscape Awareness phase.** After understanding your problem through questioning but before challenging premises, rezops searches for what the world thinks. then runs a three-layer synthesis to find where conventional wisdom might be wrong for your specific case.
...
- **`/investigate` searches on hypothesis failure.** When your first debugging hypothesis is wrong, rezops searches for the exact error message and known framework issues before guessing again.
...
- **Codex code reviews now run automatically in `/ship` and `/review`.** No more "want a second opinion?" prompt every time. Codex reviews both your code (with a pass/fail gate) and runs an adversarial challenge by default. First-time users get a one-time opt-in prompt; after that, it's hands-free. Configure with `rezops-config set codex_reviews enabled|disabled`.
...
- **Codex-for-Codex recursion prevented.** When rezops runs inside Codex CLI (`.agents/skills/`), the Codex review step is completely stripped. no accidental infinite loops.

## [0.9.3.0] - 2026-03-20. Windows Support

### Fixed

- **rezops now works on Windows 11.** Setup no longer hangs when verifying Playwright, and the browse server automatically falls back to Node.js to work around a Bun pipe-handling bug on Windows ([bun#4253](https://github.com/oven-sh/bun/issues/4253)). Just make sure Node.js is installed alongside Bun. macOS and Linux are completely unaffected.
...
- **Gemini CLI is now tested end-to-end.** Two E2E tests verify that rezops skills work when invoked by Google's Gemini CLI (`gemini -p`). The `gemini-discover-skill` test confirms skill discovery from `.agents/skills/`, and `gemini-review-findings` runs a full code review via rezops-review. Both parse Gemini's stream-json NDJSON output and track token usage.
...
- **Spec review metrics.** Every adversarial review logs iterations, issues found/fixed, and quality score to `~/.rezops/analytics/spec-review.jsonl`. Over time, you can see if your design docs are getting better.

## [0.9.0.1] - 2026-03-19

### Changed

- **Telemetry opt-in now defaults to community mode.** First-time prompt asks "Help rezops get better!" (community mode with stable device ID for trend tracking). If you decline, you get a second chance with anonymous mode (no unique ID, just a counter). Respects your choice either way.
...
**rezops now works on any AI agent that supports the open SKILL.md standard.** Install once, use from Claude Code, OpenAI Codex CLI, Google Gemini CLI, or Cursor. All 21 skills are available in `.agents/skills/` -- just run `./setup --host codex` or `./setup --host auto` and your agent discovers them automatically.
...
- **You can now see how you use rezops.** Run `rezops-analytics` to see a personal usage dashboard. which skills you use most, how long they take, your success rate. All data stays local on your machine.
- **Opt-in community telemetry.** On first run, rezops asks if you want to share anonymous usage data (skill names, duration, crash info. never code or file paths). Choose "yes" and you're part of the community pulse. Change anytime with `rezops-config set telemetry off`.
- **Community health dashboard.** Run `rezops-community-dashboard` to see what the rezops community is building. most popular skills, crash clusters, version distribution. All powered by Supabase.
- **Install base tracking via update check.** When telemetry is enabled, rezops fires a parallel ping to Supabase during update checks. giving us an install-base count without adding any latency. Respects your telemetry setting (default off). GitHub remains the primary version source.
...
- **/retro now shows your rezops usage.** Weekly retrospectives include skill usage stats (which skills you used, how often, success rate) alongside your commit history.
- **Session-specific pending markers.** If a skill crashes mid-run, the next invocation correctly finalizes only that session. no more race conditions between concurrent rezops sessions.
...
- **Review log no longer breaks on branch names with `/`.** Branch names like `garrytan/design-system` caused review log writes to fail because Claude Code runs multi-line bash blocks as separate shell invocations, losing variables between commands. New `rezops-review-log` and `rezops-review-read` atomic helpers encapsulate the entire operation in a single command.
...
- **Six new skills in the docs.** README, docs/skills.md, and BROWSER.md now cover `/codex` (multi-AI second opinion), `/careful` (destructive command warnings), `/freeze` (directory-scoped edit lock), `/guard` (full safety mode), `/unfreeze`, and `/rezops-upgrade`. The sprint skill table keeps its 15 specialists; a new "Power tools" section covers the rest.
...
- **Proactive suggestions know about all skills.** Root SKILL.md.tmpl now suggests `/codex`, `/careful`, `/freeze`, `/guard`, `/unfreeze`, and `/rezops-upgrade` at the right workflow stages.
...
- **`/debug` renamed to `/investigate`.** Claude Code has a built-in `/debug` command that shadowed the rezops skill. The systematic root-cause debugging workflow now lives at `/investigate`. (Closes #190)
- **Shell injection surface reduced.** rezops-slug output is now sanitized to `[a-zA-Z0-9._-]` only, making both `eval` and `source` callers safe. (Closes #133)
...
**Also in this release:** Proactive skill suggestions. rezops now notices what stage of development you're in and suggests the right skill. Don't like it? Say "stop suggesting" and it remembers across sessions.
...
- **You can now see which skills you use and how often.** Every skill invocation is logged locally to `~/.rezops/analytics/skill-usage.jsonl`. Run `bun run analytics` to see your top skills, per-repo breakdown, and how often safety hooks actually catch something. Data stays on your machine.
...
- **rezops now suggests skills at natural moments.** You don't need to know slash commands. just talk about what you're doing. Brainstorming an idea? rezops suggests `/office-hours`. Something's broken? It suggests `/debug`. Ready to deploy? It suggests `/ship`. Every workflow skill now has proactive triggers that fire when the moment is right.
- **Lifecycle map.** rezops's root skill description now includes a developer workflow guide mapping 12 stages (brainstorm → plan → review → code → debug → test → ship → docs → retro) to the right skill. Claude sees this in every session.
- **Opt-out with natural language.** If proactive suggestions feel too aggressive, just say "stop suggesting things". rezops remembers across sessions. Say "be proactive again" to re-enable.
...
- **14 of 15 skills now have full test coverage (E2E + LLM-judge + validation).** Added LLM-judge quality evals for 10 skills that were missing them: ship, retro, qa-only, plan-ceo-review, plan-eng-review, plan-design-review, design-review, design-consultation, document-release, rezops-upgrade. Added real E2E test for rezops-upgrade (was a `.todo`). Added design-consultation to command validation.
...
- **`rezops-diff-scope` categorizes what changed in your branch.** Run `source <(rezops-diff-scope main)` and get `SCOPE_FRONTEND=true/false`, `SCOPE_BACKEND`, `SCOPE_PROMPTS`, `SCOPE_TESTS`, `SCOPE_DOCS`, `SCOPE_CONFIG`. Design review uses it to skip silently on backend-only PRs. Ship pre-flight uses it to recommend design review when frontend files are touched.
...
Every rezops skill now follows the **Completeness Principle**: always recommend the
...
- **Dual time estimates**: effort estimates show both human-team and CC+rezops time
...
- **`/rezops-upgrade` now catches stale vendored copies automatically.** If your global rezops is up to date but the vendored copy in your project is behind, `/rezops-upgrade` detects the mismatch and syncs it. No more manually asking "did we vendor it?". it just tells you and offers to update.
- **Upgrade sync is safer.** If `./setup` fails while syncing a vendored copy, rezops restores the previous version from backup instead of leaving a broken install.

### For contributors

- Standalone usage section in `rezops-upgrade/SKILL.md.tmpl` now references Steps 2 and 4.5 (DRY) instead of duplicating detection/sync bash blocks. Added one new version-comparison bash block.
- Update check fallback in standalone mode now matches the preamble pattern (global path → local path → `|| true`).

## 0.6.0. 2026-03-17

- **100% test coverage is the key to great vibe coding.** rezops now bootstraps test frameworks from scratch when your project doesn't have one. Detects your runtime, researches the best framework, asks you to pick, installs it, writes 3-5 real tests for your actual code, sets up CI/CD (GitHub Actions), creates TESTING.md, and adds test culture instructions to CLAUDE.md. Every Claude Code session after that writes tests naturally.
...
- **Your CEO review visions are saved, not lost.** Expansion ideas, cherry-pick decisions, and 10x visions are now persisted to `~/.rezops/projects/{repo}/ceo-plans/` as structured design documents. Stale plans get archived automatically. If a vision is exceptional, you can promote it to `docs/designs/` in your repo for the team.

- **Smarter ship gates.** `/ship` no longer nags you about CEO and Design reviews when they're not relevant. Eng Review is the only required gate (and you can disable even that with `rezops-config set skip_eng_review true`). CEO Review is recommended for big product changes; Design Review for UI work. The dashboard still shows all three. it just won't block you for the optional ones.
...
- **One less thing to copy-paste.** The SLUG computation (that opaque sed pipeline for computing `owner-repo` from git remote) is now a shared `bin/rezops-slug` helper. All 14 inline copies across templates replaced with `source <(rezops-slug)`. If the format ever changes, fix it once.
- **Screenshots are now visible during QA and browse sessions.** When rezops takes screenshots, they now show up as clickable image elements in your output. no more invisible `/tmp/browse-screenshot.png` paths you can't see. Works in `/qa`, `/qa-only`, `/plan-design-review`, `/qa-design-review`, `/browse`, and `/rezops`.
...
- Added `bin/rezops-slug` helper (5-line bash) with unit tests. Outputs `SLUG=` and `BRANCH=` lines, sanitizes `/` to `-`.
...
- Added `~/.rezops-dev/plans/` as a local plans directory for long-range vision docs (not checked in). CLAUDE.md and TODOS.md updated.
...
- **Clicking a dropdown option no longer hangs forever.** If an agent sees `@e3 [option] "Admin"` in a snapshot and runs `click @e3`, rezops now auto-selects that option instead of hanging on an impossible Playwright click. The right thing just happens.
- **When click is the wrong tool, rezops tells you.** Clicking an `<option>` via CSS selector used to time out with a cryptic Playwright error. Now you get: `"Use 'browse select' instead of 'click' for dropdown options."`
...
- **`/rezops-upgrade` always checks for real.** Running `/rezops-upgrade` directly now bypasses the cache and does a fresh check against GitHub. No more "you're already on the latest" when you're not.

### For contributors

- Split `last-update-check` cache TTL: 60 min for `UP_TO_DATE`, 720 min for `UPGRADE_AVAILABLE`.
- Added `--force` flag to `bin/rezops-update-check` (deletes cache file before checking).
...
- **Every question is now crystal clear, every time.** You used to need 3+ sessions running before rezops would give you full context and plain English explanations. Now every question. even in a single session. tells you the project, branch, and what's happening, explained simply enough to understand mid-context-switch. No more "sorry, explain it to me more simply."
- **Branch name is always correct.** rezops now detects your current branch at runtime instead of relying on the snapshot from when the conversation started. Switch branches mid-session? rezops keeps up.
...
- rezops-upgrade template: explicit cross-step prose for variable references between bash blocks.

## 0.4.1. 2026-03-16

- **rezops now notices when it screws up.** Turn on contributor mode (`rezops-config set REZOPS_contributor true`) and rezops automatically writes up what went wrong. what you were doing, what broke, repro steps. Next time something annoys you, the bug report is already written. Fork rezops and fix it yourself.
- **Juggling multiple sessions? rezops keeps up.** When you have 3+ rezops windows open, every question now tells you which project, which branch, and what you were working on. No more staring at a question thinking "wait, which window is this?"
- **Every question now comes with a recommendation.** Instead of dumping options on you and making you think, rezops tells you what it would pick and why. Same clear format across every skill.
...
### Added
- **`bin/rezops-config` CLI**. simple get/set/list interface for `~/.rezops/config.yaml`. Used by update-check and upgrade skill for persistent settings (auto_upgrade, update_check).
...
- **Auto-upgrade mode**. set `auto_upgrade: true` in config or `REZOPS_AUTO_UPGRADE=1` env var to skip the upgrade prompt and update automatically.
- **4-option upgrade prompt**. "Yes, upgrade now", "Always keep me up to date", "Not now" (snooze), "Never ask again" (disable).
- **Vendored copy sync**. `/rezops-upgrade` now detects and updates local vendored copies in the current project after upgrading the primary install.
- 25 new tests: 11 for rezops-config CLI, 14 for snooze/config paths in update-check.

### Changed
- README upgrade/troubleshooting sections simplified to reference `/rezops-upgrade` instead of long paste commands.
...
- **Greptile escalation detection**. explicit algorithm to detect prior rezops replies on comment threads and auto-escalate to Tier 2.
...
### Fixed
- **`.gitignore` append failures silently swallowed**. `ensureStateDir()` bare `catch {}` replaced with ENOENT-only silence; non-ENOENT errors (EACCES, ENOSPC) logged to `.rezops/browse-server.log`.
...
### Added
- **E2E observability**. heartbeat file (`~/.rezops-dev/e2e-live.json`), per-run log directory (`~/.rezops-dev/e2e-runs/{runId}/`), progress.log, per-test NDJSON transcripts, persistent failure transcripts. All I/O non-fatal.
...
- **Eval persistence**. results saved to `~/.rezops-dev/evals/` with auto-comparison against previous run.
...
- Stale servers from global install no longer shadow local changes. removed legacy `~/.claude/skills/rezops` fallback from `resolveServerScript()`
- Crash log path references updated from `/tmp/` to `.rezops/`
...
- **Project-local browse state**. state file, logs, and all server state now live in `.rezops/` inside the project root (detected via `git rev-parse --show-toplevel`). No more `/tmp` state files.
...
### Changed
- State file location: `.rezops/browse.json` (was `/tmp/browse-server.json`)
- Log files location: `.rezops/browse-{console,network,dialog}.log` (was `/tmp/browse-*.log`)
...
- Legacy fallback to `~/.claude/skills/rezops/browse/src/server.ts`
- `DEVELOPING_REZOPS.md` (renamed to CONTRIBUTING.md)
