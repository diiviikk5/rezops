...
bun run eval:list    # list all eval runs from ~/.rezops-dev/evals/
...
--verbose`). Results are persisted to `~/.rezops-dev/evals/` with auto-comparison
...
├── bin/             # CLI utilities (rezops-repo-mode, rezops-slug, rezops-config, etc.)
├── document-release/ # /document-release skill (post-ship doc updates)
├── cso/             # /cso skill (OWASP Top 10 + STRIDE security audit)
├── design-consultation/ # /design-consultation skill (design system from scratch)
├── design-shotgun/  # /design-shotgun skill (visual design exploration)
├── open-rezops-browser/  # /open-rezops-browser skill (launch rezops Browser)
├── connect-chrome/  # symlink → open-rezops-browser (backwards compat)
...
│   └── add-host/    # /rezops-contrib-add-host skill
...
2. **If missing, AskUserQuestion** — let the user tell you or let rezops search the repo
3. **Persist the answer to CLAUDE.md** so we never have to ask again

This applies to test commands, eval commands, deploy commands, and any other
project-specific behavior. The project owns its config; rezops reads it.
...
tighter V0 prose set `rezops-config set explain_level terse` (binary switch,
...
return 403. SSE endpoints use a 30-minute HttpOnly `REZOPS_sse` cookie minted via
`POST /sse-session` (never valid against `/command`). Tunnel-surface rejections go
to `~/.rezops/security/attempts.jsonl` via `tunnel-denial-log.ts`. Before editing
...
for `server.ts`. See `~/.rezops/projects/garrytan-rezops/ceo-plans/2026-04-19-prompt-injection-guard.md`
...
**Env knobs:**
- `REZOPS_SECURITY_OFF=1` — emergency kill switch. Classifier stays off even if
  warmed. Canary is still injected; just the ML scan is skipped.
- `REZOPS_SECURITY_ENSEMBLE=deberta` — opt-in DeBERTa-v3 ensemble. Adds
...
- Classifier model cache: `~/.rezops/models/testsavant-small/` (112MB, first run only)
  plus `~/.rezops/models/deberta-v3-injection/` (721MB, only when ensemble enabled)
- Attack log: `~/.rezops/security/attempts.jsonl` (salted sha256 + domain only,
  rotates at 10MB, 5 generations)
- Per-device salt: `~/.rezops/security/device-salt` (0600)
- Session state: `~/.rezops/security/session-state.json` (cross-process, atomic)

## Dev symlink awareness

When developing rezops, `.claude/skills/rezops` may be a symlink back to this
working directory (gitignored). This means skill changes are **live immediately**,
great for rapid iteration, risky during big refactors where half-written skills
could break other Claude Code sessions using rezops concurrently.

**Check once per session:** Run `ls -la .claude/skills/rezops` to see if it's a
symlink or a real copy. If it's a symlink to your working directory, be aware that:
- Template changes + `bun run gen:skill-docs` immediately affect all rezops invocations
- Breaking changes to SKILL.md.tmpl files can break concurrent rezops sessions
- During large refactors, remove the symlink (`rm .claude/skills/rezops`) so the
  global install at `~/.claude/skills/rezops/` is used instead

**Prefix setting:** Setup creates real directories (not symlinks) at the top level
with a SKILL.md symlink inside (e.g., `qa/SKILL.md -> rezops/qa/SKILL.md`). This
ensures Claude discovers them as top-level skills, not nested under `rezops/`.
Names are either short (`qa`) or namespaced (`rezops-qa`), controlled by
`skill_prefix` in `~/.rezops/config.yaml`. Pass `--no-prefix` or `--prefix` to
skip the interactive prompt.

**Note:** Vendoring rezops into a project's repo is deprecated. Use global install
+ `./setup --team` instead. See README.md for team mode instructions.

**For plan reviews:** When reviewing plans that modify skill templates or the
gen-skill-docs pipeline, consider whether the changes should be tested in isolation
before going live (especially if the user is actively using rezops in other windows).
...
migration script to `rezops-upgrade/migrations/`. Read CONTRIBUTING.md's "Upgrade
migrations" section for the format and testing requirements. The upgrade skill runs
these automatically after `./setup` during `/rezops-upgrade`.
...
the rezops/Garry voice, one viewport's worth of prose + tables that lands like a
...
When estimating or discussing effort, always show both human-team and CC+rezops time:
...
Contributors can store long-range vision docs and design documents in `~/.rezops-dev/plans/`.
...
Native OpenClaw skills live in `openclaw/skills/rezops-openclaw-*/SKILL.md`. These are
...
```bash
clawhub publish openclaw/skills/rezops-openclaw-office-hours \
  --slug rezops-openclaw-office-hours --name "rezops Office Hours" \
  --version 1.0.0 --changelog "description of changes"
```

Repeat for each skill: `rezops-openclaw-ceo-review`, `rezops-openclaw-investigate`,
`rezops-openclaw-retro`. Bump `--version` on each update.
...
**Verification:** `clawhub search rezops` to confirm they're live.

## Deploying to the active skill

The active skill lives at `~/.claude/skills/rezops/`. After making changes:

1. Push your branch
2. Fetch and reset in the skill directory: `cd ~/.claude/skills/rezops && git fetch origin && git reset --hard origin/main`
3. Rebuild: `cd ~/.claude/skills/rezops && bun run build`

Or copy the binaries directly:
- `cp browse/dist/browse ~/.claude/skills/rezops/browse/dist/browse`
- `cp design/dist/design ~/.claude/skills/rezops/design/dist/design`
