# Contributing to rezops

Thanks for wanting to make rezops better. Whether you're fixing a typo in a skill prompt or building an entirely new workflow, this guide will get you up and running fast.

## Quick start

rezops skills are Markdown files that Claude Code discovers from a `skills/` directory. Normally they live at `~/.claude/skills/rezops/` (your global install). But when you're developing rezops itself, you want Claude Code to use the skills *in your working tree* — so edits take effect instantly without copying or deploying anything.

That's what dev mode does. It symlinks your repo into the local `.claude/skills/` directory so Claude Code reads skills straight from your checkout.

```bash
git clone https://github.com/diiviikk5/rezops.git && cd rezops
...
rezops automatically learns from failures. At the end of every skill session, the agent
reflects on what went wrong (CLI errors, wrong approaches, project quirks) and logs
operational learnings to `~/.rezops/projects/{slug}/learnings.jsonl`. Future sessions
surface these learnings automatically, so rezops gets smarter on your codebase over time.

No setup needed. Learnings are logged automatically. View them with `/learn`.

### The contributor workflow

1. **Use rezops normally** — operational learnings are captured automatically
2. **Check your learnings:** `/learn` or `ls ~/.rezops/projects/*/learnings.jsonl`
3. **Fork and clone rezops** (if you haven't already)
4. **Symlink your fork into the project where you hit the bug:**
   ```bash
   # In your core project (the one where rezops annoyed you)
   ln -sfn /path/to/your/rezops-fork .claude/skills/rezops
   cd .claude/skills/rezops && bun install && bun run build && ./setup
   ```
   Setup creates per-skill directories with SKILL.md symlinks inside (`qa/SKILL.md -> rezops/qa/SKILL.md`)
   and asks your prefix preference. Pass `--no-prefix` to skip the prompt and use short names.
5. **Fix the issue** — your changes are live immediately in this project
6. **Test by actually using rezops** — do the thing that annoyed you, verify it's fixed
7. **Open a PR from your fork**

This is the best way to contribute: fix rezops while doing your real work, in the
project where you actually felt the pain.

### Session awareness

When you have 3+ rezops sessions open simultaneously, every question tells you which project, which branch, and what's happening. No more staring at a question thinking "wait, which window is this?" The format is consistent across all skills.

## Working on rezops inside the rezops repo

When you're editing rezops skills and want to test them by actually using rezops
...
```
rezops/                          <- your working tree
├── .claude/skills/              <- created by dev-setup (gitignored)
│   ├── rezops -> ../../         <- symlink back to repo root
│   ├── review/                  <- real directory (short name, default)
│   │   └── SKILL.md -> rezops/review/SKILL.md
│   ├── ship/                    <- or rezops-review/, rezops-ship/ if --prefix
│   │   └── SKILL.md -> rezops/ship/SKILL.md
...
under `rezops/`. Names depend on your prefix setting (`~/.rezops/config.yaml`).
Short names (`/review`, `/ship`) are the default. Run `./setup --prefix` if you
prefer namespaced names (`/rezops-review`, `/rezops-ship`).
...
When E2E tests run, they produce machine-readable artifacts in `~/.rezops-dev/`:
...
Artifacts are never cleaned up — they accumulate in `~/.rezops-dev/` for post-mortem debugging and trend analysis.
...
rezops's Writing Style section (injected into every tier-≥2 skill's preamble)
...
rezops generates SKILL.md files for 8 hosts from one set of `.tmpl` templates.
...
|--------|---------------------------|
| Output directory | `{skill}/SKILL.md` vs `.agents/skills/rezops-{skill}/SKILL.md` |
| Frontmatter | Full (name, description, hooks, version) vs minimal (name + description) |
| Paths | `~/.claude/skills/rezops` vs `$REZOPS_ROOT` |
...
- **Dev mode shadows your global install.** Project-local skills take priority over `~/.claude/skills/rezops`. `bin/dev-teardown` restores the global one.
...
**This is the recommended way to develop rezops.** Symlink your rezops checkout
...
```bash
# In your core project (not the rezops repo)
ln -sfn /path/to/your/rezops-checkout .claude/skills/rezops
```

### Step 2: Run setup to create per-skill symlinks

The `rezops` symlink alone isn't enough. Claude Code discovers skills through
individual top-level directories (`qa/SKILL.md`, `ship/SKILL.md`, etc.), not through
the `rezops/` directory itself. Run `./setup` to create them:

```bash
cd .claude/skills/rezops && bun install && bun run build && ./setup
```

Setup will ask whether you want short names (`/qa`) or namespaced (`/rezops-qa`).
Your choice is saved to `~/.rezops/config.yaml` and remembered for future runs.
...
Remove the project-local symlink. Claude Code falls back to `~/.claude/skills/rezops/`:

```bash
rm .claude/skills/rezops
```

The per-skill directories (`qa/`, `ship/`, etc.) contain SKILL.md symlinks that point
to `rezops/...`, so they'll resolve to the global install automatically.

### Switching prefix mode

If you installed rezops with one prefix setting and want to switch:

```bash
cd .claude/skills/rezops && ./setup --no-prefix   # switch to /qa, /ship
cd .claude/skills/rezops && ./setup --prefix       # switch to /rezops-qa, /rezops-ship
```
...
- Renamed or moved config keys in `~/.rezops/config.yaml`
- Need to delete orphaned files from a previous version
- Changed the format of `~/.rezops/` state files
...
1. Create `rezops-upgrade/migrations/v{VERSION}.sh` where `{VERSION}` matches
   the VERSION file for the release that needs the fix.
2. Make it executable: `chmod +x rezops-upgrade/migrations/v{VERSION}.sh`
...
"$SCRIPT_DIR/bin/rezops-relink" 2>/dev/null || true
```

### How it runs

During `/rezops-upgrade`, after `./setup` completes (Step 4.75), the upgrade
skill scans `rezops-upgrade/migrations/` and runs every `v*.sh` script whose
...
verifies that all migration scripts in `rezops-upgrade/migrations/` are
