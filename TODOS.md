...
**Context:** Source of the lane data model is `plan-eng-review/SKILL.md.tmpl:240-249` (the "Parallelization Strategy" output with Lane A/B/C dependency tables and conflict flags). Deferred from the v0.18.5.0 rename PR so the rename could land as a tight, low-risk fix. Saved files currently live at `~/.rezops/projects/$SLUG/checkpoints/YYYYMMDD-HHMMSS-<title>.md` with YAML frontmatter (branch, timestamp, etc.). The lane feature would add a `lane:` field to frontmatter and a `--lane` filter to both skills.
...
**Context:** See `~/.rezops/projects/garrytan-rezops/evals/1.6.0.0-feat-opus-4.7-migration-e2e-opus-47-*.json` for the raw transcripts showing 0 parallel calls in first turn across both arms. The overlay is at `model-overlays/opus-4-7.md` with an explicit wrong/right tool_use example. The eval file at `test/skill-e2e-opus-47.test.ts` has the full setup including per-skill SKILL.md install, CLAUDE.md routing block, and overlay inlining.
...
`~/.rezops/developer-profile.json` on preamble and adapts skill-specific
...
**Pros:** rezops feels personal. Every skill adapts to the user's steering
...
**Pros:** Killer delight feature. Social surface for rezops. Concrete, specific
...
via `rezops-config set blind_spot_coach false`.

**Why:** Makes rezops a coach (challenges you) instead of a mirror (reflects
you). The killer differentiation vs. a settings menu.

**Pros:** The feature that makes rezops feel like Garry. Surfaces assumptions
...
**Why:** Biggest personality moment in rezops. The "one-word thing that makes
...
the right trigger shape. Dedup marker per PR in `~/.rezops/.landed-celebrated-*`.
...
rezops feels like it's reading your mind.
...
forced this pivot — see `~/.rezops/projects/garrytan-rezops/ceo-plans/2026-04-19-prompt-injection-guard.md`.
...
**What:** TestSavantAI was trained on direct-injection text, wrong distribution for browser-agent attacks (measured 15% recall). Take BERT-base, fine-tune on BrowseSafe-Bench (3,680 cases) + Qualifire prompt-injection-benchmark (5k) + xxz224 (3.7k) combined, ship in ~/.rezops/models/ as replacement L4 classifier.
...
**What:** Flip `REZOPS_SECURITY_ENSEMBLE=deberta` from opt-in to default. Adds a 3rd ML vote; 2-of-3 agreement rule should reduce FPs while catching attacks that only DeBERTa sees.
...
**What:** Every Allow/Block click is labeled data. Log (suspected_text hash, layer scores, user decision, ts) to ~/.rezops/security/feedback.jsonl. Aggregate via community-pulse when `telemetry: community`. Periodically retrain the classifier on aggregate feedback.
...
#### ~~Attack telemetry via rezops-telemetry-log (P1)~~ — SHIPPED
...
(community-pulse edge function security aggregation) + 756875a7 (bin/rezops-
security-dashboard). Users can now run `rezops-security-dashboard` to see
...
Web UI at rezops.gg/dashboard/security is still open — that's a separate
...
`REZOPS_SECURITY_ENSEMBLE=deberta` — sidebar-agent warmup downloads the 721MB
model to ~/.rezops/models/deberta-v3-injection/ on first run. combineVerdict
...
**Context:** Google shipped Chrome DevTools MCP in Chrome 146+ (June 2025). It provides screenshots, console messages, performance traces, Lighthouse audits, and full page interaction through the user's real browser. rezops should use it for real-session access while keeping Playwright for headless CI/testing workflows.
...
`$B state save/load` ships in v0.12.1.0. V1 saves cookies + URLs only (not localStorage, which breaks on load-before-navigate). Files at `.rezops/browse-states/{name}.json` with 0o600 permissions. Load replaces session (closes all pages first). Name sanitized to `[a-zA-Z0-9_-]`.
...
`$B connect` launches Playwright's bundled Chromium in headed mode with the rezops Chrome extension auto-loaded. `$B handoff` now produces the same result (extension + side panel). Sidebar chat gated behind `--chat` flag.
...
**What:** Publish the rezops browse Chrome extension to Chrome Web Store for easier install.
...
**What:** Append structured JSON entry to `.rezops/ship-log.json` at end of every /ship run (version, date, branch, PR URL, review findings, Greptile stats, todos completed, test results).
...
**Effort:** M
**Priority:** P2
**Depends on:** /setup-rezops-upload
...
### /setup-rezops-upload skill (S3 bucket)
...
### rezops-upload helper

**What:** `browse/bin/rezops-upload` — upload file to S3, return public URL.
...
**Context:** Reads `~/.rezops-dev/evals/*.json`. ~200 lines HTML + chart.js via Bun HTTP server.
...
**What:** `rezops-open-url` helper script — detect platform, use `open` (macOS) or `xdg-open` (Linux).

**Why:** The first-time Completeness Principle intro uses macOS `open` to launch the essay. If rezops ever supports Linux, this silently fails.
...
**Why:** Cross-team design discovery at scale. Local `~/.rezops/projects/$SLUG/` keyword-grep discovery works for same-machine users now, but Supabase sync makes it work across the whole team. Duplicate ideas surface, everyone sees what's been explored.

**Context:** /office-hours writes design docs to `~/.rezops/projects/$SLUG/`. The team store already syncs test plans, retro snapshots, QA reports. Design docs follow the same pattern — just add a sync adapter.
...
**What:** Create a placeholder resolver in gen-skill-docs.ts encoding the rezops voice guide (friendly, user-forward, lead with benefits). Inject into /ship Step 5, /document-release Step 5, and reference from CLAUDE.md.
...
`bin/rezops-diff-scope` shipped — categorizes diff into SCOPE_FRONTEND, SCOPE_BACKEND, SCOPE_PROMPTS, SCOPE_TESTS, SCOPE_DOCS, SCOPE_CONFIG. Used by design-review-lite to skip when no frontend files changed. Dashboard integration for conditional row display is a follow-up.
...
**Effort:** S
**Priority:** P3
**Depends on:** rezops-diff-scope (shipped)
...
**What:** A Codex-native skill (`.agents/skills/rezops-claude/SKILL.md`) that runs `claude -p` to get an independent second opinion from Claude — the reverse of what `/codex` does today from Claude Code.
...
**Context:** The `/codex` skill template (`codex/SKILL.md.tmpl`) shows the pattern — it wraps `codex exec` with JSONL parsing, timeout handling, and structured output. The reverse skill would wrap `claude -p` with similar infrastructure. Would be generated into `.agents/skills/rezops-claude/` by `gen-skill-docs --host codex`.
...
**What:** Track how often Claude chooses the complete option vs shortcut across rezops sessions. Aggregate into a dashboard showing completeness trend over time.
...
5. Debug session persistence (~/.rezops/investigate-sessions/ — save investigation for reuse)
...
**What:** Add ~10 lines of prose to the preamble telling the agent to re-read rezops artifacts (CEO plans, design reviews, eng reviews, checkpoints) after compaction or context degradation.

**Why:** rezops skills produce valuable artifacts stored at `~/.rezops/projects/$SLUG/`. When Claude's auto-compaction fires, it preserves a generic summary but doesn't know these artifacts exist. The plans and reviews that shaped the current work silently vanish from context, even though they're still on disk. This is the thing nobody else in the Claude Code ecosystem is solving, because nobody else has rezops's artifact architecture.

**Context:** Inspired by Anthropic's `claude-progress.txt` pattern for long-running agents. Also informed by claude-mem's "progressive disclosure" approach. See `docs/designs/SESSION_INTELLIGENCE.md` for the broader vision. CEO plan: `~/.rezops/projects/garrytan-rezops/ceo-plans/2026-03-31-session-intelligence-layer.md`.
...
**What:** Append one-line JSONL entry to `~/.rezops/projects/$SLUG/timeline.jsonl` after every skill run (timestamp, skill, branch, outcome). `/retro` renders the timeline.
...
**What:** When a new rezops session starts on a branch with recent checkpoints or plans, the preamble prints a one-line summary: "Last session: implemented JWT auth, 3/5 tasks done." Agent knows where you left off before reading any files.
...
**What:** Manual skill to snapshot current working state: what's being done and why, files being edited, decisions made (and rationale), what's done vs. remaining, critical types/signatures. Saved to `~/.rezops/projects/$SLUG/checkpoints/<timestamp>.md`.
...
**What:** Write `docs/designs/SESSION_INTELLIGENCE.md` describing the architectural vision: rezops as the persistent brain that survives Claude's ephemeral context. Every skill writes to `~/.rezops/projects/$SLUG/`, preamble re-reads, `/retro` rolls up.
...
**What:** Skill that runs type-check, lint, test suite, and dead code scan, then reports a composite 0-10 health score with breakdown by category. Tracks over time in `~/.rezops/health/<project-slug>/` for trend detection. Optionally integrates CodeScene MCP for deeper complexity/cohesion/coupling analysis.
...
**What:** Expose rezops's browse binary and key workflows as an MCP server that Factory Droid connects to natively. Factory users would run /mcp, add the rezops server, and get browse, QA, and review capabilities as Factory tools.

**Why:** Factory already supports 40+ MCP servers in its registry. Getting rezops's browse binary listed there is a distribution play. Nobody else has a real compiled browser binary as an MCP tool. This is the thing that makes rezops uniquely valuable on Factory Droid.
...
**What:** Factory has "custom droids" (subagents with tool restrictions, model selection, autonomy levels). Could ship `rezops-qa.md` droid configs alongside skills that restrict tools to read-only + execute for safety.

**Why:** Deeper Factory integration. Droid configs give Factory users tighter control over what rezops skills can do.
...
**What:** Maintain a Chromium fork where anti-bot stealth, rezops Browser branding, and native sidebar support live in the source code, not as runtime monkey-patches.

**Why:** The CDP patches are brittle. They break on every Playwright upgrade and target compiled JS with fragile string matching. A proper fork means: (1) stealth is permanent, not patched, (2) branding is native (no plist hacking at launch), (3) native sidebar replaces the extension (Phase 4 of V0 roadmap), (4) custom protocols (rezops://) for internal pages. Companies like Brave, Arc, and Vivaldi maintain Chromium forks with small teams. With CC, the rebase-on-upstream maintenance could be largely automated.

**Context:** Trigger criteria from V0 design doc: fork when extension side panel becomes the bottleneck, when anti-bot patches need to live deeper than CDP, or when native UI integration (sidebar, status bar) can't be done via extension. The Chromium build takes ~4 hours on a 32-core machine and produces ~50GB of build artifacts. CI would need dedicated build infra. See `docs/designs/REZOPS_BROWSER_V0.md` Phase 5 for full analysis.
...
- Config CLI (`bin/rezops-config`), auto-upgrade via `~/.rezops/config.yaml`, 12h cache TTL, exponential snooze backoff (24h→48h→1wk), "never ask again" option, vendored copy sync on upgrade
**Completed:** v0.3.8
