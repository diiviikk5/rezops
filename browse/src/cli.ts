/**
 * rezops CLI — thin wrapper that talks to the persistent server
 *
 * Flow:
 *   1. Read .rezops/browse.json for port + token
...
function cleanupLegacyState(): void {
  // No legacy state on Windows — /tmp and `ps` don't exist, and rezops
...
async function ensureServer(): Promise<ServerState> {
  ...
  if (process.env.BROWSE_NO_AUTOSTART === '1') {
    ...
    console.error('[browse] The headed browser may have been closed. Run /open-rezops-browser to restart.');
    process.exit(1);
  }
  ...
  if (state && state.mode === 'headed' && isProcessAlive(state.pid)) {
    ...
    console.error(`[browse] Run '/open-rezops-browser' to restart.`);
    process.exit(1);
  }
...
/** Check if ngrok is installed and authenticated (native config or rezops env). */
function isNgrokAvailable(): boolean {
  // Check rezops's own ngrok env
  const ngrokEnvPath = path.join(process.env.HOME || '/tmp', '.rezops', 'ngrok.env');
...
async function handlePairAgent(state: ServerState, args: string[]): Promise<void> {
  ...
  if (localHost) {
    try {
      ...
      let globalRoot = `.${localHost}/skills/rezops`;
      try {
...
async function main() {
  ...
  if (args.length === 0 || args[0] === '--help' || args[0] === '-h') {
    console.log(`rezops browse — Fast headless browser for AI coding agents
  ...
  if (command === 'connect') {
    ...
    try {
      ...
      try {
        ...
        const agentQueue = path.join(process.env.HOME || '/tmp', '.rezops', 'sidebar-agent-queue.jsonl');
        try {
 ...
 command: 'disconnect', args: [] }),
  ...
  if (command === 'pair-agent') {
    ...
    if (state.mode !== 'headed' && !hasFlag(commandArgs, '--headless')) {
      console.log('[browse] Opening rezops Browser so you can see what the remote agent does...');
