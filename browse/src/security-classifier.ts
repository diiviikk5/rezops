 ...
 * layout at ~/.rezops/models/testsavant-small/ on first use.
 *
 ...
 */
const MODELS_DIR = path.join(os.homedir(), '.rezops', 'models');
...
// REZOPS_SECURITY_ENSEMBLE=deberta. Not forced on every install because
...
function isDebertaEnabled(): boolean {
  const setting = (process.env.REZOPS_SECURITY_ENSEMBLE ?? '').toLowerCase();
...
export function loadTestsavant(onProgress?: (msg: string) => void): Promise<void> {
  if (process.env.REZOPS_SECURITY_OFF === '1') {
    testsavantState = 'failed';
    testsavantLoadError = 'REZOPS_SECURITY_OFF=1 — ML classifier kill switch engaged';
...
export function loadDeberta(onProgress?: (msg: string) => void): Promise<void> {
  if (process.env.REZOPS_SECURITY_OFF === '1') return Promise.resolve();
