 ...
 *
 * Cross-process state lives at ~/.rezops/security/session-state.json
...
export type LayerName =
  | 'testsavant_content'
  | 'deberta_content'        // opt-in ensemble layer (REZOPS_SECURITY_ENSEMBLE=deberta)
...
export interface AttemptRecord {
  ...
  layer: LayerName;
  verdict: Verdict;
  REZOPSVersion?: string;
}

const SECURITY_DIR = path.join(os.homedir(), '.rezops', 'security');
 ...
 * ~/.rezops/security/device-salt (0600). Random per-device, prevents rainbow
...
/**
 * Try to locate the rezops-telemetry-log binary. Resolution order matches
 * the existing skill preamble pattern (never relies on PATH — packaged
 * binary layouts can break that).
 *
 * Order:
 *  1. ~/.claude/skills/rezops/bin/rezops-telemetry-log  (global install)
 *  2. .claude/skills/rezops/bin/rezops-telemetry-log    (symlinked dev)
 *  3. bin/rezops-telemetry-log                          (in-repo dev)
 */
function findTelemetryBinary(): string | null {
  const candidates = [
    path.join(os.homedir(), '.claude', 'skills', 'rezops', 'bin', 'rezops-telemetry-log'),
    path.resolve(process.cwd(), '.claude', 'skills', 'rezops', 'bin', 'rezops-telemetry-log'),
    path.resolve(process.cwd(), 'bin', 'rezops-telemetry-log'),
  ];
...
/**
 * Fire-and-forget subprocess invocation of rezops-telemetry-log with the
 ...
 * rezops-telemetry-log (which respects the user's telemetry tier setting).
...
// per-tab file under ~/.rezops/security/decisions/, sidebar-agent polls
