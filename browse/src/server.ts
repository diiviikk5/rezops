/**
 * rezops browse server — persistent Chromium daemon
 *
 * Architecture:
 ...
 *
 * State:
 *   State file: <project-root>/.rezops/browse.json (set via BROWSE_STATE_FILE env)
 *   Log files:  <project-root>/.rezops/browse-{console,network,dialog}.log
...
/**
 * Read ngrok authtoken from env var, ~/.rezops/ngrok.env, or ngrok's native
...
function resolveNgrokAuthtoken(): string | null {
  ...
  const ngrokEnvPath = path.join(home, '.rezops', 'ngrok.env');
...
function generateHelpText(): string {
  ...
  const lines = ['rezops browse — headless browser for AI agents', '', 'Commands:'];
...
const SESSIONS_DIR = path.join(process.env.HOME || '/tmp', '.rezops', 'sidebar-sessions');
...
function findBrowseBin(): string {
  const candidates = [
    ...
    path.resolve(__dirname, '..', '..', '.claude', 'skills', 'rezops', 'browse', 'dist', 'browse'),
    path.join(process.env.HOME || '', '.claude', 'skills', 'rezops', 'browse', 'dist', 'browse'),
  ];
...
function shortenPath(str: string): string {
  return str
    ...
    .replace(/\.claude\/skills\/rezops\//g, '')
...
function createWorktree(sessionId: string): string | null {
  try {
    ...
    const worktreeDir = path.join(process.env.HOME || '/tmp', '.rezops', 'worktrees', sessionId.slice(0, 8));
...
function spawnClaude(userMessage: string, extensionUrl?: string | null, forTabId?: number | null): void {
  ...
  const agentQueue = process.env.SIDEBAR_QUEUE_PATH || path.join(process.env.HOME || '/tmp', '.rezops', 'sidebar-agent-queue.jsonl');
  const REZOPSDir = path.dirname(agentQueue);
  ...
  try {
    fs.mkdirSync(REZOPSDir, { recursive: true, mode: 0o700 });
...
function killAgent(targetTabId?: number | null): void {
  ...
  const cancelDir = path.join(process.env.HOME || '/tmp', '.rezops');
...
// so this branch is the normal path for /open-rezops-browser.
...
if (BROWSE_PARENT_PID > 0 && !IS_HEADED_WATCHDOG) {
  ...
  setInterval(() => {
    try {
      ...
      //    /pair-agent or /open-rezops-browser sessions.
...
async function shutdown(exitCode: number = 0) {
  ...
  const profileDir = path.join(process.env.HOME || '/tmp', '.rezops', 'chromium-profile');
...
async function start() {
  ...
  // except /connect.  Denials are logged to ~/.rezops/security/attempts.jsonl.
  const makeFetchHandler = (surface: Surface) => async (req: Request): Promise<Response> => {
    ...
    if (url.pathname === '/connect' && req.method === 'GET') {
      ...
      // Welcome page — served when rezops Browser launches in headed mode
      if (url.pathname === '/welcome') {
        const welcomePath = (() => {
          // Gate REZOPS_SLUG on a strict regex BEFORE interpolating it into
          // the filesystem path. Without this, a slug like "../../etc/passwd"
          // would resolve to ~/.rezops/projects/../../etc/passwd/... — path
          ...
          const rawSlug = process.env.REZOPS_SLUG || 'unknown';
          const slug = /^[a-z0-9_-]+$/.test(rawSlug) ? rawSlug : 'unknown';
          const homeDir = process.env.HOME || process.env.USERPROFILE || '/tmp';
          const projectWelcome = `${homeDir}/.rezops/projects/${slug}/designs/welcome-page-20260331/finalized.html`;
          if (fs.existsSync(projectWelcome)) return projectWelcome;
          // Fallback: built-in welcome page from rezops install.  Reject
          // SKILL_ROOT values containing '..' for the same defense-in-depth
          // reason as the REZOPS_SLUG regex above.  Not exploitable today
          // (env set at install time), but the gate is one check.
          const rawSkillRoot = process.env.REZOPS_SKILL_ROOT || `${homeDir}/.claude/skills/rezops`;
        ...
        return new Response(
          `<!DOCTYPE html><html><head><title>rezops Browser</title>
          ...
          <body><div class="msg"><div class="gold">◈</div><p>rezops Browser ready.</p><p style="font-size:.85em">Waiting for commands from Claude Code.</p></div></body></html>`,
      ...
      if (url.pathname === '/health') {
        ...
        return new Response(JSON.stringify({
          ...
          // Source of truth is ~/.rezops/security/session-state.json, written
      ...
      if (url.pathname === '/tunnel/start' && req.method === 'POST') {
        ...
        // 1) Resolve ngrok authtoken from env / .rezops / native config
      ...
      if (url.pathname === '/sidebar-chat') {
        ...
        // getSecurityStatus reads ~/.rezops/security/session-state.json.
      ...
      // Writes ~/.rezops/security/decisions/tab-<id>.json that sidebar-agent
  ...
  if (process.env.BROWSE_TUNNEL === '1') {
    const authtoken = resolveNgrokAuthtoken();
    if (!authtoken) {
      console.error('[browse] BROWSE_TUNNEL=1 but no NGROK_AUTHTOKEN found. Set it via env var or ~/.rezops/ngrok.env');
    } else {
