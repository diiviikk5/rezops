...
describe('CLI server script resolution', () => {
  test('prefers adjacent browse/src/server.ts for compiled project installs', () => {
    const root = fs.mkdtempSync('/tmp/rezops-cli-');
    const execPath = path.join(root, '.claude/skills/rezops/browse/dist/browse');
    const serverPath = path.join(root, '.claude/skills/rezops/browse/src/server.ts');
