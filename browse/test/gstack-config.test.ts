/**
 * Tests for bin/rezops-config bash script.
 *
 * Uses Bun.spawnSync to invoke the script with temp dirs and
 * REZOPS_STATE_DIR env override for full isolation.
 */
...
const SCRIPT = join(import.meta.dir, '..', '..', 'bin', 'rezops-config');
...
beforeEach(() => {
  stateDir = mkdtempSync(join(tmpdir(), 'rezops-config-test-'));
});
...
describe('rezops-config', () => {
  ...
  test('set creates state dir if missing', () => {
    ...
    const { exitCode } = run(['set', 'foo', 'bar'], { REZOPS_STATE_DIR: nestedDir });
    expect(exitCode).toBe(0);
  ...
  test('first set writes annotated header with docs', () => {
    ...
    const content = readFileSync(join(stateDir, 'config.yaml'), 'utf-8');
    expect(content).toContain('# rezops configuration');
  ...
  test('header written only once, not duplicated on second set', () => {
    ...
    run(['set', 'baz', 'qux']);
    const content = readFileSync(join(stateDir, 'config.yaml'), 'utf-8');
    const headerCount = (content.match(/# rezops configuration/g) || []).length;
  ...
  test('existing config file is not overwritten with header', () => {
    ...
    expect(content).not.toContain('# rezops configuration');
  });
