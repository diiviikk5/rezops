...
describe('security pipeline E2E (mock claude)', () => {
  ...
  test('attempts.jsonl logged with salted payload_hash and verdict=block', async () => {
    ...
    // and contains the event. The file lives at ~/.rezops/security/attempts.jsonl.
    const logPath = path.join(os.homedir(), '.rezops', 'security', 'attempts.jsonl');
