 ...
 * ~/.rezops/models/testsavant-small/ and complete in ~5s.
 *
...
const CLASSIFIER_CACHE = path.join(os.homedir(), '.rezops', 'models', 'testsavant-small');
...
async function startStack(scenario: string, attemptsDir: string): Promise<void> {
  ...
  //   from ~/.rezops/sidebar-sessions/ (caused ghost security_events to
  //   leak in from the live /open-rezops-browser session)
  // - security.ts's attempts.jsonl writes land in a test-owned dir
  // - session-state.json, chromium-profile, etc. stay isolated
  fs.mkdirSync(path.join(attemptsDir, '.rezops'), { recursive: true });
  ...
  const testModelsDir = path.join(attemptsDir, '.rezops', 'models');
  const realModelsDir = path.join(os.homedir(), '.rezops', 'models');
  try {
  ...
  attemptsPath = path.join(attemptsDir, '.rezops', 'security', 'attempts.jsonl');
