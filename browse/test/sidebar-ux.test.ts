...
describe('cleanup heuristics (write-commands.ts)', () => {
  ...
  test('sticky cleanup skips rezops control indicator', () => {
    expect(wcSrc).toContain("rezops-ctrl");
  });
...
describe('welcome page', () => {
  ...
  test('welcome page has rezops Browser branding', () => {
    expect(welcomeSrc).toContain('rezops Browser');
  });

  test('welcome page has extension-ready listener to hide prompt', () => {
    expect(welcomeSrc).toContain('rezops-extension-ready');
...
describe('server /welcome endpoint', () => {
  ...
  test('/welcome serves fallback HTML if no welcome file found', () => {
    ...
    expect(welcomeSection).toContain('rezops Browser ready');
...
describe('sidebar arrow hint hide flow (4-step signal chain)', () => {
  ...
  //   3. content.js receives 'sidebarOpened' → dispatches 'rezops-extension-ready'
  //   4. welcome.html listens for 'rezops-extension-ready' → hides arrow
  //
  const contentSrc = fs.readFileSync(path.join(ROOT, '..', 'extension', 'content.js'), 'utf-8');
  const bgSrc = fs.readFileSync(path.join(ROOT, '..', 'extension', 'background.js'), 'utf-8');
  ...
  // Step 3: content.js fires rezops-extension-ready ONLY on sidebarOpened
  test('step 3: content.js dispatches extension-ready on sidebarOpened message', () => {
    ...
    expect(contentSrc).toContain("new CustomEvent('rezops-extension-ready')");
  });
  ...
  test('step 3: content.js does NOT auto-fire extension-ready on load', () => {
    ...
    expect(beforeListener).not.toContain("dispatchEvent(new CustomEvent('rezops-extension-ready'))");
  });

  // Step 4: welcome page hides arrow on rezops-extension-ready
  test('step 4: welcome page hides arrow on rezops-extension-ready event', () => {
    expect(welcomeSrc).toContain("'rezops-extension-ready'");
    expect(welcomeSrc).toContain("classList.add('hidden')");
  });

  test('step 4: welcome page does NOT auto-hide via status pill polling', () => {
    // The old fallback (checkPill/rezops-status-pill) would hide the arrow
    // as soon as the content script injected the pill, even without sidebar open.
    expect(welcomeSrc).not.toContain('checkPill');
    expect(welcomeSrc).not.toContain('rezops-status-pill');
  });
});
...
describe('BROWSE_NO_AUTOSTART (sidebar headless prevention)', () => {
  ...
  test('cli.ts shows actionable error message when BROWSE_NO_AUTOSTART blocks', () => {
    expect(cliSrc).toContain('/open-rezops-browser');
