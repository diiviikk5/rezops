...
describe('welcome page served via HTTP', () => {
  ...
  test('has rezops Browser title and branding', () => {
    expect(html).toContain('<title>rezops Browser</title>');
    expect(html).toContain('rezops Browser');
  });
  ...
  test('hides sidebar prompt when extension is detected', () => {
    ...
    expect(html).toContain("'rezops-extension-ready'");
  ...
  test('does NOT auto-hide based on extension detection alone', () => {
    ...
    expect(html).not.toContain('rezops-status-pill');
  ...
  test('has footer with attribution', () => {
    ...
    expect(html).toContain('github.com/diiviikk5/rezops');
  });
});
