...
description: |
  ...
  "codex challenge", "ask codex", "second opinion", or "consult codex". (rezops)
voice-triggers:
  - "code x"
  - "code ex"
  - "get another opinion"
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"codex","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.rezops/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/rezops/bin/rezops-timeline-log '{"skill":"codex","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
```bash
~/.claude/skills/rezops/bin/rezops-question-log '{"skill":"codex","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
source ~/.claude/skills/rezops/bin/rezops-codex-probe 2>/dev/null && _REZOPS_codex_log_event "codex_cli_missing" 2>/dev/null || true
```
...
CLI version isn't in the known-bad list. Sourcing `rezops-codex-probe` loads the
...
if ! _REZOPS_codex_auth_probe >/dev/null; then
  _REZOPS_codex_log_event "codex_auth_failed"
  echo "AUTH_FAILED"
fi
_REZOPS_codex_version_check   # warns if known-bad, non-blocking
```
...
**Update the known-bad list** in `bin/rezops-codex-probe` when a new Codex CLI version
...
```bash
~/.claude/skills/rezops/bin/rezops-review-log '{"skill":"codex-review","timestamp":"TIMESTAMP","status":"STATUS","gate":"GATE","findings":N,"findings_fixed":N,"commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
_REZOPS_codex_timeout_wrapper 600 codex exec "<prompt>" -C "$_REPO_ROOT" -s read-only -c 'model_reasoning_effort="high"' --enable web_search_cached --json < /dev/null 2>"$TMPERR" | PYTHONUNBUFFERED=1 python3 -u -c "
import sys, json
...
if [ "$_CODEX_EXIT" = "124" ]; then
  ...
  _REZOPS_codex_log_hang "challenge" "$(wc -c < "$TMPERR" 2>/dev/null || echo 0)"
...
_REZOPS_codex_timeout_wrapper 600 codex exec "<prompt>" -C "$_REPO_ROOT" -s read-only -c 'model_reasoning_effort="medium"' --enable web_search_cached --json < /dev/null 2>"$TMPERR" | PYTHONUNBUFFERED=1 python3 -u -c "
import sys, json
for line in sys.stdin:
    line = line.strip()
    if not line: continue
...
if [ "$_CODEX_EXIT" = "124" ]; then
  _REZOPS_codex_log_event "codex_timeout" "600"
  _REZOPS_codex_log_hang "consult" "$(wc -c < "$TMPERR" 2>/dev/null || echo 0)"
  echo "Codex stalled past 10 minutes. Common causes: model API stall, long prompt, network issue. Try re-running. If persistent, split the prompt or check ~/.codex/logs/."
fi
```
...
_REPO_ROOT=$(git rev-parse --show-toplevel) || { echo "ERROR: not in a git repo" >&2; exit 1; }
# Fix 1: wrap with timeout (gtimeout/timeout fallback chain via probe helper)
_REZOPS_codex_timeout_wrapper 600 codex exec resume <session-id> "<prompt>" -C "$_REPO_ROOT" -s read-only -c 'model_reasoning_effort="medium"' --enable web_search_cached --json < /dev/null 2>"$TMPERR" | PYTHONUNBUFFERED=1 python3 -u -c "
...
if [ "$_CODEX_EXIT" = "124" ]; then
  _REZOPS_codex_log_event "codex_timeout" "600"
  _REZOPS_codex_log_hang "consult-resume" "$(wc -c < "$TMPERR" 2>/dev/null || echo 0)"
  echo "Codex stalled past 10 minutes. Common causes: model API stall, long prompt, network issue. Try re-running. If persistent, split the prompt or check ~/.codex/logs/."
fi
...
- **Detect skill-file rabbit holes.** After receiving Codex output, scan for signs
  that Codex got distracted by skill files: `rezops-config`, `rezops-update-check`,
  `SKILL.md`, or `skills/rezops`. If any of these appear in the output, append a
  warning: "Codex appears to have read rezops skill files instead of reviewing your
