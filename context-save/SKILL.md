...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"context-save","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.rezops/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/rezops/bin/rezops-timeline-log '{"skill":"context-save","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
```bash
~/.claude/skills/rezops/bin/rezops-question-log '{"skill":"context-save","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
```bash
eval "$(~/.claude/skills/rezops/bin/rezops-slug 2>/dev/null)" && mkdir -p ~/.rezops/projects/$SLUG
CHECKPOINT_DIR="${REZOPS_HOME:-$HOME/.rezops}/projects/$SLUG/checkpoints"
...
```bash
eval "$(~/.claude/skills/rezops/bin/rezops-slug 2>/dev/null)" && mkdir -p ~/.rezops/projects/$SLUG
CHECKPOINT_DIR="$HOME/.rezops/projects/$SLUG/checkpoints"
...
`WIP:` commits with structured `[rezops-context]` blocks in their bodies. These
...
if [ -n "$_BASE" ]; then
  ...
  if [ -n "$WIP_COMMITS" ]; then
    ...
    # Extract [rezops-context] blocks from each WIP commit body
    for SHA in $WIP_COMMITS; do
      echo "--- commit $SHA ---"
      git log -1 "$SHA" --format="%s%n%n%b" 2>/dev/null | \
        awk '/\[rezops-context\]/,/\[\/rezops-context\]/ { print }'
    done
  else
    echo "NO_WIP_COMMITS"
  fi
...
If `WIP_COMMITS_FOUND`: Read the extracted `[rezops-context]` blocks. Each block
...
- **This is a rezops skill, not a Claude Code built-in.** When the user types
