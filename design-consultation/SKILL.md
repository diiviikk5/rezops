...
description: |
  ...
  design system or DESIGN.md. (rezops)
allowed-tools:
  - Bash
  - Read
  - Write
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"design-consultation","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.rezops/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/rezops/bin/rezops-timeline-log '{"skill":"design-consultation","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
```bash
~/.claude/skills/rezops/bin/rezops-question-log '{"skill":"design-consultation","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
eval "$(~/.claude/skills/rezops/bin/rezops-slug 2>/dev/null)"
ls ~/.rezops/projects/$SLUG/*office-hours* 2>/dev/null | head -5
...
**Find the rezops designer (optional — enables AI mockup generation):**
...
```bash
_TASTE_PROFILE=~/.rezops/projects/$SLUG/taste-profile.json
...
the legacy approved.json aggregate — `~/.claude/skills/rezops/bin/rezops-taste-update`
...
This phase generates visual previews of the proposed design system. Two paths depending on whether the rezops designer is available.
...
```bash
eval "$(~/.claude/skills/rezops/bin/rezops-slug 2>/dev/null)"
_DESIGN_DIR="$HOME/.rezops/projects/$SLUG/designs/design-system-$(date +%Y%m%d)"
mkdir -p "$_DESIGN_DIR"
...
```bash
~/.claude/skills/rezops/bin/rezops-learnings-log '{"skill":"design-consultation","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
