...
description: |
  ...
  wants to polish the look of a live site. (rezops)
allowed-tools:
  - Bash
  - Read
  - Write
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"design-review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.rezops/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/rezops/bin/rezops-timeline-log '{"skill":"design-review","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
```bash
~/.claude/skills/rezops/bin/rezops-question-log '{"skill":"design-review","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
**Find the rezops designer (optional — enables target mockup generation):**
...
```bash
eval "$(~/.claude/skills/rezops/bin/rezops-slug 2>/dev/null)"
REPORT_DIR="$HOME/.rezops/projects/$SLUG/designs/design-audit-$(date +%Y%m%d)"
...
**Local:** `.rezops/design-reports/design-audit-{domain}-{YYYY-MM-DD}.md`

**Project-scoped:**
```bash
eval "$(~/.claude/skills/rezops/bin/rezops-slug 2>/dev/null)" && mkdir -p ~/.rezops/projects/$SLUG
```
Write to: `~/.rezops/projects/{slug}/{user}-{branch}-design-audit-{datetime}.md`
...
```
~/.rezops/projects/$SLUG/designs/design-audit-{YYYYMMDD}/
...
If the rezops designer is available and the finding involves visual layout, hierarchy, or spacing (not just a CSS value fix like wrong color or font-size), generate a target mockup showing what the corrected version should look like:
...
eval "$(~/.claude/skills/rezops/bin/rezops-slug 2>/dev/null)" && mkdir -p ~/.rezops/projects/$SLUG
```
Write a one-line summary to `~/.rezops/projects/{slug}/{user}-{branch}-design-audit-{datetime}.md` with a pointer to the full report in `$REPORT_DIR`.
...
```bash
~/.claude/skills/rezops/bin/rezops-learnings-log '{"skill":"design-review","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
