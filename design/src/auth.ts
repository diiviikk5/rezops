 ...
 *
 * Resolution order:
 * 1. ~/.rezops/openai.json → { "api_key": "sk-..." }
...
const CONFIG_PATH = path.join(process.env.HOME || "~", ".rezops", "openai.json");

export function resolveApiKey(): string | null {
  // 1. Check ~/.rezops/openai.json
  try {
...
/**
 * Save an API key to ~/.rezops/openai.json with 0600 permissions.
 */
...
export function requireApiKey(): string {
  ...
  if (!key) {
    ...
    console.error("  or save to ~/.rezops/openai.json: { \"api_key\": \"sk-...\" }");
