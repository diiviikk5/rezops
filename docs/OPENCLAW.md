# rezops x OpenClaw Integration

rezops integrates with OpenClaw as a methodology source, not a ported codebase.
OpenClaw's ACP runtime spawns Claude Code sessions natively. rezops provides the
...
```
  ...
  calendar, memory, EA                     methodology + planning
       ...
       │   investigate, retro                   │
       │                                        ├── Generates rezops-lite
       ├── sessions_spawn(runtime: "acp")       │   (planning discipline)
       │       │                                │
       │       └── Claude Code                  ├── Generates rezops-full
       │           └── rezops installed at      │   (complete pipeline)
       │               ~/.claude/skills/rezops  │
...
OpenClaw decides at spawn time which tier of rezops support to use:

| Tier | When | Prompt prefix |
|------|------|---------------|
| **Simple** | One-file edits, typos, config changes | No rezops context injected |
| **Medium** | Multi-file features, refactors | rezops-lite CLAUDE.md appended |
| **Heavy** | Specific rezops skill needed | "Load rezops. Run /X" |
| **Full** | Complete features, objectives, projects | rezops-full pipeline appended |
| **Plan** | "Help me plan a Claude Code project" | rezops-plan pipeline appended |
...
The complete ready-to-paste section lives in `openclaw/agents-rezops-section.md`.
Copy it into your OpenClaw AGENTS.md.

Key behavioral rules (these go ABOVE the dispatch tiers):

1. **Always spawn, never redirect.** When the user asks to use ANY rezops skill,
...
rezops-lite/full as a new section. Do not replace the repo's existing instructions.

## What rezops generates for OpenClaw

All artifacts live in the `openclaw/` directory and are generated by
`bun run gen:skill-docs --host openclaw`:

### rezops-lite (Medium tier)
`openclaw/rezops-lite-CLAUDE.md` — ~15 lines of planning discipline:
...
### rezops-full (Full tier)
`openclaw/rezops-full-CLAUDE.md` — chains existing rezops skills:
...
### rezops-plan (Plan tier)
`openclaw/rezops-plan-CLAUDE.md` — full review gauntlet, no implementation:
...
- `rezops-openclaw-office-hours` — Product interrogation (6 forcing questions)
- `rezops-openclaw-ceo-review` — Strategic challenge (10-section review, 4 modes)
- `rezops-openclaw-investigate` — Operational debugging (4-phase methodology)
- `rezops-openclaw-retro` — Operational retrospective (weekly review)

Source lives in `openclaw/skills/` in the rezops repo. These are hand-crafted
adaptations of the rezops methodology for OpenClaw's conversational context.
No rezops infrastructure (no browse, no telemetry, no preamble).

## Spawned session detection

When Claude Code runs inside a session spawned by OpenClaw, the `OPENCLAW_SESSION`
environment variable should be set. rezops detects this and adjusts:
...
For OpenClaw users: tell your OpenClaw agent "install rezops for openclaw."

The agent should:
1. Install rezops-lite CLAUDE.md into its coding session templates
2. Install the 4 native methodology skills
3. Add dispatch routing to AGENTS.md
4. Verify with a test spawn

For rezops developers: `./setup --host openclaw` outputs this documentation.
...
- No SOUL.md from rezops (OpenClaw has its own)
