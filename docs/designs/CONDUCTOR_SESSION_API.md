...
When Claude controls your real browser via CDP (rezops `$B connect`), you look at two
windows: **Conductor** (to see Claude's thinking) and **Chrome** (to see Claude's actions).

rezops's Chrome extension Side Panel shows browse activity — every command, result,
...
A "Session" tab in the rezops Chrome extension Side Panel that shows:
...
## What rezops builds (extension side)
