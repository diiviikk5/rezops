# Design: rezops Visual Design Generation (`design` binary)
...
**Screenshot consent (first-time only for $D evolve):** "This will send a screenshot of your live site to OpenAI for design evolution. [Proceed] [Don't ask again]" Stored in ~/.rezops/config.yaml as design_screenshot_consent.
...
- Exploration variants go to `/tmp/rezops-mockups-{session}/` (ephemeral, not committed)
...
- Fallback: if not in a git repo, save to `/tmp/rezops-mockup-{timestamp}.png`
...
```
Note: Codex runtime setup (`setup` script) must also export `REZOPS_DESIGN` env var, similar to how `REZOPS_BROWSE` is set.
...
[ -z "$D" ] && D=~/.claude/skills/rezops/design/dist/design
if [ -x "$D" ]; then
  echo "DESIGN_READY: $D"
else
...
| `scripts/resolvers/preamble.ts` | Add `REZOPS_DESIGN` env var export for Codex host |
...
1. Read `~/.rezops/openai.json` → `{ "api_key": "sk-..." }` (file permissions 0600)
...
3. If neither exists → guided setup flow:
   ...
   - Write to `~/.rezops/openai.json` with 0600 permissions
...
- Symlinked via existing `~/.claude/skills/rezops/` install path
...
- Auth module (read ~/.rezops/openai.json, fallback to env var, guided setup flow)
...
- Add REZOPS_DESIGN env var export to `scripts/resolvers/preamble.ts` (Codex host)
