# rezops Browser V0 — The AI-Native Development Browser

**Date:** 2026-03-30
**Author:** Garry Tan + Claude Code
**Status:** Phase 1a shipped, Phase 1b in progress
**Branch:** garrytan/rezops-as-browser

## The Thesis

Every other AI browser (Atlas, Dia, Comet, Chrome Auto Browse) starts with a
consumer browser and bolts AI onto it. rezops Browser inverts this. It starts
...
for text editors, rezops Browser does for the browser.

## What It Is Today (Phase 1a, shipped)

A double-clickable macOS .app that wraps Playwright's Chromium with the rezops
...
and run any rezops skill. All without touching a terminal.

```
rezops Browser.app (389MB, 189MB DMG)
...
When the extension side panel hits hard API limits, rezops Browser ships to
...
Every rezops skill becomes a browser capability.
...
| **rezops Browser** | CC runtime + browser viewport | See bug in browser, fix in code, verify | Currently macOS-only, no consumer features |

rezops Browser doesn't compete with consumer browsers. It competes with the
...
| `REZOPS_CHROMIUM_PATH` | **SHIPPED** | Custom Chromium binary support |
...
The 12-month ideal: you open rezops Browser, it detects your project, starts
