...
**Date:** 2026-03-28
**CEO Plan:** ~/.rezops/projects/garrytan-rezops/ceo-plans/2026-03-28-sidebar-prompt-injection-defense.md

## The Problem

The rezops Chrome extension sidebar gives Claude bash access to control the browser.
...
```typescript
// Public API -- any rezops component can call these
...
```json
// ~/.rezops/security/attempts.jsonl
{
...
```
AskUserQuestion:
  "rezops just blocked a prompt injection attempt from {domain}. These detections
   are rare and valuable for improving defenses for all rezops users. Can we
...
2. Cache to ~/.rezops/models/browsesafe-bench/ (not re-downloaded in CI)
...
- rezops becomes the only open source tool with native-speed prompt injection detection.
...
**Limitation:** macOS-only (Linux would need OpenBLAS FFI). But rezops already
...
- [ ] Implement `loadModel()` with download-on-first-use to ~/.rezops/models/
