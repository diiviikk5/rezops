# Design: rezops Self-Learning Infrastructure
...
rezops runs 30+ skills across sessions but learns nothing between them. A /review
...
Structured, typed, confidence-scored learnings that every rezops skill can read and
write. The goal: after 20 sessions on the same codebase, rezops knows every
...
YC founders building with AI. The people who run rezops on real codebases 20+ times
...
| **rezops** | **Per-project JSONL** | **Cross-session, cross-skill** | **Typed, scored, decaying** |

---

## State Systems

rezops has four distinct persistence layers. They share storage patterns
(JSONL in `~/.rezops/projects/$SLUG/`) but serve different purposes:
...
### Release 1: "rezops Learns" (v0.13-0.14) — SHIPPED

**Headline:** Every session makes the next one smarter.

What shipped:
- Learnings persistence at `~/.rezops/projects/{slug}/learnings.jsonl`
...
- E1: Adaptive specialist gating, auto-skip specialists with 0-finding track record.
  Store per-project hit rates via rezops-learnings-log. User can force with --security etc.
...
- E5: Cross-review finding dedup, read rezops-review-read for prior review entries.
  Suppress findings matching a prior user-skipped finding.
- E7: Specialist performance tracking, log per-specialist metrics via rezops-review-log.
...
- Session timeline: every skill auto-logs start/complete events to
  `~/.rezops/projects/$SLUG/timeline.jsonl`. Local-only, never sent anywhere,
...
- Predictive skill suggestion: if your last 3 sessions follow a pattern
  (review, ship, review), rezops suggests what you probably want next.
...
- Timeline binaries: `bin/rezops-timeline-log` and `bin/rezops-timeline-read`.
...
**Headline:** rezops respects your time without compromising your safety.
...
The self-learning roadmap was inspired by ideas from the [Compound Engineering](https://github.com/nicobailon/compound-engineering) project by Nico Bailon. Their exploration of learnings persistence, parallel review agents, and autonomous pipelines catalyzed the design of rezops's approach. We adapted every concept to fit rezops's template system, voice, and architecture rather than porting directly.
