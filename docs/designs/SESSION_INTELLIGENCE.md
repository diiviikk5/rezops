...
rezops already produces valuable artifacts that survive on disk: CEO plans,
...
survive compaction. Because nobody else has rezops's artifact architecture.

## The Insight

rezops already writes structured artifacts to `~/.rezops/projects/$SLUG/`:
- CEO plans: `ceo-plans/`
...
the agent checks `~/.rezops/projects/$SLUG/` for recent plans, reviews, and
...
Plan: ~/.rezops/projects/$SLUG/checkpoints/latest.md"
...
- Not a replacement for Claude's built-in compaction (that handles session
  state; we handle rezops artifacts)
