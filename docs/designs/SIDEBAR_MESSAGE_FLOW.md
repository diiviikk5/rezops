...
How the rezops Browser sidebar actually works. Read this before touching
...
T+1-3s    Extension loads in Chromium
            ...
            │     └── Does NOT fire rezops-extension-ready (waits for sidebar)
...
4. server.ts /sidebar-command handler
   ...
   └── Appends JSON to ~/.rezops/sidebar-agent-queue.jsonl
...
3. content.js onMessage handler
   └── document.dispatchEvent(new CustomEvent('rezops-extension-ready'))

4. welcome.html script
   └── addEventListener('rezops-extension-ready', () => arrow.classList.add('hidden'))
```
...
Server starts → AUTH_TOKEN = crypto.randomUUID()
    ...
    │
    └── sidebar-agent.ts refreshToken() → reads from .rezops/browse.json
...
| Agent stale token | Agent runs but no events appear in sidebar | sidebar-agent has old token from .rezops/browse.json | Agent re-reads token before each event POST |
...
| Queue file | `~/.rezops/sidebar-agent-queue.jsonl` | Filesystem |
| State file | `.rezops/browse.json` | Filesystem |
| Chat log | `~/.rezops/sessions/<id>/chat.jsonl` | Filesystem |
