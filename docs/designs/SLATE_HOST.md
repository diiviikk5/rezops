...
rezops projects... CLAUDE.md works as-is.
...
### Critical env vars for rezops integration
...
Without native `.slate/` publishing, rezops skills vanish when this flag is set.
...
rezops skills already work in Slate via the `.claude/skills/` fallback path.
No changes needed for basic functionality. Users who install rezops for Claude Code
...
- Host names duplicated in `bin/rezops-platform-detect`, `bin/rezops-uninstall`, `bin/dev-setup`
...
- `lib/worktree.ts` only copies `.agents/`, not `.slate/` — E2E tests in worktrees won't
  have Slate skills
- `bin/rezops-uninstall` doesn't know about `.slate/`
- `bin/dev-setup` doesn't wire `.slate/` for contributor dev mode
- `bin/rezops-platform-detect` doesn't detect Slate
