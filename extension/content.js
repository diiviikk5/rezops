...
function showStatusPill(connected, refs) {
  ...
  if (!statusPill) {
    ...
    statusPill.id = 'rezops-status-pill';
  ...
  statusPill.innerHTML = `<span class="rezops-pill-dot"></span> rezops${refText}`;
...
function ensureContainer() {
  ...
  overlayContainer.id = 'rezops-ref-overlays';
...
function renderRefBadges(refs) {
  ...
  for (const ref of refs) {
    ...
    badge.className = 'rezops-ref-badge';
...
function renderRefPanel(refs) {
  ...
  panel.className = 'rezops-ref-panel';

  const header = document.createElement('div');
  header.className = 'rezops-ref-panel-header';
  header.textContent = `rezops refs (${refs.length})`;
  header.style.cssText = 'pointer-events: auto; cursor: move;';
  panel.appendChild(header);

  const list = document.createElement('div');
  list.className = 'rezops-ref-panel-list';
  for (const ref of refs.slice(0, 30)) { // Show max 30 in panel
    const row = document.createElement('div');
    row.className = 'rezops-ref-panel-row';
    const idSpan = document.createElement('span');
    idSpan.className = 'rezops-ref-panel-id';
    idSpan.textContent = ref.ref;
    const roleSpan = document.createElement('span');
    roleSpan.className = 'rezops-ref-panel-role';
    roleSpan.textContent = ref.role;
    const nameSpan = document.createElement('span');
    nameSpan.className = 'rezops-ref-panel-name';
  ...
  if (refs.length > 30) {
    ...
    more.className = 'rezops-ref-panel-more';
...
// Do NOT dispatch rezops-extension-ready here — the extension being loaded
