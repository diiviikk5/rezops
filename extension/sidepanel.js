...
async function pollChat() {
  ...
  try {
    ...
    if (!resp.ok) {
      console.warn(`[rezops sidebar] Chat poll failed: ${resp.status} ${resp.statusText}`);
      return;
    }
    ...
    console.error('[rezops sidebar] Chat poll error:', err.message);
  } finally {
...
async function stopAgent() {
  ...
  try {
    ...
    if (!resp.ok) console.warn(`[rezops sidebar] Stop agent failed: ${resp.status}`);
  } catch (err) {
    console.error('[rezops sidebar] Stop agent error:', err.message);
  }
...
async function pollTabs() {
  ...
  try {
    ...
    try {
      ...
      console.debug('[rezops sidebar] Failed to get active tab URL:', err.message);
    }
    ...
    if (!resp.ok) {
      console.warn(`[rezops sidebar] Tab poll failed: ${resp.status} ${resp.statusText}`);
      return;
    }
    const data = await resp.json();
    if (!data.tabs) return;
    ...
    console.error('[rezops sidebar] Tab poll error:', err.message);
  }
}
...
async function switchBrowserTab(tabId) {
  ...
  try {
    ...
    console.error('[rezops sidebar] Failed to switch browser tab:', err.message);
  }
}
...
document.getElementById('clear-chat').addEventListener('click', async () => {
  ...
  try {
    ...
    if (!resp.ok) console.warn(`[rezops sidebar] Clear chat failed: ${resp.status}`);
  } catch (err) {
    console.error('[rezops sidebar] Clear chat error:', err.message);
  }
  // Reset local state
  chatLineCount = 0;
...
document.getElementById('chat-cookies-btn').addEventListener('click', async () => {
  ...
  try {
    ...
    console.error('[rezops sidebar] Failed to open cookie picker:', err.message);
  }
});
...
async function ensureSseSessionCookie() {
  ...
  try {
    ...
    console.warn('[rezops sidebar] Failed to mint SSE session cookie:', err && err.message);
    return false;
  }
}
...
async function connectSSE() {
  ...
  eventSource.addEventListener('activity', (e) => {
    try { addEntry(JSON.parse(e.data)); } catch (err) {
      console.error('[rezops sidebar] Failed to parse activity event:', err.message);
    }
  });
  ...
  eventSource.addEventListener('gap', (e) => {
    try {
      ...
      console.error('[rezops sidebar] Failed to parse gap event:', err.message);
    }
  });
}
...
async function fetchRefs() {
  ...
  try {
    ...
    console.error('[rezops sidebar] Failed to fetch refs:', err.message);
  }
}
  ...
  try {
    ...
    inspectorSSE.addEventListener('inspectResult', (e) => {
      try {
        ...
        console.error('[rezops sidebar] Failed to parse inspectResult:', err.message);
      }
    });
    ...
    console.debug('[rezops sidebar] Inspector SSE not available:', err.message);
  }
}
...
document.getElementById('conn-copy').addEventListener('click', () => {
  navigator.clipboard.writeText('/open-rezops-browser').then(() => {
    const btn = document.getElementById('conn-copy');
    btn.textContent = 'copied!';
    setTimeout(() => { btn.textContent = '/open-rezops-browser'; }, 2000);
  });
});
