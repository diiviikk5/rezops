...
version: 1.0.0
description: |
  Manage project learnings. Review, search, prune, and export what rezops
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"learn","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.rezops/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/rezops/bin/rezops-timeline-log '{"skill":"learn","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
```bash
~/.claude/skills/rezops/bin/rezops-question-log '{"skill":"learn","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
see what rezops has learned across sessions on this project, search for relevant
...
```bash
eval "$(~/.claude/skills/rezops/bin/rezops-slug 2>/dev/null)"
~/.claude/skills/rezops/bin/rezops-learnings-search --limit 20 2>/dev/null || echo "No learnings yet."
```
...
rezops will automatically capture patterns, pitfalls, and insights it discovers."
...
```bash
eval "$(~/.claude/skills/rezops/bin/rezops-slug 2>/dev/null)"
~/.claude/skills/rezops/bin/rezops-learnings-search --query "USER_QUERY" --limit 20 2>/dev/null || echo "No matches."
```
...
```bash
eval "$(~/.claude/skills/rezops/bin/rezops-slug 2>/dev/null)"
~/.claude/skills/rezops/bin/rezops-learnings-search --limit 100 2>/dev/null
```
...
```bash
eval "$(~/.claude/skills/rezops/bin/rezops-slug 2>/dev/null)"
~/.claude/skills/rezops/bin/rezops-learnings-search --limit 50 2>/dev/null
```
...
LEARN_FILE="$REZOPS_HOME/projects/$SLUG/learnings.jsonl"
...
```bash
~/.claude/skills/rezops/bin/rezops-learnings-log '{"skill":"learn","type":"TYPE","key":"KEY","insight":"INSIGHT","confidence":N,"source":"user-stated","files":["FILE1"]}'
```
