 ...
 *   3. ~/.claude/skills/rezops/browse/dist/browse
...
export function resolveBrowseBin(): string {
  ...
  const globalPath = path.join(home, ".claude/skills/rezops/browse/dist/browse");
  ...
  throw new BrowseClientError(
    ...
    [
      ...
      "",
      "make-pdf needs browse (the rezops Chromium daemon) to render PDFs.",
      "Tried:",
      ...
      "  - PATH: `browse`",
      "",
      "To fix: run rezops setup from the rezops repo:",
      "  cd ~/.claude/skills/rezops && ./setup",
      "",
