---
name: open-rezops-browser
version: 0.2.0
description: |
  Launch rezops Browser — AI-controlled Chromium with the sidebar extension baked in.
  Opens a visible browser window where you can watch every action in real time.
  The sidebar shows a live activity feed and chat. Anti-bot stealth built in.
  Use when asked to "open rezops browser", "launch browser", "connect chrome",
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"open-rezops-browser","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.rezops/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/rezops/bin/rezops-timeline-log '{"skill":"open-rezops-browser","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
```bash
~/.claude/skills/rezops/bin/rezops-question-log '{"skill":"open-rezops-browser","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
# /open-rezops-browser — Launch rezops Browser

Launch rezops Browser — AI-controlled Chromium with the sidebar extension,
...
if [ -f "$(git rev-parse --show-toplevel 2>/dev/null)/.rezops/browse.json" ]; then
  _OLD_PID=$(cat "$(git rev-parse --show-toplevel)/.rezops/browse.json" 2>/dev/null | grep -o '"pid":[0-9]*' | grep -o '[0-9]*')
  [ -n "$_OLD_PID" ] && kill "$_OLD_PID" 2>/dev/null || true
  sleep 1
  [ -n "$_OLD_PID" ] && kill -9 "$_OLD_PID" 2>/dev/null || true
  rm -f "$(git rev-parse --show-toplevel)/.rezops/browse.json"
fi
# Clean Chromium profile locks (can persist after crashes)
_PROFILE_DIR="$HOME/.rezops/chromium-profile"
...
This launches rezops Browser (rebranded Chromium) in headed mode with:
- A visible window you can watch (not your regular Chrome — it stays untouched)
- The rezops sidebar extension auto-loaded via `launchPersistentContext`
- Anti-bot stealth patches (sites like Google and NYTimes work without captchas)
- Custom user agent and rezops Browser branding in Dock/menu bar
- A sidebar agent process for chat commands

The `connect` command auto-discovers the extension from the rezops install
...
```bash
cat "$(git rev-parse --show-toplevel 2>/dev/null)/.rezops/browse.json" 2>/dev/null | grep -o '"port":[0-9]*' | grep -o '[0-9]*'
```
...
_EXT_PATH=""
_ROOT=$(git rev-parse --show-toplevel 2>/dev/null)
[ -n "$_ROOT" ] && [ -f "$_ROOT/.claude/skills/rezops/extension/manifest.json" ] && _EXT_PATH="$_ROOT/.claude/skills/rezops/extension"
[ -z "$_EXT_PATH" ] && [ -f "$HOME/.claude/skills/rezops/extension/manifest.json" ] && _EXT_PATH="$HOME/.claude/skills/rezops/extension"
...
> Chrome is launched with rezops control. You should see Playwright's Chromium
...
>    already show the rezops icon if the extension loaded successfully
> 2. Click the **puzzle piece** → find **rezops browse** → click the **pin icon**
> 3. Click the pinned **rezops icon** in the toolbar
...
> 2. Look for **"rezops browse"** — it should be listed and enabled
...
>
> If the Side Panel badge stays gray (disconnected), click the rezops icon
...
> - Run any rezops skill (`/qa`, `/design-review`, `/benchmark`) and watch
