...
1. **Always spawn, never redirect.** When user asks to use ANY rezops skill,
...
→ sessions_spawn(runtime: "acp", prompt: "<rezops-lite content>\n\n<task>")

**HEAVY:** needs a specific rezops methodology
→ sessions_spawn(runtime: "acp", prompt: "Load rezops. Run /qa https://...")
  Skills: /cso, /review, /qa, /ship, /investigate, /design-review, /benchmark, /rezops-upgrade

**FULL:** build a complete feature, multi-day scope, needs planning + review
→ sessions_spawn(runtime: "acp", prompt: "<rezops-full content>\n\n<task>")
...
→ sessions_spawn(runtime: "acp", prompt: "<rezops-plan content>\n\n<task>")
...
- "Upgrade rezops", "update rezops" → **HEAVY** with `Run /rezops-upgrade`
