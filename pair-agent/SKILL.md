...
description: |
  ...
  "let another agent use my browser", or "give browser access". (rezops)
voice-triggers:
  - "pair agent"
  - "connect agent"
  - "share my browser"
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"pair-agent","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.rezops/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/rezops/bin/rezops-timeline-log '{"skill":"pair-agent","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
```bash
~/.claude/skills/rezops/bin/rezops-question-log '{"skill":"pair-agent","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
Your rezops browser runs a local HTTP server. This skill creates a one-time setup key,
...
side panel if you have rezops Browser open."
...
credentials are written to `~/.openclaw/skills/rezops/browse-remote.json`.
...
to `~/.codex/skills/rezops/browse-remote.json`.
...
`~/.cursor/skills/rezops/browse-remote.json`.
