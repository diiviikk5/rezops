...
description: |
  ...
  should be reviewed before implementation. (rezops)
allowed-tools:
  - Read
  - Edit
  - Grep
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"plan-design-review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.rezops/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/rezops/bin/rezops-timeline-log '{"skill":"plan-design-review","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
```bash
~/.claude/skills/rezops/bin/rezops-question-log '{"skill":"plan-design-review","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
### The rezops designer — YOUR PRIMARY TOOL

You have the **rezops designer**, an AI mockup generator that creates real visual mockups
...
MUST be saved to `~/.rezops/projects/$SLUG/designs/`, NEVER to `.context/`,
...
rezops designer is available (`DESIGN_READY` was printed during setup), **generate
mockups immediately.** Do not ask permission. This is the default behavior.

Tell the user: "Generating visual mockups with the rezops designer. This is how we
...
`~/.rezops/projects/$SLUG/designs/` (user config directory, not project files).
Mockups are design artifacts that inform the plan, not code changes. The rezops
...
- `mkdir -p ~/.rezops/projects/$SLUG/designs/...`
...
```bash
eval "$(~/.claude/skills/rezops/bin/rezops-slug 2>/dev/null)"
_DESIGN_DIR="$HOME/.rezops/projects/$SLUG/designs/<screen-name>-$(date +%Y%m%d)"
mkdir -p "$_DESIGN_DIR"
...
**If `DESIGN_NOT_AVAILABLE`:** Tell the user: "The rezops designer isn't set up yet. Run `$D setup` to enable visual mockups. Proceeding with text-only review, but you're missing the best part." Then proceed to review passes with text-based review.
...
**Log the result:**
```bash
~/.claude/skills/rezops/bin/rezops-review-log '{"skill":"design-outside-voices","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
```bash
$D generate --brief "<description of what 10/10 looks like for this dimension>" --output /tmp/rezops-ideal-<dimension>.png
```
...
Source: [OpenAI "Designing Delightful Frontends with GPT-5.4"](https://developers.openai.com/blog/designing-delightful-frontends-with-gpt-5-4) (Mar 2026) + rezops design methodology.
...
|----------------|-------------|-----------|-------|
| [screen name]  | ~/.rezops/projects/$SLUG/designs/[folder]/[filename].png | [brief description] | [constraints from review] |
```
...
`~/.rezops/` (user config directory, not project files). The skill preamble
already writes to `~/.rezops/sessions/` and `~/.rezops/analytics/` — this is
the same pattern. The review dashboard depends on this data. Skipping this
command breaks the review readiness dashboard in /ship.

```bash
~/.claude/skills/rezops/bin/rezops-review-log '{"skill":"plan-design-review","timestamp":"TIMESTAMP","status":"STATUS","initial_score":N,"overall_score":N,"unresolved":N,"decisions_made":N,"commit":"COMMIT"}'
```
...
```bash
~/.claude/skills/rezops/bin/rezops-learnings-log '{"skill":"plan-design-review","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
