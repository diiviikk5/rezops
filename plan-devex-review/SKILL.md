...
description: |
  ...
  (APIs, CLIs, SDKs, libraries, platforms, docs). (rezops)
voice-triggers:
  - "dx review"
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"plan-devex-review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.rezops/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/rezops/bin/rezops-timeline-log '{"skill":"plan-devex-review","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
```bash
~/.claude/skills/rezops/bin/rezops-question-log '{"skill":"plan-devex-review","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
\`~/.claude/skills/rezops/plan-devex-review/dx-hall-of-fame.md\`
...
Load the "## Pass 1" section from `~/.claude/skills/rezops/plan-devex-review/dx-hall-of-fame.md`
...
```bash
eval "$(~/.claude/skills/rezops/bin/rezops-slug 2>/dev/null)"
~/.claude/skills/rezops/bin/rezops-review-read 2>/dev/null | grep plan-devex-review || echo "NO_PRIOR_DX_REVIEWS"
```
...
Load reference: Read the "## Pass 1" section from `~/.claude/skills/rezops/plan-devex-review/dx-hall-of-fame.md`.
...
Load reference: Read the "## Pass 2" section from `~/.claude/skills/rezops/plan-devex-review/dx-hall-of-fame.md`.
...
Load reference: Read the "## Pass 3" section from `~/.claude/skills/rezops/plan-devex-review/dx-hall-of-fame.md`.
...
Load reference: Read the "## Pass 4" section from `~/.claude/skills/rezops/plan-devex-review/dx-hall-of-fame.md`.
...
Load reference: Read the "## Pass 5" section from `~/.claude/skills/rezops/plan-devex-review/dx-hall-of-fame.md`.
...
Load reference: Read the "## Pass 6" section from `~/.claude/skills/rezops/plan-devex-review/dx-hall-of-fame.md`.
...
Load reference: Read the "## Pass 7" section from `~/.claude/skills/rezops/plan-devex-review/dx-hall-of-fame.md`.
...
Load reference: Read the "## Pass 8" section from `~/.claude/skills/rezops/plan-devex-review/dx-hall-of-fame.md`.
...
This is NOT a scored pass. It's a checklist of proven patterns from rezops's own DX.
...
**Persist the result:**
```bash
~/.claude/skills/rezops/bin/rezops-review-log '{"skill":"codex-plan-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
`~/.rezops/` (user config directory, not project files).

```bash
~/.claude/skills/rezops/bin/rezops-review-log '{"skill":"plan-devex-review","timestamp":"TIMESTAMP","status":"STATUS","initial_score":N,"overall_score":N,"product_type":"TYPE","tthw_current":"TTHW_CURRENT","tthw_target":"TTHW_TARGET","mode":"MODE","persona":"PERSONA","competitive_tier":"TIER","pass_scores":{"getting_started":N,"api_design":N,"errors":N,"docs":N,"upgrade":N,"dev_env":N,"community":N,"measurement":N},"unresolved":N,"commit":"COMMIT"}'
```
...
- Search the plan file for a \`## rezops REVIEW REPORT\` section **anywhere** in the file
  (not just at the end — content may have been added after it).
- If found, **replace it** entirely using the Edit tool. Match from \`## rezops REVIEW REPORT\`
...
```bash
~/.claude/skills/rezops/bin/rezops-learnings-log '{"skill":"plan-devex-review","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
