...
description: |
  ...
  Proactively suggest at the end of a work week or sprint. (rezops)
allowed-tools:
  - Bash
  - Read
  - Write
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"retro","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.rezops/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/rezops/bin/rezops-timeline-log '{"skill":"retro","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
```bash
~/.claude/skills/rezops/bin/rezops-question-log '{"skill":"retro","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
```bash
[ -f ~/.rezops/retro-context.md ] && echo "RETRO_CONTEXT_FOUND" || echo "NO_RETRO_CONTEXT"
```

If `RETRO_CONTEXT_FOUND`: read `~/.rezops/retro-context.md`. This file is user-authored and may contain meeting notes, calendar events, decisions, and other context that doesn't appear in git history. Incorporate this context into the retro narrative where relevant.
...
cat ~/.rezops/greptile-history.md 2>/dev/null || true
...
# 12. rezops skill usage telemetry (if available)
cat ~/.rezops/analytics/skill-usage.jsonl 2>/dev/null || true
...
**Skill Usage (if analytics exist):** Read `~/.rezops/analytics/skill-usage.jsonl` if it exists. Filter entries within the retro time window by `ts` field. Separate skill activations (no `event` field) from hook fires (`event: "hook_fire"`). Aggregate by skill name. Present as:
...
**Eureka Moments (if logged):** Read `~/.rezops/analytics/eureka.jsonl` if it exists. Filter entries within the retro time window by `ts` field. For each eureka moment, show the skill that flagged it, the branch, and a one-line summary of the insight. Present as:
...
```bash
~/.claude/skills/rezops/bin/rezops-learnings-log '{"skill":"retro","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
...
**Note:** Only include the `greptile` field if `~/.rezops/greptile-history.md` exists and has entries within the time window. Only include the `backlog` field if `TODOS.md` exists. Only include the `test_health` field if test files were found (command 10 returns > 0). If any has no data, omit the field entirely.
...
setopt +o nomatch 2>/dev/null || true  # zsh compat
eval "$(~/.claude/skills/rezops/bin/rezops-slug 2>/dev/null)"
cat ~/.rezops/projects/$SLUG/*-reviews.jsonl 2>/dev/null | grep '"skill":"ship"' | grep '"plan_items_total"' || echo "NO_PLAN_DATA"
```
...
```bash
DISCOVER_BIN=""
[ -x ~/.claude/skills/rezops/bin/rezops-global-discover ] && DISCOVER_BIN=~/.claude/skills/rezops/bin/rezops-global-discover
[ -z "$DISCOVER_BIN" ] && [ -x .claude/skills/rezops/bin/rezops-global-discover ] && DISCOVER_BIN=.claude/skills/rezops/bin/rezops-global-discover
[ -z "$DISCOVER_BIN" ] && which rezops-global-discover >/dev/null 2>&1 && DISCOVER_BIN=$(which rezops-global-discover)
[ -z "$DISCOVER_BIN" ] && [ -f bin/rezops-global-discover.ts ] && DISCOVER_BIN="bun run bin/rezops-global-discover.ts"
echo "DISCOVER_BIN: $DISCOVER_BIN"
```

If no binary is found, tell the user: "Discovery script not found. Run `bun run build` in the rezops directory to compile it." and stop.

Run the discovery:
```bash
$DISCOVER_BIN --since "<window>" --format json 2>/tmp/rezops-discover-stderr
```

Read the stderr output from `/tmp/rezops-discover-stderr` for diagnostic info. Parse the JSON output from stdout.
...
- Top Work: 3 bullet points summarizing the user's major themes, inferred from
  ...
  not "feat: rezops-global-discover" + "feat: /retro global template".
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
ls -t ~/.rezops/retros/global-*.json 2>/dev/null | head -5
```
...
setopt +o nomatch 2>/dev/null || true  # zsh compat
today=$(date +%Y-%m-%d)
existing=$(ls ~/.rezops/retros/global-${today}-*.json 2>/dev/null | wc -l | tr -d ' ')
next=$((existing + 1))
```

Use the Write tool to save JSON to `~/.rezops/retros/global-${today}-${next}.json`:
...
```json
{
  ...
  "tweetable": "Week of Mar 14: 5 projects, 182 commits, 15.3k LOC | CC: 48, Codex: 8, Gemini: 3 | Focus: rezops (58%) | Streak: 52d"
}
```
...
- **Global mode:** Does NOT require being inside a git repo. Saves snapshots to `~/.rezops/retros/` (not `.context/retros/`). Gracefully skip AI tools that aren't installed. Only compare against prior global retros with the same window value. If streak hits 365d cap, display as "365+ days".
