#!/usr/bin/env bun
/**
 * analytics — CLI for viewing rezops skill usage statistics.
 *
 * Reads ~/.rezops/analytics/skill-usage.jsonl and displays:
...
const ANALYTICS_FILE = path.join(os.homedir(), '.rezops', 'analytics', 'skill-usage.jsonl');
...
export function formatReport(events: AnalyticsEvent[], period: string = 'all'): string {
  ...
  const lines: string[] = [];
  lines.push('rezops skill usage analytics');
