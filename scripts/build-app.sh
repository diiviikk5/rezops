#!/bin/bash
# Build rezops Browser.app — macOS application bundle
#
...
#
# Output: dist/rezops Browser.app and dist/rezops-Browser.dmg
#
# Usage:
...
APP_NAME="rezops Browser"
BUNDLE_ID="com.rezops.browser"
...
# Launcher script
cp "$ROOT/scripts/app/rezops-browser" "$APP_DIR/Contents/MacOS/rezops-browser"
chmod +x "$APP_DIR/Contents/MacOS/rezops-browser"
...
# Patch the bundled Chromium's Info.plist so macOS shows "rezops Browser"
...
if [ -f "$CHROMIUM_PLIST" ]; then
  ...
  # Replace Chromium's icon with ours so the Dock shows the rezops icon
...
echo "    Contents/MacOS/rezops-browser     (launcher)"
...
DMG_PATH="$BUILD_DIR/rezops-Browser.dmg"
echo ""
echo "  Creating DMG..."
rm -f "$DMG_PATH"
