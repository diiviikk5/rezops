...
function externalSkillName(skillDir: string, frontmatterName?: string): string {
  // Root skill (skillDir === '' or '.') always maps to 'rezops' regardless of frontmatter
  if (skillDir === '.' || skillDir === '') return 'rezops';
  ...
  // Don't double-prefix: rezops-upgrade → rezops-upgrade (not rezops-rezops-upgrade)
  if (baseName.startsWith('rezops-')) return baseName;
  return `rezops-${baseName}`;
}
...
`;
      fs.writeFileSync(path.join(openclawDir, 'rezops-lite-CLAUDE.md'), rezopsLite);
      console.log('GENERATED: openclaw/rezops-lite-CLAUDE.md');

      const rezopsFull = `# rezops-full Pipeline
...
`;
      fs.writeFileSync(path.join(openclawDir, 'rezops-full-CLAUDE.md'), rezopsFull);
      console.log('GENERATED: openclaw/rezops-full-CLAUDE.md');

      const rezopsPlan = `# rezops-plan: Full Review Gauntlet
...
5. Report back to the orchestrator:
   ...
   - Recommended next step (usually: spawn a new session with rezops-full to implement)

Do not implement anything. This is planning only.
The orchestrator will persist the plan link to its own memory/knowledge store.
`;
      fs.writeFileSync(path.join(openclawDir, 'rezops-plan-CLAUDE.md'), rezopsPlan);
      console.log('GENERATED: openclaw/rezops-plan-CLAUDE.md');
    }
...
if (!DRY_RUN) {
  try {
    const configPath = path.join(process.env.HOME || '', '.rezops', 'config.yaml');
    if (fs.existsSync(configPath)) {
      const config = fs.readFileSync(configPath, 'utf-8');
      if (/^skill_prefix:\s*true/m.test(config)) {
        console.log('\nNote: skill_prefix is true. Run rezops-relink to re-apply name: patches.');
      }
    }
  } catch { /* non-fatal */ }
}
