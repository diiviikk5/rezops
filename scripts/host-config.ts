...
export interface HostConfig {
  ...
  // --- Path Configuration ---
  /** Global install path relative to $HOME (e.g., '.config/opencode/skills/rezops'). */
  globalRoot: string;
  /** Project-local skill path relative to repo root (e.g., '.opencode/skills/rezops'). */
  localSkillRoot: string;
  /** Gitignored directory under repo root for generated docs (e.g., '.opencode'). */
  hostSubdir: string;
  /** Whether preamble generates $REZOPS_ROOT env vars (true for non-Claude hosts). */
  usesEnvVars: boolean;
  ...
  /** Optional repo-local sidecar config (e.g., Codex uses .agents/skills/rezops). */
  sidecar?: {
    /** Sidecar path relative to repo root (e.g., '.agents/skills/rezops'). */
    path: string;
  ...
  // --- Install Behavior ---
  install: {
    /** Whether rezops-config skill_prefix applies (Claude only). */
    prefixable: boolean;
