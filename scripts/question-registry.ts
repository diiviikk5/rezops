/**
 * Question Registry — typed schema for AskUserQuestion invocations across rezops.
 *
 * Purpose
 * -------
 ...
 *
 * Not every AskUserQuestion in rezops needs a registry entry right away. Skills
...
export interface QuestionDef {
  ...
  id: string;
  /** Skill that owns this question (must match a rezops skill directory name) */
  skill: string;
...
export const QUESTIONS = {
  ...
  // -----------------------------------------------------------------------
  // /rezops-upgrade — version upgrade
  // -----------------------------------------------------------------------
  'rezops-upgrade-inline': {
    id: 'rezops-upgrade-inline',
    skill: 'rezops-upgrade',
    category: 'approval',
    door_type: 'two-way',
    options: ['yes-upgrade', 'always-auto', 'not-now', 'never-ask'],
    description: "Upgrade rezops now? (Also: always auto-upgrade, snooze, or disable the prompt.)",
  },
  ...
  'preamble-telemetry-consent': {
    ...
    description: "Share usage data with rezops? community (recommended) / anonymous / off",
  },
  'preamble-proactive-behavior': {
    ...
    door_type: 'two-way',
    options: ['on', 'off'],
    description: "Let rezops proactively suggest skills based on conversation context?",
  },
  'preamble-routing-injection': {
    ...
    description: "Add rezops skill routing rules to CLAUDE.md?",
  },
  'preamble-vendored-migration': {
    ...
    description: "This repo has vendored rezops (deprecated) — migrate to team mode?",
  },
