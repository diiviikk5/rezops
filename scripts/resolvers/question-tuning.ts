...
function binDir(ctx: TemplateContext): string {
  return ctx.host === 'codex' ? '$REZOPS_BIN' : ctx.paths.binDir;
}
...
\`${bin}/rezops-question-preference --check "<id>"\`.
...
\`\`\`bash
${bin}/rezops-question-log '{"skill":"${ctx.skillName}","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
\`\`\`
...
\`\`\`bash
${bin}/rezops-question-preference --write '{"question_id":"<id>","preference":"<pref>","source":"inline-user","free_text":"<optional original words>"}'
\`\`\`
...
Before each AskUserQuestion, run: \`${bin}/rezops-question-preference --check "<id>"\`.
...
\`\`\`bash
${bin}/rezops-question-log '{"skill":"${ctx.skillName}","question_id":"<id>","question_summary":"<short>","category":"<cat>","door_type":"<one|two>-way","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
\`\`\``;
}
...
\`\`\`bash
${bin}/rezops-question-preference --write '{"question_id":"<id>","preference":"<never|always-ask|ask-only-for-one-way>","source":"inline-user"}'
\`\`\`
