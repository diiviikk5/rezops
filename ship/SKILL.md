...
~/.claude/skills/rezops/bin/rezops-timeline-log '{"skill":"ship","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
```bash
eval "$(~/.claude/skills/rezops/bin/rezops-slug 2>/dev/null)"
...
`~/.claude/skills/rezops/bin/rezops-question-preference --check "<id>"`.
...
```bash
~/.claude/skills/rezops/bin/rezops-question-log '{"skill":"ship","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
```bash
~/.claude/skills/rezops/bin/rezops-question-preference --write '{"question_id":"<id>","preference":"<pref>","source":"inline-user","free_text":"<optional original words>"}'
```
...
Before building anything unfamiliar, **search first.** See `~/.claude/skills/rezops/ETHOS.md`.
...
```bash
~/.claude/skills/rezops/bin/rezops-review-read
```
...
For Design Review: run `source <(~/.claude/skills/rezops/bin/rezops-diff-scope <base> 2>/dev/null)`. If `SCOPE_FRONTEND=true` and no design review (plan-design-review or design-review-lite) exists in the dashboard, mention: "Design Review not run — this PR changes frontend code. The lite design check will run automatically in Step 9, but consider running /design-review for a full visual audit post-implementation." Still never block.
...
```bash
eval "$(~/.claude/skills/rezops/bin/rezops-slug 2>/dev/null)" && mkdir -p ~/.rezops/projects/$SLUG
USER=$(whoami)
...
```bash
_CROSS_PROJ=$(~/.claude/skills/rezops/bin/rezops-config get cross_project_learnings 2>/dev/null || echo "unset")
echo "CROSS_PROJECT: $_CROSS_PROJ"
if [ "$_CROSS_PROJ" = "true" ]; then
  ~/.claude/skills/rezops/bin/rezops-learnings-search --limit 10 --cross-project 2>/dev/null || true
else
  ~/.claude/skills/rezops/bin/rezops-learnings-search --limit 10 2>/dev/null || true
fi
```
...
If A: run `~/.claude/skills/rezops/bin/rezops-config set cross_project_learnings true`
If B: run `~/.claude/skills/rezops/bin/rezops-config set cross_project_learnings false`
...
```bash
source <(~/.claude/skills/rezops/bin/rezops-diff-scope <base> 2>/dev/null)
```
...
```bash
~/.claude/skills/rezops/bin/rezops-review-log '{"skill":"design-review-lite","timestamp":"TIMESTAMP","status":"STATUS","findings":N,"auto_fixed":M,"commit":"COMMIT"}'
```
...
```bash
source <(~/.claude/skills/rezops/bin/rezops-diff-scope <base> 2>/dev/null) || true
...
```bash
~/.claude/skills/rezops/bin/rezops-specialist-stats 2>/dev/null || true
```
...
1. **Testing** — read `~/.claude/skills/rezops/review/specialists/testing.md`
2. **Maintainability** — read `~/.claude/skills/rezops/review/specialists/maintainability.md`
...
3. **Security** — if SCOPE_AUTH=true, OR if SCOPE_BACKEND=true AND DIFF_LINES > 100. Read `~/.claude/skills/rezops/review/specialists/security.md`
4. **Performance** — if SCOPE_BACKEND=true OR SCOPE_FRONTEND=true. Read `~/.claude/skills/rezops/review/specialists/performance.md`
5. **Data Migration** — if SCOPE_MIGRATIONS=true. Read `~/.claude/skills/rezops/review/specialists/data-migration.md`
6. **API Contract** — if SCOPE_API=true. Read `~/.claude/skills/rezops/review/specialists/api-contract.md`
7. **Design** — if SCOPE_FRONTEND=true. Use the existing design review checklist at `~/.claude/skills/rezops/review/design-checklist.md`
...
```bash
~/.claude/skills/rezops/bin/rezops-learnings-search --type pitfall --query "{specialist domain}" --limit 5 2>/dev/null || true
```
...
1. The red-team checklist from `~/.claude/skills/rezops/review/specialists/red-team.md`
...
```bash
~/.claude/skills/rezops/bin/rezops-review-log '{"skill":"review","timestamp":"TIMESTAMP","status":"STATUS","issues_found":N,"critical":N,"informational":N,"quality_score":SCORE,"specialists":SPECIALISTS_JSON,"findings":FINDINGS_JSON,"commit":"'"$(git rev-parse --short HEAD)"'","via":"ship"}'
```
...
OLD_CFG=$(~/.claude/skills/rezops/bin/rezops-config get codex_reviews 2>/dev/null || true)
...
```bash
~/.claude/skills/rezops/bin/rezops-review-log '{"skill":"adversarial-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","tier":"always","gate":"GATE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
```bash
~/.claude/skills/rezops/bin/rezops-learnings-log '{"skill":"ship","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
...
> You are executing the /document-release workflow after a code push. Read the full skill file `${HOME}/.claude/skills/rezops/document-release/SKILL.md` and execute its complete workflow end-to-end, including CHANGELOG clobber protection, doc exclusions, risky-change gates, and named staging. Do NOT attempt to edit the PR body — no PR exists yet. Branch: `<branch>`, base: `<base>`.
>
