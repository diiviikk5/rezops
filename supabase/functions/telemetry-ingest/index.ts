// rezops telemetry-ingest edge function
// Validates and inserts a batch of telemetry events.
// Called by bin/rezops-telemetry-sync.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (const event of events) {
      // Required fields
      if (!event.ts || !event.rezops_version || !event.os || !event.outcome) {
      ...
      rows.push({
        ...
        rezops_version: String(event.rezops_version).slice(0, 20),
