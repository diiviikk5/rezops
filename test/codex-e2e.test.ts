...
const CODEX_E2E_TOUCHFILES: Record<string, string[]> = {
  ...
  'codex-review-findings':   ['review/**', '.agents/skills/rezops-review/**', 'codex/**', 'test/helpers/codex-session-runner.ts'],
};
...
describeCodex('Codex E2E', () => {
  ...
  testIfSelected('codex-discover-skill', async () => {
    // Install rezops-review skill to a temp HOME and ask Codex to list skills
    const skillDir = path.join(testWorktree, '.agents', 'skills', 'rezops-review');
    ...
    expect(
      outputLower.includes('review') || outputLower.includes('rezops') || outputLower.includes('skill'),
    ).toBe(true);
  }, 120_000);

  // Validates that Codex can invoke the rezops-review skill, run a diff-based
  ...
  testIfSelected('codex-review-findings', async () => {
    // Install rezops-review skill and ask Codex to review the worktree
    const skillDir = path.join(testWorktree, '.agents', 'skills', 'rezops-review');

    const result = await runCodexSkill({
      skillDir,
      prompt: 'Run the rezops-review skill on this repository. Review the current branch diff and report your findings.',
      timeoutMs: 540_000,
      cwd: testWorktree,
      skillName: 'rezops-review',
    });
