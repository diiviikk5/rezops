/**
 * rezops-config explain_level round-trip + validation tests.
 *
 * Coverage:
...
const BIN_CONFIG = path.join(ROOT, 'bin', 'rezops-config');

let tmpHome: string;

beforeEach(() => {
  tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'rezops-cfg-test-'));
});
...
function run(...args: string[]): { stdout: string; stderr: string; status: number } {
  const res = spawnSync(BIN_CONFIG, args, {
    env: { ...process.env, REZOPS_STATE_DIR: tmpHome },
    encoding: 'utf-8',
    cwd: ROOT,
  });
  return {
...
describe('rezops-config explain_level', () => {
