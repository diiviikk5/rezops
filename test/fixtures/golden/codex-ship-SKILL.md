...
REZOPS_ROOT="$HOME/.codex/skills/rezops"
[ -n "$_ROOT" ] && [ -d "$_ROOT/.agents/skills/rezops" ] && REZOPS_ROOT="$_ROOT/.agents/skills/rezops"
REZOPS_BIN="$REZOPS_ROOT/bin"
REZOPS_BROWSE="$REZOPS_ROOT/browse/dist"
REZOPS_DESIGN="$REZOPS_ROOT/design/dist"
_UPD=$($REZOPS_BIN/rezops-update-check 2>/dev/null || .agents/skills/rezops/bin/rezops-update-check 2>/dev/null || true)
...
_VENDORED="no"
if [ -d ".agents/skills/rezops" ] && [ ! -L ".agents/skills/rezops" ]; then
  if [ -f ".agents/skills/rezops/VERSION" ] || [ -d ".agents/skills/rezops/.git" ]; then
    _VENDORED="yes"
  fi
fi
...
`.agents/skills/rezops/`. Vendoring is deprecated. We will not keep vendored copies
...
> This project has rezops vendored in `.agents/skills/rezops/`. Vendoring is deprecated.
...
If A:
1. Run `git rm -r .agents/skills/rezops/`
2. Run `echo '.agents/skills/rezops/' >> .gitignore`
...
**If "Add as P0 TODO":**
- If `TODOS.md` exists, add the entry following the format in `review/TODOS-format.md` (or `.agents/skills/rezops/review/TODOS-format.md`).
...
1. Read `.agents/skills/rezops/review/checklist.md`. If the file cannot be read, **STOP** and report the error.
...
2. **Read `.agents/skills/rezops/review/design-checklist.md`.** If the file cannot be read, skip design review with a note: "Design checklist not found — skipping design review."
...
> You are classifying Greptile review comments for a /ship workflow. Read `.agents/skills/rezops/review/greptile-triage.md` and follow the fetch, filter, classify, and **escalation detection** steps. Do NOT fix code, do NOT reply to comments, do NOT commit — report only.
>
...
Read `.agents/skills/rezops/review/TODOS-format.md` for the canonical format reference.
...
> You are executing the /document-release workflow after a code push. Read the full skill file `${HOME}/.agents/skills/rezops/document-release/SKILL.md` and execute its complete workflow end-to-end, including CHANGELOG clobber protection, doc exclusions, risky-change gates, and named staging. Do NOT attempt to edit the PR body — no PR exists yet. Branch: `<branch>`, base: `<base>`.
>
