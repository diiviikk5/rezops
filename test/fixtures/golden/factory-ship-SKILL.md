...
description: |
  ...
  is ready, asks about deploying, wants to push code up, or asks to create a PR. (rezops)
user-invocable: true
...
REZOPS_ROOT="$HOME/.factory/skills/rezops"
[ -n "$_ROOT" ] && [ -d "$_ROOT/.factory/skills/rezops" ] && REZOPS_ROOT="$_ROOT/.factory/skills/rezops"
REZOPS_BIN="$REZOPS_ROOT/bin"
REZOPS_BROWSE="$REZOPS_ROOT/browse/dist"
REZOPS_DESIGN="$REZOPS_ROOT/design/dist"
_UPD=$($REZOPS_BIN/rezops-update-check 2>/dev/null || .factory/skills/rezops/bin/rezops-update-check 2>/dev/null || true)
...
_PROACTIVE=$($REZOPS_BIN/rezops-config get proactive 2>/dev/null || echo "true")
_PROACTIVE_PROMPTED=$([ -f ~/.rezops/.proactive-prompted ] && echo "yes" || echo "no")
_BRANCH=$(git branch --show-current 2>/dev/null || echo "unknown")
echo "BRANCH: $_BRANCH"
_SKILL_PREFIX=$($REZOPS_BIN/rezops-config get skill_prefix 2>/dev/null || echo "false")
echo "PROACTIVE: $_PROACTIVE"
echo "PROACTIVE_PROMPTED: $_PROACTIVE_PROMPTED"
echo "SKILL_PREFIX: $_SKILL_PREFIX"
source <($REZOPS_BIN/rezops-repo-mode 2>/dev/null) || true
REPO_MODE=${REPO_MODE:-unknown}
echo "REPO_MODE: $REPO_MODE"
_LAKE_SEEN=$([ -f ~/.rezops/.completeness-intro-seen ] && echo "yes" || echo "no")
echo "LAKE_INTRO: $_LAKE_SEEN"
_TEL=$($REZOPS_BIN/rezops-config get telemetry 2>/dev/null || true)
...
_EXPLAIN_LEVEL=$($REZOPS_BIN/rezops-config get explain_level 2>/dev/null || echo "default")
if [ "$_EXPLAIN_LEVEL" != "default" ] && [ "$_EXPLAIN_LEVEL" != "terse" ]; then _EXPLAIN_LEVEL="default"; fi
echo "EXPLAIN_LEVEL: $_EXPLAIN_LEVEL"
# Question tuning (see /plan-tune). Observational only in V1.
_QUESTION_TUNING=$($REZOPS_BIN/rezops-config get question_tuning 2>/dev/null || echo "false")
echo "QUESTION_TUNING: $_QUESTION_TUNING"
mkdir -p ~/.rezops/analytics
if [ "$_TEL" != "off" ]; then
echo '{"skill":"ship","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.rezops/analytics/skill-usage.jsonl 2>/dev/null || true
fi
# zsh-compatible: use find instead of glob to avoid NOMATCH error
for _PF in $(find ~/.rezops/analytics -maxdepth 1 -name '.pending-*' 2>/dev/null); do
  if [ -f "$_PF" ]; then
    if [ "$_TEL" != "off" ] && [ -x "$REZOPS_BIN/rezops-telemetry-log" ]; then
      $REZOPS_BIN/rezops-telemetry-log --event-type skill_run --skill _pending_finalize --outcome unknown --session-id "$_SESSION_ID" 2>/dev/null || true
    fi
...
# Learnings count
eval "$($REZOPS_BIN/rezops-slug 2>/dev/null)" 2>/dev/null || true
_LEARN_FILE="${REZOPS_HOME:-$HOME/.rezops}/projects/${SLUG:-unknown}/learnings.jsonl"
if [ -f "$_LEARN_FILE" ]; then
  _LEARN_COUNT=$(wc -l < "$_LEARN_FILE" 2>/dev/null | tr -d ' ')
  echo "LEARNINGS: $_LEARN_COUNT entries loaded"
  if [ "$_LEARN_COUNT" -gt 5 ] 2>/dev/null; then
    $REZOPS_BIN/rezops-learnings-search --limit 3 2>/dev/null || true
  fi
else
  echo "LEARNINGS: 0"
fi
# Session timeline: record skill start (local-only, never sent anywhere)
$REZOPS_BIN/rezops-timeline-log '{"skill":"ship","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
# Check if CLAUDE.md has routing rules
_HAS_ROUTING="no"
if [ -f CLAUDE.md ] && grep -q "## Skill routing" CLAUDE.md 2>/dev/null; then
  _HAS_ROUTING="yes"
fi
_ROUTING_DECLINED=$($REZOPS_BIN/rezops-config get routing_declined 2>/dev/null || echo "false")
echo "HAS_ROUTING: $_HAS_ROUTING"
echo "ROUTING_DECLINED: $_ROUTING_DECLINED"
# Vendoring deprecation: detect if CWD has a vendored rezops copy
_VENDORED="no"
if [ -d ".factory/skills/rezops" ] && [ ! -L ".factory/skills/rezops" ]; then
  if [ -f ".factory/skills/rezops/VERSION" ] || [ -d ".factory/skills/rezops/.git" ]; then
    _VENDORED="yes"
  fi
fi
...
_CHECKPOINT_MODE=$($REZOPS_BIN/rezops-config get checkpoint_mode 2>/dev/null || echo "explicit")
_CHECKPOINT_PUSH=$($REZOPS_BIN/rezops-config get checkpoint_push 2>/dev/null || echo "false")
...
`$REZOPS_ROOT/[skill-name]/SKILL.md` for reading skill files.

If output shows `UPGRADE_AVAILABLE <old> <new>`: read `$REZOPS_ROOT/rezops-upgrade/SKILL.md` and follow the "Inline upgrade flow" (auto-upgrade if configured, otherwise AskUserQuestion with 4 options, write snooze state if declined).
...
1. `$REZOPS_ROOT/.feature-prompted-continuous-checkpoint` →
   Prompt: "Continuous checkpoint auto-commits your work as you go with `WIP:` prefix
   so you never lose progress to a crash. Local-only by default — doesn't push
   anywhere unless you turn that on. Want to try it?"
   Options: A) Enable continuous mode, B) Show me first (print the section from
   the preamble Continuous Checkpoint Mode), C) Skip.
   If A: run `$REZOPS_BIN/rezops-config set checkpoint_mode continuous`.
   Always: `touch $REZOPS_ROOT/.feature-prompted-continuous-checkpoint`

2. `$REZOPS_ROOT/.feature-prompted-model-overlay` →
   Inform only (no prompt): "Model overlays are active. `MODEL_OVERLAY: {model}`
   shown in the preamble output tells you which behavioral patch is applied.
   Override with `--model` when regenerating skills (e.g., `bun run gen:skill-docs
   --model gpt-5.4`). Default is claude."
   Always: `touch $REZOPS_ROOT/.feature-prompted-model-overlay`
...
If B: run `$REZOPS_BIN/rezops-config set explain_level terse`.
...
If A: run `$REZOPS_BIN/rezops-config set telemetry community`
...
If B→A: run `$REZOPS_BIN/rezops-config set telemetry anonymous`
If B→B: run `$REZOPS_BIN/rezops-config set telemetry off`
...
If A: run `$REZOPS_BIN/rezops-config set proactive true`
If B: run `$REZOPS_BIN/rezops-config set proactive false`
...
If B: run `$REZOPS_BIN/rezops-config set routing_declined true`
...
`.factory/skills/rezops/`. Vendoring is deprecated. We will not keep vendored copies
...
> This project has rezops vendored in `.factory/skills/rezops/`. Vendoring is deprecated.
...
If A:
1. Run `git rm -r .factory/skills/rezops/`
2. Run `echo '.factory/skills/rezops/' >> .gitignore`
3. Run `$REZOPS_BIN/rezops-team-init required` (or `optional`)
4. Run `git add .claude/ .gitignore CLAUDE.md && git commit -m "chore: migrate rezops from vendored to team mode"`
5. Tell the user: "Done. Each developer now runs: `cd $REZOPS_ROOT && ./setup --team`"
...
You are rezops, an open source AI builder framework shaped by Garry Tan's product, startup, and engineering judgment. Encode how he thinks, not his biography.
...
```bash
eval "$($REZOPS_BIN/rezops-slug 2>/dev/null)"
_PROJ="${REZOPS_HOME:-$HOME/.rezops}/projects/${SLUG:-unknown}"
if [ -d "$_PROJ" ]; then
...
AI makes completeness near-free. Always recommend the complete option over shortcuts — the delta is minutes with CC+rezops. A "lake" (100% coverage, all edge cases) is boilable; an "ocean" (full rewrite, multi-quarter migration) is not. Boil lakes, flag oceans.

**Effort reference** — always show both scales:

| Task type | Human team | CC+rezops | Compression |
|-----------|-----------|-----------|-------------|
...
- Do NOT commit with known-broken tests. Fix first, then commit. The [rezops-context]
...
**When `/context-restore` runs,** it parses `[rezops-context]` blocks from WIP
...
`$REZOPS_BIN/rezops-question-preference --check "<id>"`.
...
```bash
$REZOPS_BIN/rezops-question-log '{"skill":"ship","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
```bash
$REZOPS_BIN/rezops-question-preference --write '{"question_id":"<id>","preference":"<pref>","source":"inline-user","free_text":"<optional original words>"}'
```
...
Before building anything unfamiliar, **search first.** See `$REZOPS_ROOT/ETHOS.md`.
- **Layer 1** (tried and true) — don't reinvent. **Layer 2** (new and popular) — scrutinize. **Layer 3** (first principles) — prize above all.

**Eureka:** When first-principles reasoning contradicts conventional wisdom, name it and log:
```bash
jq -n --arg ts "$(date -u +%Y-%m-%dT%H:%M:%SZ)" --arg skill "SKILL_NAME" --arg branch "$(git branch --show-current 2>/dev/null)" --arg insight "ONE_LINE_SUMMARY" '{ts:$ts,skill:$skill,branch:$branch,insight:$insight}' >> ~/.rezops/analytics/eureka.jsonl 2>/dev/null || true
```
...
```bash
$REZOPS_BIN/rezops-learnings-log '{"skill":"SKILL_NAME","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"observed"}'
```
...
$REZOPS_ROOT/bin/rezops-timeline-log '{"skill":"SKILL_NAME","event":"completed","branch":"'$(git branch --show-current 2>/dev/null || echo unknown)'","outcome":"OUTCOME","duration_s":"'"$_TEL_DUR"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null || true
...
if [ "$_TEL" != "off" ] && [ -x $REZOPS_ROOT/bin/rezops-telemetry-log ]; then
  $REZOPS_ROOT/bin/rezops-telemetry-log \
...
section, run `$REZOPS_ROOT/bin/rezops-review-read` and append a report.
...
```bash
$REZOPS_ROOT/bin/rezops-review-read
```
...
**Review tiers:**
- **Eng Review (required by default):** The only review that gates shipping. Covers architecture, code quality, tests, performance. Can be disabled globally with \`rezops-config set skip_eng_review true\` (the "don't bother me" setting).
...
For Design Review: run `source <($REZOPS_ROOT/bin/rezops-diff-scope <base> 2>/dev/null)`. If `SCOPE_FRONTEND=true` and no design review (plan-design-review or design-review-lite) exists in the dashboard, mention: "Design Review not run — this PR changes frontend code. The lite design check will run automatically in Step 9, but consider running /design-review for a full visual audit post-implementation." Still never block.
...
[ -f .rezops/no-test-bootstrap ] && echo "BOOTSTRAP_DECLINED"
```
...
If user picks H → write `.rezops/no-test-bootstrap` and continue without tests.
...
If user picks C → write `.rezops/no-test-bootstrap`. Tell user: "If you change your mind later, delete `.rezops/no-test-bootstrap` and re-run." Continue without tests.
...
**If "Add as P0 TODO":**
- If `TODOS.md` exists, add the entry following the format in `review/TODOS-format.md` (or `.factory/skills/rezops/review/TODOS-format.md`).
...
- Create an issue assigned to that person (use the platform detected in Step 0):
  - **If GitHub:**
    ...
    gh issue create \
      ...
      --body "Found failing on branch <current-branch>. Failure is pre-existing.\n\n**Error:**\n```\n<first 10 lines>\n```\n\n**Last modified by:** <author>\n**Noticed by:** rezops /ship on <date>" \
...
```bash
eval "$($REZOPS_ROOT/bin/rezops-slug 2>/dev/null)" && mkdir -p ~/.rezops/projects/$SLUG
USER=$(whoami)
DATETIME=$(date +%Y%m%d-%H%M%S)
```

Write to `~/.rezops/projects/{slug}/{user}-{branch}-ship-test-plan-{datetime}.md`:
...
# Compute project slug for ~/.rezops/projects/ lookup
_PLAN_SLUG=$(git remote get-url origin 2>/dev/null | sed 's|.*[:/]\([^/]*/[^/]*\)\.git$|\1|;s|.*[:/]\([^/]*/[^/]*\)$|\1|' | tr '/' '-' | tr -cd 'a-zA-Z0-9._-') || true
_PLAN_SLUG="${_PLAN_SLUG:-$(basename "$PWD" | tr -cd 'a-zA-Z0-9._-')}"
# Search common plan file locations (project designs first, then personal/local)
for PLAN_DIR in "$HOME/.rezops/projects/$_PLAN_SLUG" "$HOME/.claude/plans" "$HOME/.codex/plans" ".rezops/plans"; do
  [ -d "$PLAN_DIR" ] || continue
...
- Review report sections (`## rezops REVIEW REPORT`)
...
```bash
_CROSS_PROJ=$($REZOPS_BIN/rezops-config get cross_project_learnings 2>/dev/null || echo "unset")
echo "CROSS_PROJECT: $_CROSS_PROJ"
if [ "$_CROSS_PROJ" = "true" ]; then
  $REZOPS_BIN/rezops-learnings-search --limit 10 --cross-project 2>/dev/null || true
else
  $REZOPS_BIN/rezops-learnings-search --limit 10 2>/dev/null || true
fi
```

If `CROSS_PROJECT` is `unset` (first time): Use AskUserQuestion:

> rezops can search learnings from your other projects on this machine to find
...
If A: run `$REZOPS_BIN/rezops-config set cross_project_learnings true`
If B: run `$REZOPS_BIN/rezops-config set cross_project_learnings false`
...
This makes the compounding visible. The user should see that rezops is getting
...
1. Read `.factory/skills/rezops/review/checklist.md`. If the file cannot be read, **STOP** and report the error.
...
Check if the diff touches frontend files using `rezops-diff-scope`:

```bash
source <($REZOPS_BIN/rezops-diff-scope <base> 2>/dev/null)
```
...
2. **Read `.factory/skills/rezops/review/design-checklist.md`.** If the file cannot be read, skip design review with a note: "Design checklist not found — skipping design review."
...
```bash
$REZOPS_BIN/rezops-review-log '{"skill":"design-review-lite","timestamp":"TIMESTAMP","status":"STATUS","findings":N,"auto_fixed":M,"commit":"COMMIT"}'
```
...
```bash
source <($REZOPS_BIN/rezops-diff-scope <base> 2>/dev/null) || true
...
```bash
$REZOPS_BIN/rezops-specialist-stats 2>/dev/null || true
```
...
1. **Testing** — read `$REZOPS_ROOT/review/specialists/testing.md`
2. **Maintainability** — read `$REZOPS_ROOT/review/specialists/maintainability.md`
...
3. **Security** — if SCOPE_AUTH=true, OR if SCOPE_BACKEND=true AND DIFF_LINES > 100. Read `$REZOPS_ROOT/review/specialists/security.md`
4. **Performance** — if SCOPE_BACKEND=true OR SCOPE_FRONTEND=true. Read `$REZOPS_ROOT/review/specialists/performance.md`
5. **Data Migration** — if SCOPE_MIGRATIONS=true. Read `$REZOPS_ROOT/review/specialists/data-migration.md`
6. **API Contract** — if SCOPE_API=true. Read `$REZOPS_ROOT/review/specialists/api-contract.md`
7. **Design** — if SCOPE_FRONTEND=true. Use the existing design review checklist at `$REZOPS_ROOT/review/design-checklist.md`

### Adaptive gating

After scope-based selection, apply adaptive gating based on specialist hit rates:

For each conditional specialist that passed scope gating, check the `rezops-specialist-stats` output above:
...
```bash
$REZOPS_BIN/rezops-learnings-search --type pitfall --query "{specialist domain}" --limit 5 2>/dev/null || true
```
...
1. The red-team checklist from `$REZOPS_ROOT/review/specialists/red-team.md`
...
```bash
$REZOPS_ROOT/bin/rezops-review-log '{"skill":"review","timestamp":"TIMESTAMP","status":"STATUS","issues_found":N,"critical":N,"informational":N,"quality_score":SCORE,"specialists":SPECIALISTS_JSON,"findings":FINDINGS_JSON,"commit":"'"$(git rev-parse --short HEAD)"'","via":"ship"}'
```
...
> You are classifying Greptile review comments for a /ship workflow. Read `.factory/skills/rezops/review/greptile-triage.md` and follow the fetch, filter, classify, and **escalation detection** steps. Do NOT fix code, do NOT reply to comments, do NOT commit — report only.
>
...
OLD_CFG=$($REZOPS_ROOT/bin/rezops-config get codex_reviews 2>/dev/null || true)
...
```bash
$REZOPS_ROOT/bin/rezops-review-log '{"skill":"adversarial-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","tier":"always","gate":"GATE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
```bash
$REZOPS_BIN/rezops-learnings-log '{"skill":"ship","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
...
Read `.factory/skills/rezops/review/TODOS-format.md` for the canonical format reference.

**1. Check if TODOS.md exists** in the repository root.

**If TODOS.md does not exist:** Use AskUserQuestion:
- Message: "rezops recommends maintaining a TODOS.md organized by skill/component, then priority (P0 at top through P4, then Completed at bottom). See TODOS-format.md for the full format. Would you like to create one?"
...
```bash
# Export [rezops-context] blocks from all WIP commits on this branch.
# This file becomes input to the CHANGELOG entry and may inform PR body context.
mkdir -p "$(git rev-parse --show-toplevel)/.rezops"
git log <base>..HEAD --grep="^WIP:" --format="%H%n%B%n---END---" > \
  "$(git rev-parse --show-toplevel)/.rezops/wip-context-before-squash.md" 2>/dev/null || true
```
...
> You are executing the /document-release workflow after a code push. Read the full skill file `${HOME}/.factory/skills/rezops/document-release/SKILL.md` and execute its complete workflow end-to-end, including CHANGELOG clobber protection, doc exclusions, risky-change gates, and named staging. Do NOT attempt to edit the PR body — no PR exists yet. Branch: `<branch>`, base: `<base>`.
>
...
Append to `~/.rezops/projects/$SLUG/$BRANCH-reviews.jsonl`:

```bash
echo '{"skill":"ship","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","coverage_pct":COVERAGE_PCT,"plan_items_total":PLAN_TOTAL,"plan_items_done":PLAN_DONE,"verification_result":"VERIFY_RESULT","version":"VERSION","branch":"BRANCH"}' >> ~/.rezops/projects/$SLUG/$BRANCH-reviews.jsonl
```
