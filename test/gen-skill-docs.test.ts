...
const ALL_SKILLS = (() => {
  ...
  if (fs.existsSync(path.join(ROOT, 'SKILL.md.tmpl'))) {
    skills.push({ dir: '.', name: 'root rezops' });
  }
...
describe('gen-skill-docs', () => {
  ...
  test('generated SKILL.md contains operational self-improvement (replaced contributor mode)', () => {
    ...
    expect(content).not.toContain('REZOPS_contributor');
    expect(content).not.toContain('contributor-logs');
    expect(content).toContain('Operational Self-Improvement');
    expect(content).toContain('rezops-learnings-log');
    expect(content).toContain('rezops-learnings-search --limit 3');
  });
  ...
  test('generated SKILL.md contains telemetry line', () => {
    ...
    expect(content).toContain('~/.rezops/analytics');
  });
  ...
  test('preamble .pending-* glob is zsh-safe (uses find, not shell glob)', () => {
    for (const skill of ALL_SKILLS) {
      ...
      expect(content).toContain("find ~/.rezops/analytics -maxdepth 1 -name '.pending-*'");
    }
  });
...
describe('REVIEW_DASHBOARD resolver', () => {
  ...
  for (const skill of REVIEW_SKILLS) {
    test(`review dashboard appears in ${skill} generated file`, () => {
      ...
      expect(content).toContain('rezops-review');
...
describe('PLAN_FILE_REVIEW_REPORT resolver', () => {
  ...
  for (const skill of REVIEW_SKILLS) {
    test(`plan file review report appears in ${skill} generated file`, () => {
      const content = fs.readFileSync(path.join(ROOT, skill, 'SKILL.md'), 'utf-8');
      expect(content).toContain('rezops REVIEW REPORT');
    });
  }
...
describe('Plan status footer in preamble', () => {
  test('preamble contains plan status footer', () => {
    ...
    expect(content).toContain('rezops REVIEW REPORT');
    expect(content).toContain('rezops-review-read');
...
describe('CODEX_SECOND_OPINION resolver', () => {
  const content = fs.readFileSync(path.join(ROOT, 'office-hours', 'SKILL.md'), 'utf-8');
  const codexContent = fs.readFileSync(path.join(ROOT, '.agents', 'skills', 'rezops-office-hours', 'SKILL.md'), 'utf-8');
  ...
  test('Codex host variant does NOT contain the Phase 3.5 resolver output', () => {
    ...
    expect(codexContent).not.toContain('rezops-codex-oh-');
  });
});
...
describe('Codex filesystem boundary', () => {
  ...
  test('codex skill has rabbit-hole detection rule', () => {
    ...
    expect(content).toContain('rezops-update-check');
  ...
  test('autoplan boundary text avoids host-specific paths for cross-host compatibility', () => {
    ...
    // autoplan template uses generic 'skills/rezops' pattern instead of host-specific
    ...
    expect(boundarySection).toContain('skills/rezops');
...
describe('Codex generation (--host codex)', () => {
  ...
  const CODEX_SKILLS = (() => {
    ...
    if (fs.existsSync(path.join(ROOT, 'SKILL.md.tmpl'))) {
      if (!isSymlinkLoop('rezops')) {
        skills.push({ dir: '.', codexName: 'rezops' });
      }
    }
    for (const entry of fs.readdirSync(ROOT, { withFileTypes: true })) {
      if (!entry.isDirectory() || entry.name.startsWith('.') || entry.name === 'node_modules') continue;
      ...
      const codexName = entry.name.startsWith('rezops-') ? entry.name : `rezops-${entry.name}`;
  ...
  test('root rezops bundle has OpenAI metadata for Codex skill browsing', () => {
    ...
    expect(content).toContain('display_name: "rezops"');
    expect(content).toContain('Use $rezops to locate the bundled rezops skills.');
    expect(content).toContain('allow_implicit_invocation: true');
  });

  test('externalSkillName mapping: root is rezops, others are rezops-{dir}', () => {
    // Root → rezops
    expect(fs.existsSync(path.join(AGENTS_DIR, 'rezops', 'SKILL.md'))).toBe(true);
    // Subdirectories → rezops-{dir}
    expect(fs.existsSync(path.join(AGENTS_DIR, 'rezops-review', 'SKILL.md'))).toBe(true);
    expect(fs.existsSync(path.join(AGENTS_DIR, 'rezops-ship', 'SKILL.md'))).toBe(true);
    // rezops-upgrade doesn't double-prefix
    expect(fs.existsSync(path.join(AGENTS_DIR, 'rezops-upgrade', 'SKILL.md'))).toBe(true);
    // No double-prefix: rezops-rezops-upgrade must NOT exist
    expect(fs.existsSync(path.join(AGENTS_DIR, 'rezops-rezops-upgrade', 'SKILL.md'))).toBe(false);
  });
  ...
  test('/codex skill excluded from Codex output', () => {
    expect(fs.existsSync(path.join(AGENTS_DIR, 'rezops-codex', 'SKILL.md'))).toBe(false);
    expect(fs.existsSync(path.join(AGENTS_DIR, 'rezops-codex'))).toBe(false);
  });

  test('Codex review step stripped from Codex-host ship and review', () => {
    const shipContent = fs.readFileSync(path.join(AGENTS_DIR, 'rezops-ship', 'SKILL.md'), 'utf-8');
    expect(shipContent).not.toContain('codex review --base');
    expect(shipContent).not.toContain('CODEX_REVIEWS');

    const reviewContent = fs.readFileSync(path.join(AGENTS_DIR, 'rezops-review', 'SKILL.md'), 'utf-8');
  ...
  test('multiline descriptions preserved in Codex output', () => {
    ...
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'rezops-office-hours', 'SKILL.md'), 'utf-8');
    const fmEnd = content.indexOf('\n---', 4);
    const frontmatter = content.slice(4, fmEnd);
  ...
  test('hook skills have safety prose and no hooks: in frontmatter', () => {
    const HOOK_SKILLS = ['rezops-careful', 'rezops-freeze', 'rezops-guard'];
  ...
  test('Codex preamble resolves runtime assets from repo-local or global rezops roots', () => {
    // Check a skill that has a preamble (review is a good candidate)
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'rezops-review', 'SKILL.md'), 'utf-8');
    expect(content).toContain('REZOPS_ROOT');
    expect(content).toContain('$_ROOT/.agents/skills/rezops');
    expect(content).toContain('$REZOPS_BIN/rezops-config');
    expect(content).toContain('$REZOPS_ROOT/rezops-upgrade/SKILL.md');
    expect(content).not.toContain('~/.codex/skills/rezops/bin/rezops-config get telemetry');
  });

  // ─── Path rewriting regression tests ─────────────────────────

  test('sidecar paths point to .agents/skills/rezops/review/ (not rezops-review/)', () => {
    // Regression: gen-skill-docs rewrote .claude/skills/review → .agents/skills/rezops-review
    // but setup puts sidecars under .agents/skills/rezops/review/. Must match setup layout.
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'rezops-review', 'SKILL.md'), 'utf-8');
    // Correct: references to sidecar files use rezops/review/ path
    expect(content).toContain('.agents/skills/rezops/review/checklist.md');
    // design-checklist.md is now referenced via Review Army specialist (Claude only, stripped for Codex)
    // Wrong: must NOT reference rezops-review/checklist.md (file doesn't exist there)
    expect(content).not.toContain('.agents/skills/rezops-review/checklist.md');
  });

  test('sidecar paths in ship skill point to rezops/review/ for pre-landing review', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'rezops-ship', 'SKILL.md'), 'utf-8');
    // Ship references the review checklist in its pre-landing review step
    if (content.includes('checklist.md')) {
      expect(content).toContain('.agents/skills/rezops/review/');
      expect(content).not.toContain('.agents/skills/rezops-review/checklist');
    }
  });
  ...
  test('greptile-triage sidecar path is correct', () => {
    ...
    if (content.includes('greptile-triage')) {
      expect(content).toContain('.agents/skills/rezops/review/greptile-triage.md');
      expect(content).not.toContain('.agents/skills/rezops-review/greptile-triage');
    }
  });
  ...
  test('all four path rewrite rules produce correct output', () => {
    ...
    // Rule 1: ~/.claude/skills/rezops → $REZOPS_ROOT
    expect(content).not.toContain('~/.claude/skills/rezops');
    expect(content).toContain('$REZOPS_ROOT');

    // Rule 2: .claude/skills/rezops → .agents/skills/rezops
    expect(content).not.toContain('.claude/skills/rezops');

    // Rule 3: .claude/skills/review → .agents/skills/rezops/review
  ...
  test('path rewrite rules apply to all Codex skills with sidecar references', () => {
    ...
    for (const skill of CODEX_SKILLS) {
      ...
      expect(content).not.toContain('~/.claude/skills');
      expect(content).not.toContain('.claude/skills');
      if (content.includes('rezops-config') || content.includes('rezops-update-check') || content.includes('rezops-telemetry-log')) {
        expect(content).toContain('$REZOPS_ROOT');
      }
      ...
      if (content.includes('checklist.md') && !content.includes('design-checklist.md')) {
        expect(content).not.toContain('rezops-review/checklist.md');
      }
    }
  });
  ...
  test('Claude output unchanged: review skill still uses .claude/skills/ paths', () => {
    ...
    expect(content).toContain('~/.claude/skills/rezops');
  ...
  test('Claude output unchanged: all Claude skills have zero Codex paths', () => {
    for (const skill of ALL_SKILLS) {
      ...
      // not the rezops Codex host install path.
      if (skill.dir !== 'pair-agent' && skill.dir !== 'codex' && skill.dir !== 'autoplan') {
        expect(content).not.toContain('~/.codex/');
      }
      // rezops-upgrade legitimately references .agents/skills for cross-platform detection
      if (skill.dir !== 'rezops-upgrade') {
        expect(content).not.toContain('.agents/skills');
      }
    }
  });
  ...
  test('codex host produces empty outside voices in design-review', () => {
    const codexContent = fs.readFileSync(path.join(AGENTS_DIR, 'rezops-design-review', 'SKILL.md'), 'utf-8');
    expect(codexContent).not.toContain('Design Outside Voices');
  });

  test('codex host does not include Codex design block in ship', () => {
    const codexContent = fs.readFileSync(path.join(AGENTS_DIR, 'rezops-ship', 'SKILL.md'), 'utf-8');
...
describe('Factory generation (--host factory)', () => {
  ...
  const FACTORY_SKILLS = (() => {
    ...
    if (fs.existsSync(path.join(ROOT, 'SKILL.md.tmpl'))) {
      if (!isSymlinkLoop('rezops')) skills.push({ dir: '.', factoryName: 'rezops' });
    }
    for (const entry of fs.readdirSync(ROOT, { withFileTypes: true })) {
      if (!entry.isDirectory() || entry.name.startsWith('.') || entry.name === 'node_modules') continue;
      if (entry.name === 'codex') continue;
      if (!fs.existsSync(path.join(ROOT, entry.name, 'SKILL.md.tmpl'))) continue;
      const factoryName = entry.name.startsWith('rezops-') ? entry.name : `rezops-${entry.name}`;
  ...
  test('sensitive skills have disable-model-invocation', () => {
    const SENSITIVE = ['rezops-ship', 'rezops-land-and-deploy', 'rezops-guard', 'rezops-careful', 'rezops-freeze', 'rezops-unfreeze'];
  ...
  test('non-sensitive skills lack disable-model-invocation', () => {
    const NON_SENSITIVE = ['rezops-qa', 'rezops-review', 'rezops-investigate', 'rezops-browse'];
  ...
  test('/codex skill excluded from Factory output', () => {
    expect(fs.existsSync(path.join(FACTORY_DIR, 'rezops-codex', 'SKILL.md'))).toBe(false);
    expect(fs.existsSync(path.join(FACTORY_DIR, 'rezops-codex'))).toBe(false);
  });

  test('Factory keeps Codex integration blocks', () => {
    // Factory users CAN use Codex second opinions (codex exec is a standalone binary)
    const shipContent = fs.readFileSync(path.join(FACTORY_DIR, 'rezops-ship', 'SKILL.md'), 'utf-8');
  ...
  test('Factory preamble uses .factory paths', () => {
    const content = fs.readFileSync(path.join(FACTORY_DIR, 'rezops-review', 'SKILL.md'), 'utf-8');
    expect(content).toContain('REZOPS_ROOT');
    expect(content).toContain('$_ROOT/.factory/skills/rezops');
    expect(content).toContain('$REZOPS_BIN/rezops-config');
  });
});
    ...
    describe(`${hostConfig.displayName} (--host ${hostConfig.name})`, () => {
      ...
      test('no .claude/skills path leakage in non-root skills', () => {
        ...
        for (const skill of skills) {
          // Skip root rezops skill — it contains preamble with intentional .claude/skills
          // fallback paths for binary lookup and skill prefix instructions
          if (skill === 'rezops') continue;
      ...
      if (hostConfig.generation.skipSkills?.includes('codex')) {
        test('/codex skill excluded', () => {
          expect(fs.existsSync(path.join(hostDir, 'rezops-codex', 'SKILL.md'))).toBe(false);
        });
      }
    });
  }
...
describe('setup script validation', () => {
  ...
  test('Codex install uses link_codex_skill_dirs', () => {
    ...
    expect(codexSection).not.toContain('ln -snf "$REZOPS_DIR" "$CODEX_REZOPS"');
  });
  ...
  test('setup separates install path from source path for symlinked repo-local installs', () => {
    expect(setupContent).toContain('INSTALL_REZOPS_DIR=');
    expect(setupContent).toContain('SOURCE_REZOPS_DIR=');
    expect(setupContent).toContain('INSTALL_SKILLS_DIR=');
    expect(setupContent).toContain('CODEX_REZOPS="$INSTALL_REZOPS_DIR"');
    expect(setupContent).toContain('link_codex_skill_dirs "$SOURCE_REZOPS_DIR" "$CODEX_SKILLS"');
  });
  ...
  test('Codex installs always create sidecar runtime assets for the real skill target', () => {
    ...
    expect(setupContent).toContain('create_agents_sidecar "$SOURCE_REZOPS_DIR"');
  });
  ...
  test('link_codex_skill_dirs reads from .agents/skills/', () => {
    ...
    expect(fnBody).toContain('rezops*');
  });
  ...
  test('link_claude_skill_dirs creates real directories with absolute SKILL.md symlinks', () => {
    ...
    // to ensure Claude Code discovers them as top-level skills (not nested under rezops/)
    const fnStart = setupContent.indexOf('link_claude_skill_dirs()');
    const fnEnd = setupContent.indexOf('}', setupContent.indexOf('linked[@]}', fnStart));
    const fnBody = setupContent.slice(fnStart, fnEnd);
    expect(fnBody).toContain('mkdir -p "$target"');
    expect(fnBody).toContain('ln -snf "$REZOPS_dir/$dir_name/SKILL.md" "$target/SKILL.md"');
  });
  ...
  // T1: Sidecar skip guard — prevents .agents/skills/rezops from being linked as a skill
  test('link_codex_skill_dirs skips the rezops sidecar directory', () => {
    const fnStart = setupContent.indexOf('link_codex_skill_dirs()');
    const fnEnd = setupContent.indexOf('}', setupContent.indexOf('done', fnStart));
    const fnBody = setupContent.slice(fnStart, fnEnd);
    expect(fnBody).toContain('[ "$skill_name" = "rezops" ] && continue');
  });

  // T2: Dynamic $REZOPS_ROOT paths in generated Codex preambles
  test('generated Codex preambles use dynamic REZOPS_ROOT paths', () => {
    const codexSkillDir = path.join(ROOT, '.agents', 'skills', 'rezops-ship');
    ...
    expect(content).toContain('REZOPS_ROOT=');
    expect(content).toContain('$REZOPS_BIN/');
  });
  ...
  test('setup supports --host kiro with install section and sed rewrites', () => {
    ...
    expect(setupContent).toContain('~/.kiro/skills/rezops');
  });
  ...
  test('setup supports --host opencode with install section and OpenCode skill path vars', () => {
    ...
    expect(setupContent).toContain('OPENCODE_REZOPS="$OPENCODE_SKILLS/rezops"');
  });

  test('setup installs OpenCode skills into a nested rezops runtime root', () => {
  ...
  test('create_codex_runtime_root exposes only runtime assets', () => {
    ...
    const fnBody = setupContent.slice(fnStart, fnEnd);
    expect(fnBody).toContain('rezops/SKILL.md');
    expect(fnBody).toContain('browse/dist');
    expect(fnBody).toContain('browse/bin');
    expect(fnBody).toContain('rezops-upgrade/SKILL.md');
    ...
    expect(fnBody).not.toContain('ln -snf "$REZOPS_dir" "$codex_REZOPS"');
  });

  test('direct Codex installs are migrated out of ~/.codex/skills/rezops', () => {
    expect(setupContent).toContain('migrate_direct_codex_install');
    expect(setupContent).toContain('$HOME/.rezops/repos/rezops');
    expect(setupContent).toContain('avoid duplicate skill discovery');
  });

  // --- Symlink prefix tests (PR #503) ---

  test('link_claude_skill_dirs applies rezops- prefix by default', () => {
    const fnStart = setupContent.indexOf('link_claude_skill_dirs()');
    const fnEnd = setupContent.indexOf('}', setupContent.indexOf('linked[@]}', fnStart));
    const fnBody = setupContent.slice(fnStart, fnEnd);
    expect(fnBody).toContain('SKILL_PREFIX');
    expect(fnBody).toContain('link_name="rezops-$skill_name"');
  });
  ...
  test('link_claude_skill_dirs preserves already-prefixed dirs', () => {
    ...
    const fnEnd = setupContent.indexOf('}', setupContent.indexOf('linked[@]}', fnStart));
    const fnBody = setupContent.slice(fnStart, fnEnd);
    // rezops-* dirs should keep their name (e.g., rezops-upgrade stays rezops-upgrade)
    expect(fnBody).toContain('rezops-*) link_name="$skill_name"');
  });
  ...
  test('cleanup_old_claude_symlinks removes only rezops-pointing symlinks', () => {
    ...
    expect(fnBody).toContain('rezops-*) continue');
  });
  ...
  test('setup reads skill_prefix from config', () => {
    ...
    expect(setupContent).toContain('REZOPS_CONFIG');
  });
  ...
  test('cleanup_prefixed_claude_symlinks exists and uses readlink', () => {
    ...
    const fnBody = setupContent.slice(fnStart, fnEnd);
    expect(fnBody).toContain('readlink');
    expect(fnBody).toContain('rezops-$skill_name');
  });
  ...
  test('welcome message references SKILL_PREFIX', () => {
    // rezops-upgrade is always called rezops-upgrade (it's the actual dir name)
    // but the welcome section should exist near the prefix logic
    expect(setupContent).toContain('Run /rezops-upgrade anytime');
  });
});

describe('discover-skills hidden directory filtering', () => {
  test('discoverTemplates skips dot-prefixed directories', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'rezops-discover-'));
    try {
...
describe('telemetry', () => {
  test('generated SKILL.md contains telemetry start block', () => {
    ...
    expect(content).toContain('rezops-config get telemetry');
  });

  test('generated SKILL.md contains telemetry opt-in prompt', () => {
    const content = fs.readFileSync(path.join(ROOT, 'SKILL.md'), 'utf-8');
    expect(content).toContain('.telemetry-prompted');
    expect(content).toContain('Help rezops get better');
    expect(content).toContain('rezops-config set telemetry community');
    expect(content).toContain('rezops-config set telemetry anonymous');
    expect(content).toContain('rezops-config set telemetry off');
  });

  test('generated SKILL.md contains telemetry epilogue', () => {
    const content = fs.readFileSync(path.join(ROOT, 'SKILL.md'), 'utf-8');
    expect(content).toContain('Telemetry (run last)');
    expect(content).toContain('rezops-telemetry-log');
...
describe('community fixes wave', () => {
  ...
  // #594 — Discoverability: every SKILL.md.tmpl description contains "rezops"
  test('every SKILL.md.tmpl description contains "rezops"', () => {
    for (const skill of ALL_SKILLS) {
      const tmplPath = skill.dir === '.' ? path.join(ROOT, 'SKILL.md.tmpl') : path.join(ROOT, skill.dir, 'SKILL.md.tmpl');
      const content = fs.readFileSync(tmplPath, 'utf-8');
      const desc = extractDescription(content);
      expect(desc.toLowerCase()).toContain('rezops');
    }
  });
...
describe('codex commands must not use inline $(git rev-parse --show-toplevel) for cwd', () => {
  ...
  // Use Bun.Glob to avoid ELOOP from .claude/skills/rezops symlink back to ROOT
...
describe('LEARNINGS_SEARCH resolver', () => {
  ...
  for (const skill of SEARCH_SKILLS) {
    test(`${skill} generated SKILL.md contains learnings search`, () => {
      ...
      expect(content).toContain('rezops-learnings-search');
    });
  }
...
describe('gen-skill-docs prefix warning (#620/#578)', () => {
  ...
  test('warns about skill_prefix when config has prefix=true', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'rezops-prefix-warn-'));
    try {
      // Create a fake ~/.rezops/config.yaml with skill_prefix: true
      const fakeHome = tmpDir;
      const fakeREZOPS = path.join(fakeHome, '.rezops');
      fs.mkdirSync(fakeREZOPS, { recursive: true });
      fs.writeFileSync(path.join(fakeREZOPS, 'config.yaml'), 'skill_prefix: true\n');
      ...
      expect(output).toContain('rezops-relink');
    } finally {
      fs.rmSync(tmpDir, { recursive: true, force: true });
    }
  });
  ...
  test('no warning when skill_prefix is false or absent', () => {
    ...
    try {
      ...
      const fakeREZOPS = path.join(fakeHome, '.rezops');
      fs.mkdirSync(fakeREZOPS, { recursive: true });
      fs.writeFileSync(path.join(fakeREZOPS, 'config.yaml'), 'skill_prefix: false\n');
