...
const scriptPath = join(import.meta.dir, "..", "bin", "rezops-global-discover.ts");

describe("rezops-global-discover", () => {
  describe("normalizeRemoteUrl", () => {
    // Dynamically import to test the exported function
    let normalizeRemoteUrl: (url: string) => string;

    beforeEach(async () => {
      const mod = await import("../bin/rezops-global-discover.ts");
    ...
    test("SSH and HTTPS for same repo normalize to same URL", () => {
      const ssh = normalizeRemoteUrl("git@github.com:garrytan/rezops.git");
      const https = normalizeRemoteUrl("https://github.com/diiviikk5/rezops.git");
      const httpsNoDotGit = normalizeRemoteUrl("https://github.com/diiviikk5/rezops");
  ...
  describe("codex large session_meta parsing", () => {
    ...
    beforeEach(() => {
      tmpDir = mkdtempSync(join(tmpdir(), "rezops-codex-test-"));
