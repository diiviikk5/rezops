/**
 * bin/rezops-developer-profile — subcommand behavior tests.
 *
 * Covers:
...
const BIN_DEV = path.join(ROOT, 'bin', 'rezops-developer-profile');
const BIN_LOG = path.join(ROOT, 'bin', 'rezops-question-log');

let tmpHome: string;

beforeEach(() => {
  tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'rezops-test-'));
});
...
function runDev(...args: string[]): { stdout: string; stderr: string; status: number } {
  const res = spawnSync(BIN_DEV, args, {
    env: { ...process.env, REZOPS_HOME: tmpHome },
    encoding: 'utf-8',
    cwd: ROOT,
  });
  return {
...
describe('rezops-developer-profile --read', () => {
...
describe('rezops-developer-profile --migrate', () => {
...
describe('rezops-developer-profile tier calculation', () => {
...
describe('rezops-developer-profile --derive', () => {
...
describe('rezops-developer-profile --trace <dim>', () => {
...
describe('rezops-developer-profile --gap', () => {
...
describe('rezops-developer-profile --vibe', () => {
...
describe('rezops-developer-profile --check-mismatch', () => {
...
describe('rezops-developer-profile errors', () => {
