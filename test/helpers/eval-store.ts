 ...
 * ~/.rezops/projects/$SLUG/evals/{version}-{branch}-{tier}-{timestamp}.json,
...
const LEGACY_EVAL_DIR = path.join(os.homedir(), '.rezops-dev', 'evals');

/**
 * Detect project-scoped eval dir via rezops-slug.
 * Falls back to legacy ~/.rezops-dev/evals/ if slug detection fails.
 */
export function getProjectEvalDir(): string {
  try {
    // Try repo-local rezops-slug first, then global install
    const localSlug = spawnSync('bash', ['-c', '.claude/skills/rezops/bin/rezops-slug 2>/dev/null || ~/.claude/skills/rezops/bin/rezops-slug 2>/dev/null'], {
    ...
    if (output) {
      ...
      if (slugMatch && slugMatch[1]) {
        const dir = path.join(os.homedir(), '.rezops', 'projects', slugMatch[1], 'evals');
