...
const REZOPS_DEV_DIR = path.join(os.homedir(), '.rezops-dev');
const HEARTBEAT_PATH = path.join(REZOPS_DEV_DIR, 'e2e-live.json'); // heartbeat stays global
const PROJECT_DIR = path.dirname(getProjectEvalDir()); // ~/.rezops/projects/$SLUG/
...
export async function runSkillTest(options: {
   ...
   *  per-test REZOPS_HOME overrides so the test doesn't have to spell out
  ...
  if (browseErrors.length > 0 || exitReason !== 'success') {
    try {
      const failureDir = runDir || path.join(workingDirectory, '.rezops', 'test-transcripts');
