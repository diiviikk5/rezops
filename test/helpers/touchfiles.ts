...
export const E2E_TOUCHFILES: Record<string, string[]> = {
  ...
  'operational-learning':     ['scripts/resolvers/preamble.ts', 'bin/rezops-learnings-log'],
  ...
  'review-army-migration-safety': ['review/**', 'scripts/resolvers/review-army.ts', 'bin/rezops-diff-scope'],
  'review-army-perf-n-plus-one':  ['review/**', 'scripts/resolvers/review-army.ts', 'bin/rezops-diff-scope'],
  ...
  'plan-tune-inspect':         ['plan-tune/**', 'scripts/question-registry.ts', 'scripts/psychographic-signals.ts', 'scripts/one-way-doors.ts', 'bin/rezops-question-log', 'bin/rezops-question-preference', 'bin/rezops-developer-profile'],
  ...
  // Ship
  'ship-base-branch': ['ship/**', 'bin/rezops-repo-mode'],
  ...
  // Global discover
  'global-discover':   ['bin/rezops-global-discover.ts', 'test/global-discover.test.ts'],
  ...
  // Learnings
  'learnings-show': ['learn/**', 'bin/rezops-learnings-search', 'bin/rezops-learnings-log', 'scripts/resolvers/learnings.ts'],

  // Session Intelligence (timeline, context recovery, /context-save + /context-restore)
  'timeline-event-flow':            ['bin/rezops-timeline-log', 'bin/rezops-timeline-read'],
  'context-recovery-artifacts':     ['scripts/resolvers/preamble.ts', 'bin/rezops-timeline-log', 'bin/rezops-slug', 'learn/**'],
  'context-save-writes-file':       ['context-save/**', 'bin/rezops-slug'],
  'context-restore-loads-latest':   ['context-restore/**', 'bin/rezops-slug'],
  ...
  'context-save-then-restore-roundtrip':   ['context-save/**', 'context-restore/**', 'bin/rezops-slug'],
  ...
  'codex-review-findings': ['review/**', '.agents/skills/rezops-review/**', 'codex/**', 'test/helpers/codex-session-runner.ts', 'lib/worktree.ts'],
  ...
  'ship-coverage-audit': ['ship/**', 'test/fixtures/coverage-audit-fixture.ts', 'bin/rezops-repo-mode'],
  ...
  'ship-triage': ['ship/**', 'bin/rezops-repo-mode'],
  ...
  // rezops-upgrade
  'rezops-upgrade-happy-path': ['rezops-upgrade/**'],

  // Deploy skills
  'land-and-deploy-workflow':      ['land-and-deploy/**', 'scripts/gen-skill-docs.ts'],
  'land-and-deploy-first-run':     ['land-and-deploy/**', 'scripts/gen-skill-docs.ts', 'bin/rezops-slug'],
  'land-and-deploy-review-gate':   ['land-and-deploy/**', 'bin/rezops-review-read'],
  ...
  'autoplan-dual-voice': ['autoplan/**', 'codex/**', 'bin/rezops-codex-probe', 'scripts/resolvers/review.ts', 'scripts/resolvers/design.ts'],

  // Multi-provider benchmark adapters — live API smoke against real claude/codex/gemini CLIs
  'benchmark-providers-live': ['bin/rezops-model-benchmark', 'test/helpers/providers/**', 'test/helpers/benchmark-runner.ts', 'test/helpers/pricing.ts'],
...
export const E2E_TIERS: Record<string, 'gate' | 'periodic'> = {
  ...
  // rezops-upgrade
  'rezops-upgrade-happy-path': 'gate',
...
export const LLM_JUDGE_TOUCHFILES: Record<string, string[]> = {
  ...
  'rezops-upgrade/SKILL.md upgrade flow': ['rezops-upgrade/SKILL.md', 'rezops-upgrade/SKILL.md.tmpl'],
