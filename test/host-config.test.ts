...
describe('validateHostConfig', () => {
  function makeValid(): HostConfig {
    return {
      ...
      cliCommand: 'testcli',
      globalRoot: '.test/skills/rezops',
      localSkillRoot: '.test/skills/rezops',
      hostSubdir: '.test',
      usesEnvVars: true,
  ...
  test('paths with $ and ~ are valid', () => {
    const c = makeValid();
    c.globalRoot = '$HOME/.test/skills/rezops';
    c.localSkillRoot = '~/.test/skills/rezops';
    expect(validateHostConfig(c)).toEqual([]);
  });
...
describe('validateAllConfigs', () => {
  ...
  test('duplicate hostSubdir detected', () => {
    const dup = { ...codex, name: 'dup-host', hostSubdir: '.claude', globalRoot: '.dup/skills/rezops' } as HostConfig;
    const errors = validateAllConfigs([claude, dup]);
    expect(errors.some(e => e.includes('Duplicate hostSubdir'))).toBe(true);
  });

  test('duplicate globalRoot detected', () => {
    const dup = { ...codex, name: 'dup-host', hostSubdir: '.dup', globalRoot: '.claude/skills/rezops' } as HostConfig;
    const errors = validateAllConfigs([claude, dup]);
...
describe('HOST_PATHS derivation from configs', () => {
  test('Claude uses literal home paths (no env vars)', () => {
    expect(HOST_PATHS.claude.skillRoot).toBe('~/.claude/skills/rezops');
    expect(HOST_PATHS.claude.binDir).toBe('~/.claude/skills/rezops/bin');
    expect(HOST_PATHS.claude.browseDir).toBe('~/.claude/skills/rezops/browse/dist');
    expect(HOST_PATHS.claude.designDir).toBe('~/.claude/skills/rezops/design/dist');
  });

  test('Codex uses $REZOPS_ROOT env vars', () => {
    expect(HOST_PATHS.codex.skillRoot).toBe('$REZOPS_ROOT');
    expect(HOST_PATHS.codex.binDir).toBe('$REZOPS_BIN');
    expect(HOST_PATHS.codex.browseDir).toBe('$REZOPS_BROWSE');
    expect(HOST_PATHS.codex.designDir).toBe('$REZOPS_DESIGN');
  });

  test('every host with usesEnvVars=true gets env var paths', () => {
    for (const config of ALL_HOST_CONFIGS) {
      if (config.usesEnvVars) {
        expect(HOST_PATHS[config.name].skillRoot).toBe('$REZOPS_ROOT');
        expect(HOST_PATHS[config.name].binDir).toBe('$REZOPS_BIN');
      }
    }
  });
...
describe('host-config-export.ts CLI', () => {
  ...
  test('get returns string field', () => {
    ...
    expect(exitCode).toBe(0);
    expect(stdout).toBe('.codex/skills/rezops');
  });
...
describe('golden-file regression', () => {
  ...
  test('Codex ship skill matches golden baseline', () => {
    ...
    const current = fs.readFileSync(path.join(ROOT, '.agents', 'skills', 'rezops-ship', 'SKILL.md'), 'utf-8');
    expect(current).toBe(golden);
  });
  ...
  test('Factory ship skill matches golden baseline', () => {
    ...
    const current = fs.readFileSync(path.join(ROOT, '.factory', 'skills', 'rezops-ship', 'SKILL.md'), 'utf-8');
    expect(current).toBe(golden);
  });
});
...
describe('host config correctness', () => {
  ...
  test('codex has sidecar config', () => {
    ...
    expect(codex.sidecar!.path).toBe('.agents/skills/rezops');
  });
