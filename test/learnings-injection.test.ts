...
const SCRIPT = path.join(import.meta.dir, "..", "bin", "rezops-learnings-search");

describe("rezops-learnings-search injection prevention", () => {
  ...
  test("uses process.env for all user-controlled values", () => {
    ...
    expect(bunBlock).toContain("process.env.REZOPS_SEARCH_TYPE");
    expect(bunBlock).toContain("process.env.REZOPS_SEARCH_QUERY");
    expect(bunBlock).toContain("process.env.REZOPS_SEARCH_LIMIT");
    expect(bunBlock).toContain("process.env.REZOPS_SEARCH_SLUG");
    expect(bunBlock).toContain("process.env.REZOPS_SEARCH_CROSS");
  });

  test("env vars are set on the bun command line", () => {
    // The env vars must be passed to bun, not just set in the shell
    expect(script).toContain("REZOPS_SEARCH_TYPE=");
    expect(script).toContain("REZOPS_SEARCH_QUERY=");
    expect(script).toContain("REZOPS_SEARCH_LIMIT=");
    expect(script).toContain("REZOPS_SEARCH_SLUG=");
    expect(script).toContain("REZOPS_SEARCH_CROSS=");
  });
});
