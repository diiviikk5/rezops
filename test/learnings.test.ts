...
function runLog(input: string, opts: { expectFail?: boolean } = {}): { stdout: string; exitCode: number } {
  const execOpts: ExecSyncOptionsWithStringEncoding = {
    cwd: ROOT,
    env: { ...process.env, REZOPS_HOME: tmpDir },
    encoding: 'utf-8',
    timeout: 15000,
  };
  try {
    const stdout = execSync(`${BIN}/rezops-learnings-log '${input.replace(/'/g, "'\\''")}'`, execOpts).trim();
...
function runSearch(args: string = ''): string {
  ...
  try {
    return execSync(`${BIN}/rezops-learnings-search ${args}`, execOpts).trim();
  } catch {
    return '';
  }
}
...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'rezops-learn-'));
...
describe('rezops-learnings-search', () => {
...
describe('rezops-learnings-log edge cases', () => {
...
describe('rezops-learnings-search edge cases', () => {
