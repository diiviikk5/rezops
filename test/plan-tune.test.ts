...
describe('preamble — QUESTION_TUNING injection', () => {
  test('tier 2+ skills include the Question Tuning section', async () => {
    ...
    const ctx = {
      ...
      paths: {
        skillRoot: '~/.claude/skills/rezops',
        localSkillRoot: '.claude/skills/rezops',
        binDir: '~/.claude/skills/rezops/bin',
        browseDir: '~/.claude/skills/rezops/browse/dist',
        designDir: '~/.claude/skills/rezops/design/dist',
      },
      preambleTier: 2,
    };
    ...
    expect(out).toContain('rezops-question-preference --check');
    expect(out).toContain('rezops-question-log');
  ...
  test('codex host produces different paths', async () => {
    ...
    const codexCtx = {
      ...
      paths: {
        ...
        localSkillRoot: '.agents/skills/rezops',
        binDir: '$REZOPS_BIN',
    ...
    expect(out).toContain('$REZOPS_BIN/rezops-question-preference');
    expect(out).toContain('$REZOPS_BIN/rezops-question-log');
  });
});
...
describe('end-to-end pipeline (binaries working together)', () => {
  test('log many expand choices → derive pushes scope_appetite up', () => {
    const tmpHome = fs.mkdtempSync(path.join(require('os').tmpdir(), 'rezops-e2e-'));
    try {
      const env = { ...process.env, REZOPS_HOME: tmpHome };
      const { spawnSync } = require('child_process');
      const logBin = path.join(ROOT, 'bin', 'rezops-question-log');
      const devBin = path.join(ROOT, 'bin', 'rezops-developer-profile');
  ...
  test('preference blocks tune: write from inline-tool-output in full pipeline', () => {
    ...
    try {
      const env = { ...process.env, REZOPS_HOME: tmpHome };
      const { spawnSync } = require('child_process');
      const prefBin = path.join(ROOT, 'bin', 'rezops-question-preference');
  ...
  test('migration preserves sessions, builder-profile shim still works', () => {
    ...
    try {
      ...
      const { spawnSync } = require('child_process');
      const devBin = path.join(ROOT, 'bin', 'rezops-developer-profile');
      const shimBin = path.join(ROOT, 'bin', 'rezops-builder-profile');
