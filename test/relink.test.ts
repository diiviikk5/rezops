...
// Every test in this file shells out to rezops-config + rezops-relink (bash scripts
...
function run(cmd: string, env: Record<string, string> = {}, expectFail = false): string {
  try {
    return execSync(cmd, {
      cwd: ROOT,
      env: { ...process.env, REZOPS_STATE_DIR: tmpDir, ...env },
      encoding: 'utf-8',
      timeout: 10000,
...
// Create a mock rezops install directory with skill subdirs
function setupMockInstall(skills: string[]): void {
  installDir = path.join(tmpDir, 'rezops-install');
  skillsDir = path.join(tmpDir, 'skills');
  fs.mkdirSync(installDir, { recursive: true });
  fs.mkdirSync(skillsDir, { recursive: true });

  // Copy the real rezops-config and rezops-relink to the mock install
  const mockBin = path.join(installDir, 'bin');
  fs.mkdirSync(mockBin, { recursive: true });
  fs.copyFileSync(path.join(BIN, 'rezops-config'), path.join(mockBin, 'rezops-config'));
  fs.chmodSync(path.join(mockBin, 'rezops-config'), 0o755);
  if (fs.existsSync(path.join(BIN, 'rezops-relink'))) {
    fs.copyFileSync(path.join(BIN, 'rezops-relink'), path.join(mockBin, 'rezops-relink'));
    fs.chmodSync(path.join(mockBin, 'rezops-relink'), 0o755);
  }
  if (fs.existsSync(path.join(BIN, 'rezops-patch-names'))) {
    fs.copyFileSync(path.join(BIN, 'rezops-patch-names'), path.join(mockBin, 'rezops-patch-names'));
    fs.chmodSync(path.join(mockBin, 'rezops-patch-names'), 0o755);
  }
...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'rezops-relink-test-'));
});
...
describe('rezops-relink (#578)', () => {
  // Test 11: prefixed symlinks when skill_prefix=true
  test('creates rezops-* symlinks when skill_prefix=true', () => {
    setupMockInstall(['qa', 'ship', 'review']);
    // Set config to prefix mode (pass install/skills env so auto-relink uses mock install)
    run(`${path.join(installDir, 'bin', 'rezops-config')} set skill_prefix true`, {
      REZOPS_INSTALL_DIR: installDir,
      REZOPS_SKILLS_DIR: skillsDir,
    });
    // Run relink with env pointing to the mock install
    const output = run(`${path.join(installDir, 'bin', 'rezops-relink')}`, {
      REZOPS_INSTALL_DIR: installDir,
      REZOPS_SKILLS_DIR: skillsDir,
    });
    // Verify rezops-* symlinks exist
    expect(fs.existsSync(path.join(skillsDir, 'rezops-qa'))).toBe(true);
    expect(fs.existsSync(path.join(skillsDir, 'rezops-ship'))).toBe(true);
    expect(fs.existsSync(path.join(skillsDir, 'rezops-review'))).toBe(true);
    expect(output).toContain('rezops-');
  });

  // Test 12: flat symlinks when skill_prefix=false
  test('creates flat symlinks when skill_prefix=false', () => {
    setupMockInstall(['qa', 'ship', 'review']);
    run(`${path.join(installDir, 'bin', 'rezops-config')} set skill_prefix false`, {
      REZOPS_INSTALL_DIR: installDir,
      REZOPS_SKILLS_DIR: skillsDir,
    });
    const output = run(`${path.join(installDir, 'bin', 'rezops-relink')}`, {
  ...
  // e.g., `qa -> rezops/qa` gets discovered as "rezops-qa", not "qa".
  ...
  test('unprefixed skills are real directories with SKILL.md symlinks, not dir symlinks', () => {
    ...
    run(`${path.join(installDir, 'bin', 'rezops-relink')}`, {
      REZOPS_INSTALL_DIR: installDir,
      REZOPS_SKILLS_DIR: skillsDir,
    });
  ...
  test('prefixed skills are real directories with SKILL.md symlinks, not dir symlinks', () => {
    ...
    for (const skill of ['rezops-qa', 'rezops-ship']) {
      const skillPath = path.join(skillsDir, skill);
      const skillMdPath = path.join(skillPath, 'SKILL.md');
      expect(fs.lstatSync(skillPath).isDirectory()).toBe(true);
      expect(fs.lstatSync(skillPath).isSymbolicLink()).toBe(false);
  ...
  // FIRST INSTALL: --no-prefix must create ONLY flat names, zero rezops-* pollution
  test('first install --no-prefix: only flat names exist, zero rezops-* entries', () => {
    setupMockInstall(['qa', 'ship', 'review', 'plan-ceo-review', 'rezops-upgrade']);
    ...
    // Expected: qa, ship, review, plan-ceo-review, rezops-upgrade (its real name)
    expect(entries.sort()).toEqual(['rezops-upgrade', 'plan-ceo-review', 'qa', 'review', 'ship']);
    // No rezops-qa, rezops-ship, rezops-review, rezops-plan-ceo-review
    const leaked = entries.filter(e => e.startsWith('rezops-') && e !== 'rezops-upgrade');
    expect(leaked).toEqual([]);
  });

  // FIRST INSTALL: --prefix must create ONLY rezops-* names, zero flat-name pollution
  test('first install --prefix: only rezops-* entries exist, zero flat names', () => {
    setupMockInstall(['qa', 'ship', 'review', 'plan-ceo-review', 'rezops-upgrade']);
    run(`${path.join(installDir, 'bin', 'rezops-config')} set skill_prefix true`, {
      REZOPS_INSTALL_DIR: installDir,
      REZOPS_SKILLS_DIR: skillsDir,
    ...
    const entries = fs.readdirSync(skillsDir);
    // Expected: rezops-qa, rezops-ship, rezops-review, rezops-plan-ceo-review, rezops-upgrade
    expect(entries.sort()).toEqual([
      'rezops-plan-ceo-review', 'rezops-qa', 'rezops-review', 'rezops-ship', 'rezops-upgrade',
    ]);
    // No unprefixed qa, ship, review, plan-ceo-review
    const leaked = entries.filter(e => !e.startsWith('rezops-'));
    expect(leaked).toEqual([]);
  });
  ...
  test('non-TTY first install defaults to flat names via relink', () => {
    ...
    // rezops-relink reads config; on fresh install config returns empty → defaults to false
    run(`${path.join(installDir, 'bin', 'rezops-relink')}`, {
      REZOPS_INSTALL_DIR: installDir,
      REZOPS_SKILLS_DIR: skillsDir,
    });
  ...
  // SWITCH: prefix → no-prefix must clean up ALL rezops-* entries
  test('switching prefix to no-prefix removes all rezops-* entries completely', () => {
    setupMockInstall(['qa', 'ship', 'review', 'plan-ceo-review', 'rezops-upgrade']);
    ...
    expect(entries.filter(e => !e.startsWith('rezops-'))).toEqual([]);
    ...
    // Only flat names + rezops-upgrade (its real name)
    expect(entries.sort()).toEqual(['rezops-upgrade', 'plan-ceo-review', 'qa', 'review', 'ship']);
    const leaked = entries.filter(e => e.startsWith('rezops-') && e !== 'rezops-upgrade');
    expect(leaked).toEqual([]);
  });
  ...
  test('switching no-prefix to prefix removes all flat entries completely', () => {
    setupMockInstall(['qa', 'ship', 'review', 'rezops-upgrade']);
    ...
    let entries = fs.readdirSync(skillsDir);
    expect(entries.filter(e => e.startsWith('rezops-') && e !== 'rezops-upgrade')).toEqual([]);
    ...
    // Only rezops-* names
    expect(entries.sort()).toEqual([
      'rezops-qa', 'rezops-review', 'rezops-ship', 'rezops-upgrade',
    ]);
    const leaked = entries.filter(e => !e.startsWith('rezops-'));
    expect(leaked).toEqual([]);
  });
  ...
  test('cleans up stale symlinks from opposite mode', () => {
    ...
    expect(fs.existsSync(path.join(skillsDir, 'qa'))).toBe(true);
    expect(fs.existsSync(path.join(skillsDir, 'rezops-qa'))).toBe(false);
  });

  // Test 14: error when install dir missing
  test('prints error when install dir missing', () => {
    const output = run(`${BIN}/rezops-relink`, {
      REZOPS_INSTALL_DIR: '/nonexistent/path/rezops',
      REZOPS_SKILLS_DIR: '/nonexistent/path/skills',
    }, true);
    expect(output).toContain('setup');
  });

  // Test: rezops-upgrade does NOT get double-prefixed
  test('does not double-prefix rezops-upgrade directory', () => {
    setupMockInstall(['qa', 'ship', 'rezops-upgrade']);
    run(`${path.join(installDir, 'bin', 'rezops-config')} set skill_prefix true`, {
      REZOPS_INSTALL_DIR: installDir,
      REZOPS_SKILLS_DIR: skillsDir,
    });
    ...
    // rezops-upgrade should keep its name, NOT become rezops-rezops-upgrade
    expect(fs.existsSync(path.join(skillsDir, 'rezops-upgrade'))).toBe(true);
    expect(fs.existsSync(path.join(skillsDir, 'rezops-rezops-upgrade'))).toBe(false);
    // Regular skills still get prefixed
    expect(fs.existsSync(path.join(skillsDir, 'rezops-qa'))).toBe(true);
  });

  // Test 15: rezops-config set skill_prefix triggers relink
  test('rezops-config set skill_prefix triggers relink', () => {
    setupMockInstall(['qa', 'ship']);
    // Run rezops-config set which should auto-trigger relink
    run(`${path.join(installDir, 'bin', 'rezops-config')} set skill_prefix true`, {
      REZOPS_INSTALL_DIR: installDir,
      REZOPS_SKILLS_DIR: skillsDir,
    });
...
describe('upgrade migrations', () => {
  const MIGRATIONS_DIR = path.join(ROOT, 'rezops-upgrade', 'migrations');
  ...
  test('v0.15.2.0 migration runs rezops-relink', () => {
    const content = fs.readFileSync(path.join(MIGRATIONS_DIR, 'v0.15.2.0.sh'), 'utf-8');
    expect(content).toContain('rezops-relink');
  });
  ...
  test('v0.15.2.0 migration fixes stale directory symlinks', () => {
    ...
    run(`${path.join(installDir, 'bin', 'rezops-config')} set skill_prefix false`, {
      REZOPS_SETUP_RUNNING: '1',
    });
    // Verify old state: symlinks
    expect(fs.lstatSync(path.join(skillsDir, 'qa')).isSymbolicLink()).toBe(true);

    // Run the migration (it calls rezops-relink internally)
...
describe('rezops-patch-names (#620/#578)', () => {
  ...
  test('prefix=true patches name: field in SKILL.md', () => {
    ...
    // Verify name: field is patched with rezops- prefix
    expect(readSkillName(path.join(installDir, 'qa'))).toBe('rezops-qa');
    expect(readSkillName(path.join(installDir, 'ship'))).toBe('rezops-ship');
    expect(readSkillName(path.join(installDir, 'review'))).toBe('rezops-review');
  });
  ...
  test('rezops-upgrade name: not double-prefixed', () => {
    setupMockInstall(['qa', 'rezops-upgrade']);
    run(`${path.join(installDir, 'bin', 'rezops-config')} set skill_prefix true`, {
      REZOPS_INSTALL_DIR: installDir,
      REZOPS_SKILLS_DIR: skillsDir,
    });
    ...
    // rezops-upgrade should keep its name, NOT become rezops-rezops-upgrade
    expect(readSkillName(path.join(installDir, 'rezops-upgrade'))).toBe('rezops-upgrade');
