 ...
 * for /rewind, silently shadowing the rezops /checkpoint skill. Users
 ...
 *
 * This file is the "never again" test. It enumerates every rezops skill name
 * from every SKILL.md.tmpl file in the repo and cross-checks against a
 * per-host list of known built-in slash commands. If any rezops skill name
 ...
 * below. If a rezops skill needs to coexist with a built-in anyway (e.g.,
...
const KNOWN_BUILTINS: Record<string, string[]> = {
  ...
  // invoke codex from rezops, we don't install skills INTO codex the same
...
describe('skill-collision-sentinel', () => {
  ...
  test('no duplicate skill names within rezops', () => {
  ...
  // Hard check: no rezops skill name collides with a known host built-in
    ...
    test(`no skill name collides with a ${host} built-in (or has written justification)`, () => {
      ...
      if (collisions.length > 0) {
        const msg = collisions.map(c =>
          ...
          `    Fix: rename the rezops skill (precedent: /checkpoint → /context-save+/context-restore),\n` +
  ...
  test('KNOWN_COLLISIONS_TOLERATED entries are all still active collisions', () => {
    ...
    for (const name of Object.keys(KNOWN_COLLISIONS_TOLERATED)) {
      if (!skillNames.has(name)) {
        stale.push(`  "${name}" is in KNOWN_COLLISIONS_TOLERATED but no rezops skill has that name — remove the exception`);
