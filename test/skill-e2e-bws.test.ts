...
Report the exact output — either "READY: <path>" or "NEEDS_SETUP".`,
  ...
  testConcurrentIfSelected('operational-learning', async () => {
    ...
    const REZOPSHome = path.join(opDir, '.rezops-home');
    ...
    for (const script of ['rezops-learnings-log', 'rezops-slug']) {
      fs.copyFileSync(path.join(ROOT, 'bin', script), path.join(binDir, script));
      fs.chmodSync(path.join(binDir, script), 0o755);
    }

    // rezops-learnings-log will create the project dir automatically via rezops-slug
...
\`\`\`bash
REZOPS_HOME="${REZOPSHome}" ${binDir}/rezops-learnings-log '{"skill":"qa","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"observed"}'
\`\`\`
...
Log the operational learning now. Then say what you logged.`,
    ...
    const projectsDir = path.join(REZOPSHome, 'projects');
