...
function setupWorkdir(suffix: string): { workDir: string; REZOPSHome: string; slug: string } {
  const workDir = fs.mkdtempSync(path.join(os.tmpdir(), `skill-e2e-ctx-${suffix}-`));
  const REZOPSHome = path.join(workDir, '.rezops-home');
  ...
  for (const script of [
    'rezops-timeline-log', 'rezops-timeline-read', 'rezops-slug',
    'rezops-learnings-log', 'rezops-learnings-search',
    'rezops-update-check', 'rezops-config', 'rezops-repo-mode',
  ]) {
...
Environment:
- Use REZOPS_HOME="${REZOPSHome}" for all rezops bin scripts.
...
`);
  ...
  return { workDir, REZOPSHome, slug };
}

// Helper: seed a saved-context file into the storage dir.
function seedSave(REZOPSHome: string, slug: string, filename: string, frontmatter: Record<string, string>, body: string) {
  const dir = path.join(REZOPSHome, 'projects', slug, 'checkpoints');
...
describeIfSelected('Context Skills E2E (live-fire)', [
  ...
  testConcurrentIfSelected('context-save-routing', async () => {
    const { workDir, REZOPSHome, slug } = setupWorkdir('routing');

    // Prompt pattern: the slash command + explicit "invoke via Skill tool"
    // instruction. The REZOPS_HOME / ./bin bash setup that used to be in
    ...
    const checkpointDir = path.join(REZOPSHome, 'projects', slug, 'checkpoints');
  ...
  testConcurrentIfSelected('context-save-then-restore-roundtrip', async () => {
    const { workDir, REZOPSHome, slug } = setupWorkdir('roundtrip');
...
Do NOT use AskUserQuestion.`,
  ...
  testConcurrentIfSelected('context-restore-fragment-match', async () => {
    const { workDir, REZOPSHome, slug } = setupWorkdir('fragment');

    // Seed three saves with distinct titles.
    seedSave(REZOPSHome, slug, '20260101-120000-alpha-feature.md',
    ...
    seedSave(REZOPSHome, slug, '20260202-120000-middle-payments.md',
    ...
    seedSave(REZOPSHome, slug, '20260303-120000-omega-release.md',
  ...
  testConcurrentIfSelected('context-restore-empty-state', async () => {
    const { workDir, REZOPSHome, slug } = setupWorkdir('empty');
  ...
  testConcurrentIfSelected('context-restore-list-delegates', async () => {
    const { workDir, REZOPSHome, slug } = setupWorkdir('delegates');
    seedSave(REZOPSHome, slug, '20260101-120000-seed.md',
  ...
  testConcurrentIfSelected('context-restore-legacy-compat', async () => {
    const { workDir, REZOPSHome, slug } = setupWorkdir('legacy');
    ...
    // exactly this reason.
    seedSave(REZOPSHome, slug, '20260301-120000-legacy-pre-rename-work.md',
      {
        status: 'in-progress',
  ...
  testConcurrentIfSelected('context-save-list-current-branch', async () => {
    const { workDir, REZOPSHome, slug } = setupWorkdir('list-current');

    // Seed 3 files on 3 different branches. Current branch is "main".
    seedSave(REZOPSHome, slug, '20260101-120000-main-work.md',
      { status: 'in-progress', branch: 'main', timestamp: '2026-01-01T12:00:00Z' },
      '## Working on: main work LISTCURR_MAIN_TOKEN\n');
    seedSave(REZOPSHome, slug, '20260202-120000-feat-alpha.md',
      { status: 'in-progress', branch: 'feat/alpha', timestamp: '2026-02-02T12:00:00Z' },
      '## Working on: alpha LISTCURR_ALPHA_TOKEN\n');
    seedSave(REZOPSHome, slug, '20260303-120000-feat-beta.md',
  ...
  testConcurrentIfSelected('context-save-list-all-branches', async () => {
    const { workDir, REZOPSHome, slug } = setupWorkdir('list-all');
