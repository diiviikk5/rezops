...
describeIfSelected('Learnings E2E', ['learnings-show'], () => {
  ...
  beforeAll(() => {
    ...
    for (const script of ['rezops-learnings-search', 'rezops-learnings-log', 'rezops-slug']) {
      fs.copyFileSync(path.join(ROOT, 'bin', script), path.join(binDir, script));
      fs.chmodSync(path.join(binDir, script), 0o755);
    }

    // Seed learnings JSONL — slug must match what rezops-slug computes.
    // With no git remote, rezops-slug falls back to basename(workDir).
...
- Replace any references to ~/.claude/skills/rezops/bin/rezops-slug with ./bin/rezops-slug.
