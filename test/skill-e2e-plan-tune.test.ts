...
describeIfSelected('PlanTune E2E', ['plan-tune-inspect'], () => {
  ...
  beforeAll(() => {
    ...
    for (const script of [
      ...
      'rezops-config',
      'rezops-question-log',
      'rezops-question-preference',
      'rezops-developer-profile',
      'rezops-builder-profile',
    ]) {
    ...
    // rezops-developer-profile --derive imports from scripts/ — copy those too.
    ...
    const cfgDir = path.join(REZOPSHome);
...
IMPORTANT:
- Use REZOPS_HOME="${REZOPSHome}" as an environment variable for all bin calls.
- Replace any ~/.claude/skills/rezops/bin/ references with ./bin/ (relative path).
- Replace any ~/.claude/skills/rezops/scripts/ references with ./scripts/.
