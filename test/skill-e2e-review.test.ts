...
describeIfSelected('Review Dashboard Via Attribution', ['review-dashboard-via'], () => {
  ...
  beforeAll(() => {
    ...
    // rezops-review-read reads from ~/.rezops/projects/$SLUG/$BRANCH-reviews.jsonl
    // For the test, we'll write a mock rezops-review-read script that returns our test data
    ...
    // Write a mock rezops-review-read that returns our test data
    const mockBinDir = path.join(dashDir, '.mock-bin');
    fs.mkdirSync(mockBinDir, { recursive: true });
    fs.writeFileSync(path.join(mockBinDir, 'rezops-review-read'), [
      '#!/usr/bin/env bash',
    ...
    fs.chmodSync(path.join(mockBinDir, 'rezops-review-read'), 0o755);
...
Instead of running ~/.claude/skills/rezops/bin/rezops-review-read, run this mock: ${mockBinDir}/rezops-review-read
