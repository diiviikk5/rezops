...
describeIfSelected('Session Intelligence E2E', [
  ...
  beforeAll(() => {
    ...
    // Compute slug (same logic as rezops-slug without git remote)
  ...
  // Write a timeline event via rezops-timeline-log, read it back via rezops-timeline-read.
  ...
  testConcurrentIfSelected('timeline-event-flow', async () => {
    const projectDir = path.join(REZOPSHome, 'projects', slug);
    fs.mkdirSync(projectDir, { recursive: true });

    // Write two events via the binary
    const logBin = path.join(workDir, 'bin', 'rezops-timeline-log');
    const readBin = path.join(workDir, 'bin', 'rezops-timeline-read');
    const env = { ...process.env, REZOPS_HOME: REZOPSHome };
    ...
    // Verify rezops-timeline-read output includes the events
...
IMPORTANT:
- Use REZOPS_HOME="${REZOPSHome}" as an environment variable when running bin scripts.
- The bin scripts are at ./bin/ (relative to this directory), not at ~/.claude/skills/rezops/bin/.
  Replace any references to ~/.claude/skills/rezops/bin/ with ./bin/ when running commands.
