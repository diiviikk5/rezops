...
describeIfSelected('Ship workflow E2E', ['ship-local-workflow'], () => {
  ...
  beforeAll(() => {
    shipRemoteDir = fs.mkdtempSync(path.join(os.tmpdir(), 'rezops-ship-remote-'));
    shipWorkDir = fs.mkdtempSync(path.join(os.tmpdir(), 'rezops-ship-work-'));
...
describeIfSelected('rezops-upgrade E2E', ['rezops-upgrade-happy-path'], () => {
  ...
  testConcurrentIfSelected('rezops-upgrade-happy-path', async () => {
    const mockGstack = path.join(upgradeDir, '.claude', 'skills', 'rezops');
    const result = await runSkillTest({
...
IMPORTANT: The install directory is at ./.claude/skills/rezops — use that exact path.`,
    ...
    recordE2E(evalCollector, '/rezops-upgrade happy path', 'rezops-upgrade E2E', result, {
