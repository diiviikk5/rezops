...
// Eval result collector — accumulates test results, writes to ~/.rezops-dev/evals/ on finalize
...
function dumpOutcomeDiagnostic(dir: string, label: string, report: string, judgeResult: any) {
  try {
    const transcriptDir = path.join(dir, '.rezops', 'test-transcripts');
...
Report whether it worked.`,
  ...
  testIfSelected('skillmd-no-local-binary', async () => {
    // Create a tmpdir with no browse binary — no local .claude/skills/rezops/browse/dist/browse
...
Report the exact output. Do NOT try to fix or install anything — just report what you see.`,
    ...
    // On dev machines with rezops installed globally, the fallback path
    // ~/.claude/skills/rezops/browse/dist/browse exists, so we get READY.
...
Report the exact output — either "READY: <path>" or "NEEDS_SETUP".`,
  ...
  testIfSelected('session-awareness', async () => {
    ...
    const result = await runSkillTest({
      prompt: `You are running a rezops skill. The session preamble detected _SESSIONS=4 (the user has 4 rezops windows open).
...
Write your report to ${qaOnlyDir}/qa-reports/qa-only-report.md`,
    ...
    const statusLines = gitStatus.stdout.toString().trim().split('\n').filter(
      (l: string) => l.trim() && !l.includes('.prompt-tmp') && !l.includes('.rezops/') && !l.includes('qa-reports/'),
    );
...
`);
    ...
    projectDir = path.join(os.homedir(), '.rezops', 'projects', 'test-project');
  ...
  test('/plan-eng-review writes test-plan artifact to ~/.rezops/projects/', async () => {
...
describeIfSelected('rezops-upgrade E2E', ['rezops-upgrade-happy-path'], () => {
  let upgradeDir: string;
  let remoteDir: string;

  beforeAll(() => {
    upgradeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'skill-e2e-upgrade-'));
    remoteDir = fs.mkdtempSync(path.join(os.tmpdir(), 'rezops-remote-'));
    ...
    // Create mock rezops install directory (local-git type)
    const mockGstack = path.join(upgradeDir, '.claude', 'skills', 'rezops');
    ...
    // Copy rezops-upgrade skill
    fs.mkdirSync(path.join(upgradeDir, 'rezops-upgrade'), { recursive: true });
    fs.copyFileSync(
      path.join(ROOT, 'rezops-upgrade', 'SKILL.md'),
      path.join(upgradeDir, 'rezops-upgrade', 'SKILL.md'),
    );
  ...
  testIfSelected('rezops-upgrade-happy-path', async () => {
    const mockGstack = path.join(upgradeDir, '.claude', 'skills', 'rezops');
    const result = await runSkillTest({
      prompt: `Read rezops-upgrade/SKILL.md for the upgrade workflow.

You are running /rezops-upgrade standalone. The rezops installation is at ./.claude/skills/rezops (local-git type — it has a .git directory with an origin remote).
...
IMPORTANT: The install directory is at ./.claude/skills/rezops — use that exact path.`,
      workingDirectory: upgradeDir,
      maxTurns: 20,
      timeout: 180_000,
      testName: 'rezops-upgrade-happy-path',
      runId,
    });

    logCost('/rezops-upgrade happy path', result);
    ...
    recordE2E('/rezops-upgrade happy path', 'rezops-upgrade E2E', result, {
