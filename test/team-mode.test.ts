...
const SETTINGS_HOOK = path.join(ROOT, 'bin', 'rezops-settings-hook');
const SESSION_UPDATE = path.join(ROOT, 'bin', 'rezops-session-update');
const TEAM_INIT = path.join(ROOT, 'bin', 'rezops-team-init');

function mkTmpDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'rezops-team-test-'));
}
...
describe('rezops-settings-hook', () => {
  ...
  test('add creates settings.json if missing', () => {
    const result = run(`${SETTINGS_HOOK} add /path/to/rezops-session-update`, {
      env: { REZOPS_SETTINGS_FILE: settingsFile },
    });
    expect(result.exitCode).toBe(0);
    const settings = JSON.parse(fs.readFileSync(settingsFile, 'utf-8'));
    expect(settings.hooks.SessionStart).toHaveLength(1);
    expect(settings.hooks.SessionStart[0].hooks[0].command).toBe('/path/to/rezops-session-update');
  });
  ...
  test('add deduplicates (running twice does not double-add)', () => {
    run(`${SETTINGS_HOOK} add /path/to/rezops-session-update`, {
      env: { REZOPS_SETTINGS_FILE: settingsFile },
    });
    run(`${SETTINGS_HOOK} add /path/to/rezops-session-update`, {
      env: { REZOPS_SETTINGS_FILE: settingsFile },
  ...
  test('remove removes the hook', () => {
    ...
    const result = run(`${SETTINGS_HOOK} remove /path/to/rezops-session-update`, {
      env: { REZOPS_SETTINGS_FILE: settingsFile },
    });
    expect(result.exitCode).toBe(0);
    const settings = JSON.parse(fs.readFileSync(settingsFile, 'utf-8'));
  ...
  test('remove preserves other hooks', () => {
    fs.writeFileSync(settingsFile, JSON.stringify({
      hooks: {
        SessionStart: [
          { hooks: [{ type: 'command', command: '/path/to/rezops-session-update' }] },
          { hooks: [{ type: 'command', command: '/other/hook' }] },
        ],
      },
    }, null, 2));
    run(`${SETTINGS_HOOK} remove /path/to/rezops-session-update`, {
      env: { REZOPS_SETTINGS_FILE: settingsFile },
    });
    const settings = JSON.parse(fs.readFileSync(settingsFile, 'utf-8'));
    expect(settings.hooks.SessionStart).toHaveLength(1);
...
describe('rezops-session-update', () => {
  let tmpDir: string;
  let REZOPSDir: string;
  let stateDir: string;

  beforeEach(() => {
    tmpDir = mkTmpDir();
    REZOPSDir = path.join(tmpDir, 'rezops');
    stateDir = path.join(tmpDir, 'state');
    fs.mkdirSync(REZOPSDir, { recursive: true });
    fs.mkdirSync(stateDir, { recursive: true });

    // Init a git repo to pass the .git guard
    execSync('git init', { cwd: REZOPSDir });
    execSync('git commit --allow-empty -m "init"', { cwd: REZOPSDir });
    fs.writeFileSync(path.join(REZOPSDir, 'VERSION'), '0.1.0');

    // Create a minimal rezops-config that returns auto_upgrade=true
    const binDir = path.join(REZOPSDir, 'bin');
    fs.mkdirSync(binDir, { recursive: true });
    fs.writeFileSync(path.join(binDir, 'rezops-config'), '#!/bin/bash\necho "true"');
    fs.chmodSync(path.join(binDir, 'rezops-config'), 0o755);
  });
  ...
  test('exits 0 when .git is missing', () => {
    fs.rmSync(path.join(REZOPSDir, '.git'), { recursive: true });
    const result = run(SESSION_UPDATE, {
      env: { REZOPS_DIR: REZOPSDir, REZOPS_STATE_DIR: stateDir },
    });
    expect(result.exitCode).toBe(0);
  });

  test('exits 0 when auto_upgrade is not true', () => {
    // Override rezops-config to return false
    fs.writeFileSync(path.join(REZOPSDir, 'bin', 'rezops-config'), '#!/bin/bash\necho "false"');
    const result = run(SESSION_UPDATE, {
      env: { REZOPS_DIR: REZOPSDir, REZOPS_STATE_DIR: stateDir },
    });
    expect(result.exitCode).toBe(0);
  ...
  test('always exits 0 (non-fatal)', () => {
    ...
    const result = run(SESSION_UPDATE, {
      env: { REZOPS_DIR: '/nonexistent/path', REZOPS_STATE_DIR: stateDir },
    });
    expect(result.exitCode).toBe(0);
  });
});

describe('rezops-team-init', () => {
  let tmpDir: string;
  ...
  test('optional: creates CLAUDE.md with recommended section', () => {
    ...
    expect(claude).toContain('## rezops (recommended)');
  ...
  test('required: creates CLAUDE.md with required section', () => {
    ...
    expect(result.exitCode).toBe(0);
    const claude = fs.readFileSync(path.join(tmpDir, 'CLAUDE.md'), 'utf-8');
    expect(claude).toContain('## rezops (REQUIRED');
    expect(claude).toContain('REZOPS_MISSING');
  });

  test('required: creates enforcement hook', () => {
    run(`${TEAM_INIT} required`, { cwd: tmpDir });
    const hookPath = path.join(tmpDir, '.claude', 'hooks', 'check-rezops.sh');
    expect(fs.existsSync(hookPath)).toBe(true);
    const hook = fs.readFileSync(hookPath, 'utf-8');
    expect(hook).toContain('BLOCKED: rezops is not installed');
  ...
  test('required: creates project settings.json with PreToolUse hook', () => {
    ...
    expect(settings.hooks.PreToolUse[0].hooks[0].command).toContain('check-rezops');
  });
  ...
  test('idempotent: running twice does not duplicate CLAUDE.md section', () => {
    ...
    run(`${TEAM_INIT} optional`, { cwd: tmpDir });
    const claude = fs.readFileSync(path.join(tmpDir, 'CLAUDE.md'), 'utf-8');
    const matches = claude.match(/## rezops/g);
    expect(matches).toHaveLength(1);
  });

  test('removes vendored copy when present', () => {
    // Create a fake vendored rezops with VERSION file
    const vendoredDir = path.join(tmpDir, '.claude', 'skills', 'rezops');
    ...
    // Track it in git
    execSync('git add .claude/skills/rezops/', { cwd: tmpDir });
    execSync('git commit -m "add vendored rezops"', { cwd: tmpDir });

    const result = run(`${TEAM_INIT} optional`, { cwd: tmpDir });
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toContain('Found vendored rezops copy');
    ...
    expect(gitignore).toContain('.claude/skills/rezops/');
  });

  test('skips when no vendored copy present', () => {
    const result = run(`${TEAM_INIT} optional`, { cwd: tmpDir });
    expect(result.exitCode).toBe(0);
    expect(result.stdout).not.toContain('Found vendored rezops copy');
  });

  test('skips when .claude/skills/rezops is a symlink', () => {
    ...
    fs.symlinkSync(targetDir, path.join(skillsDir, 'rezops'));

    const result = run(`${TEAM_INIT} optional`, { cwd: tmpDir });
    expect(result.exitCode).toBe(0);
    expect(result.stdout).not.toContain('Found vendored rezops copy');
    // Symlink should still exist
    expect(fs.lstatSync(path.join(skillsDir, 'rezops')).isSymbolicLink()).toBe(true);
  ...
  test('does not duplicate .gitignore entry on re-run', () => {
    ...
    const gitignore = fs.readFileSync(path.join(tmpDir, '.gitignore'), 'utf-8');
    const matches = gitignore.match(/\.claude\/skills\/rezops\//g);
    expect(matches).toHaveLength(1);
  });
});
...
describe('setup --team / --no-team / -q', () => {
  ...
  test(
    ...
    () => {
      ...
      // The key test is that the "Skill naming:" prompt and "rezops ready" messages are suppressed
      expect(result.stdout).not.toContain('Skill naming:');
      expect(result.stdout).not.toContain('rezops ready');
    },
    180_000,
  );
