...
function runLog(input: string, opts: { expectFail?: boolean } = {}): { stdout: string; exitCode: number } {
  ...
  try {
    const stdout = execSync(`${BIN}/rezops-timeline-log '${input.replace(/'/g, "'\\''")}'`, execOpts).trim();
...
function runRead(args: string = ''): string {
  ...
  try {
    return execSync(`${BIN}/rezops-timeline-read ${args}`, execOpts).trim();
  } catch {
    return '';
  }
}
...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'rezops-timeline-'));
